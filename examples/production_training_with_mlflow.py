...
def train_with_tracking(config: Dict[str, Any], run_dir: str = "artifacts/runs") -> Dict[str, float]:
    ...
    if mlflow_enabled and is_mlflow_available():
        ...
        if mlflow_enabled and not is_mlflow_available():
            logger.warning("MLflow tracking requested but MLflow not available. Continuing without tracking.")
        else:
