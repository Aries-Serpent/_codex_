...
def demo_chaos_theory():
    ...
    for i, params in enumerate(test_cases[:10]):
        print(f"  Test {i+1}: complexity={params[0]:.2f}, "
...
def demo_fractal_geometry():
    ...
    for anomaly in anomalies:
        print(f"    Structure {anomaly['index']}: dimension={anomaly['fractal_dimension']:.2f}, "
...
def demo_fluid_dynamics():
    ...
    for ch_id, channel in scheduler.channels.items():
        print(f"  {ch_id}: flow={channel.current_flow:.1f}, "
    ...
    print(f"  Pressure variance: {result['initial']['pressure_variance']:.3f} → "
...
def demo_electromagnetic_fields():
    ...
    for i, region in enumerate(regions):
        print(f"  Region {i+1}: position={region['position']}, "
              f"priority={region['priority']:.2f}")
...
def demo_wave_propagation():
    ...
    for i, source in enumerate(sources):
        ...
        print(f"  Source {i+1}: position={source['position']}, "
              f"amplitude={source['amplitude']}")
...
def demo_unified_orchestration():
    ...
    decision_space = {
        ...
        'hotspots': [
            ...
            (np.array([0.7, 0.7]), 3.0)
        ],
        'signals': [
            {'position': (20, 20), 'amplitude': 1.0, 'frequency': 1.0}
        ],
        'agents': [
