...
def audit(session: nox.Session) -> None:
    ...
    recorder.record_statement(
        "Starting offline audit workflow (install dev deps, pytest, space audit).",
        category="AUDIT",
        verified=True,
    )
    ...
    try:
        ...
        recorder.record_statement(
            "Development dependencies installed for audit run.",
            category="ASSERTED",
            verified=True,
        )
    ...
    except subprocess.CalledProcessError as exc:  # pragma: no cover - surfaced to nox
        success = False
