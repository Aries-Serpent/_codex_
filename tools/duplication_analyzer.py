    ...
    def analyze(self, extensions: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        self.stats = {
            ...
            "content_duplicate_groups": len(
                [h for h, paths in self.content_hashes.items() if len(paths) > 1]
            ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if ratio < self.acceptable_ratio:
            recommendations.append(
                f"✅ Duplication ratio ({ratio:.2%}) is within acceptable limits."
            )
        elif ratio < WARNING_DUP_RATIO:
            recommendations.append(
                f"⚠️ Duplication ratio ({ratio:.2%}) is elevated. Review duplicate groups."
            )
        else:
            recommendations.append(
                f"🔴 Duplication ratio ({ratio:.2%}) is high. Immediate action recommended."
            )

        if self.duplicate_groups:
            recommendations.append(
                f"Found {len(self.duplicate_groups)} groups of files with duplicate names."
            )
