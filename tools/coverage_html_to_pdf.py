...
def create_combined_html(html_files: List[Path], output_path: Path) -> None:
    ...
    for i, html_file in enumerate(html_files):
        try:
            ...
            # Add source header
            combined_content.append(f"<h2>Source: {html_file.relative_to(ROOT) if html_file.is_relative_to(ROOT) else html_file}</h2>")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not html_files:
        ...
        print(f"Searched in: {search_dirs or [ROOT / d for d in DEFAULT_COVERAGE_DIRS]}", file=sys.stderr)
        return 1
