        ...
        with open(output_md, "w", encoding="utf-8") as f:
            ...
            for category, items in sorted(by_category.items()):
                ...
                for issue in items[:10]:  # Limit to first 10 per category
                    f.write(
                        f"|{Path(issue.filename).name}|{issue.lineno}|{issue.message}|{issue.severity.value}|\n"
                    )
                if len(items) > 10:
...
def main():
    ...
    parser.add_argument(
        "root", nargs="?", default="src", help="Root directory to scan (default: src)"
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed suggestions")
    parser.add_argument(
        "--check-walrus", action="store_true", help="Check for walrus operator opportunities"
    )
    ...
    parser.add_argument(
        "--fail-on-error", action="store_true", help="Exit with error code if ERROR severity found"
    )
    ...
    for py_file in root_path.rglob("*.py"):
        ...
        if any(part.startswith(".") for part in py_file.parts):
            continue
        if any(
            part in ("node_modules", "venv", "__pycache__", "build", "dist")
    ...
    if args.json or args.md:
        generate_report(all_issues, output_json=args.json, output_md=args.md)
