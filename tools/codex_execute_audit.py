...
import argparse, contextlib, datetime as dt, json, os, re, subprocess, sys, textwrap
...
def write_error(step_num: str, step_desc: str, err: Exception, ctx: str = "") -> None:
    ...
    with ERRORS_NDJSON.open("a", encoding="utf-8") as f:
        json.dump({"ts": utcnow(), "step": step_num, "desc": step_desc,
                   "error": f"{type(err).__name__}: {err}", "context": ctx}, f)
        f.write("\n")
    ...
    try:
        for candidate in ["README.md", "Readme.md", "readme.md"]:
            ...
            if p.exists():
                ...
                if changed:
                    append_changelog("README cleanup", [f"Sanitized links in {candidate} for offline use"])
                return
...
def generate_lockfile():
    ...
    try:
        ...
        write_error(step, "Generate requirements.lock", e, "pip freeze failed (offline environment?)")
...
def suggest_tests_and_gates():
    ...
    try:
        ...
        append_changelog("Local gates", ["Added codex_local_gates.sh for offline lint/tests/coverage"])
    except Exception as e:
        write_error(step, "Create local gates script", e, "")

def main():
    parser = argparse.ArgumentParser(description="Codex offline executor for Implementation Status Audit")
