...
def _write_json(path: Path, data: dict[str, object]) -> None:
    ...
    path.write_text(  # nosec  # codeql[py/clear-text-storage-sensitive-data] Snippets are replaced with <redacted> sentinel before storing
...
def _write_markdown(path: Path, data: dict[str, object]) -> None:
    ...
    lines: list[str] = []
    lines.append("# `_codex_` Secret Scan Stub\n")  # nosec  # codeql[py/clear-text-storage-sensitive-data] Static header text only
