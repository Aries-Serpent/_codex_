...
class TestSuiteAnalyzer:
    ...
    def generate_report(self, metrics: Dict[str, TestMetrics]) -> str:
        ...
        report.append(f"Assertions per Test:    {total_assertions/total_methods if total_methods > 0 else 0:.2f}")
        report.append(f"Expected Coverage Gain: {total_expected_gain:.2f}%")
        report.append("")
        
        # Phase breakdown
        phase1_files = [k for k in metrics.keys() if 'phase1' in k.lower() or 'phase_1' in k.lower() or any(x in k for x in ['smoke', 'expanded', 'invariant', 'properties', 'final_push', '30pct', 'exhaustive'])]
        phase2_files = [k for k in metrics.keys() if 'phase2' in k.lower() or 'phase_2' in k.lower()]
        ...
        if phase2_files:
            ...
            phase1_final = 27.57 + (sum(metrics[k].expected_coverage_gain for k in phase1_files) if phase1_files else 0)
        ...
        for filename in sorted(metrics.keys()):
            m = metrics[filename]
            report.append(f"{filename:<45} {m.test_classes:<8} {m.test_methods:<8} {m.expected_coverage_gain:<8.2f}%")
        ...
        report.append(f"Test Method Density:            {total_methods / len(metrics):.1f} tests/file")
        ...
        report.append("-" * 80)
        current = 27.57
        after_phase1 = current + (sum(metrics[k].expected_coverage_gain for k in phase1_files) if phase1_files else 0)
        after_phase2 = after_phase1 + (sum(metrics[k].expected_coverage_gain for k in phase2_files) if phase2_files else 0)
        ...
        for m in metrics.values():
            for target in m.targeted_classes:
                # Determine module
                if any(x in target.lower() for x in ['physics', 'orchestrator', 'decision', 'force', 'action']):
                    module_tests['physics_orchestrator'] += 1
                elif any(x in target.lower() for x in ['quantum', 'game', 'payoff', 'strategy', 'bell']):
                    module_tests['quantum_game_theory'] += 1
                elif any(x in target.lower() for x in ['mental', 'mapping', 'reasoning', 'concept', 'knowledge']):
