...
commits_output = run_git(
    "git log --format='%H|||%s|||%b|||%an|||%ad' main..0D_base_ -100"
)
...
for entry in commits_output.split('\n'):
    if '|||' in entry:
        parts = entry.split('|||', 4)
        if len(parts) >= 4:
            sha, subject, body, author, date = parts[0], parts[1], parts[2] if len(parts) > 2 else "", parts[3] if len(parts) > 3 else "", parts[4] if len(parts) > 4 else ""
            commits_list.append({
                "sha": sha[:8],
                "message": subject,
                "body": body,
...
for commit in commits_list:
    ...
    if any(kw in full_text for kw in ['audit', 'scoring', 'coverage_map', 'token_similarity', 'v1.4']):
    ...
    if objectives:
        if 'docs' in objectives and has_docs:
            ...
            evidence_found = "yes"
        elif ('feature' in objectives or 'bugfix' in objectives or 'audit' in objectives) and has_code:
            evidence_found = "yes"
...
with open(csv_path, 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=[
        'commit_sha', 'objective_summary', 'files_changed', 
        'evidence_found', 'evidence_notes', 'message'
    ])
    writer.writeheader()
