...
## COMPREHENSIVE USER PROMPT FOR COPILOT
...
1. **Verify ALL 126 modified files** are appropriate for main branch integration
2. **Validate CI/CD pipeline integrity** — ensure all 40+ workflows execute correctly
3. **Confirm security gates pass** — Bandit, Semgrep, secrets detection
4. **Validate test coverage** — enforce 96% minimum threshold
5. **Check dependency management** — GPU package removal, lock file consistency
6. **Review documentation** — Copilot.md enhancements, PR #2224 review response
7. **Authenticate ownership** — CODEOWNERS assignments for all modified files
8. **Plan post-merge validation** — ensure monitoring & rollback procedures ready

### 📋 CRITICAL FILES TO INSPECT

| File | Impact | Action |
|------|--------|--------|
| `.github/Copilot.md` | CI/CD Guidelines | ☐ Review for accuracy & completeness |
| `.github/coverage_threshold.txt` | Coverage Enforcement | ☐ Verify 96% threshold justified |
| `.bandit.yaml` | Security Scanning | ☐ Validate exclusions & targets |
| `uv.lock` | Dependency Lock | ☐ Confirm NO GPU packages |
| `.github/CODEOWNERS` | Ownership | ☐ All 126 files have owner |
| `.github/workflows/post-merge-validation-optimized.yml` | Post-Merge | ☐ Workflow chain complete |
