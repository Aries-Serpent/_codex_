...
**Agent Name**: Config Validator Agent
**Agent Type**: Monitoring & Validation
**Energy Level**: 3/5
**Operational Status**: ✅ Active

### Purpose
This agent provides specialized functionality for config validator agent operations within the Codex ecosystem.
...
```yaml
agent_type: config-validator-agent
prompt: |
...
```bash
# Via task tool
task agent_type="config-validator-agent" description="<description>" prompt="<prompt>"
```

### GitHub Actions Trigger

```yaml
- name: Activate config-validator-agent
  uses: ./.github/actions/agent-runner
  with:
    agent: config-validator-agent
    parameters: |
