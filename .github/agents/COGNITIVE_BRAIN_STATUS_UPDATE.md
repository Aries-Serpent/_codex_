...
```yaml
agent_type: cognitive-brain-status-update
prompt: |
...
```bash
# Via task tool
task agent_type="cognitive-brain-status-update" description="<description>" prompt="<prompt>"
```

### GitHub Actions Trigger

```yaml
- name: Activate cognitive-brain-status-update
  uses: ./.github/actions/agent-runner
  with:
    agent: cognitive-brain-status-update
    parameters: |
