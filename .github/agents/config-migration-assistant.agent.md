...
**Agent Name**: Config Migration Assistant Agent
**Agent Type**: Specialized Domain
**Energy Level**: 3/5
**Operational Status**: ✅ Active

### Purpose
This agent provides specialized functionality for config migration assistant agent operations within the Codex ecosystem.
...
```yaml
agent_type: config-migration-assistant-agent
prompt: |
...
```bash
# Via task tool
task agent_type="config-migration-assistant-agent" description="<description>" prompt="<prompt>"
```

### GitHub Actions Trigger

```yaml
- name: Activate config-migration-assistant-agent
  uses: ./.github/actions/agent-runner
  with:
    agent: config-migration-assistant-agent
    parameters: |
