...
from phase8_9_emergent_behavior import (
    ...
    CAPABILITY_TAXONOMY_DEPTH,
    COHERENCE_DECAY,
    CONSENSUS_THRESHOLD,
    IMPROVEMENT_THRESHOLD,
    # Constants
    K1_PHASE_8_9_TARGET,
    META_META_RECURSION_DEPTH,
    PATTERN_NOVELTY_THRESHOLD,
    PLANNING_MAX_DEPTH,
...
class TestSelfImprovementEngine:
    ...
    def test_improvement_threshold(self):
        ...
        assert not engine.evaluate_improvement_opportunity("metric", 1.09)

        # At threshold
        assert engine.evaluate_improvement_opportunity("metric", 1.10)

        # Above threshold
        assert engine.evaluate_improvement_opportunity("metric", 1.20)
    ...
    def test_rollback_threshold(self):
        ...
        assert not engine.check_rollback_needed("metric", 0.86)

        # At threshold
        assert engine.check_rollback_needed("metric", 0.85)

        # Below threshold
        assert engine.check_rollback_needed("metric", 0.80)
