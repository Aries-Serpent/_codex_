...
FIXES APPLIED:
- Deduplication layer in payload preparation (P0)
- Turn-state isolation with unique turn IDs (P0)
- Pre-flight validation for function calls (P0)
- _generate_integration_guidance() string literal fix (P0 patch)
...
# ============================================================================
# Deduplication & Turn State Management
# ============================================================================

@dataclass
class TurnState:
    """Tracks state isolation for each agentic turn."""
    turn_id: str
    timestamp: str
    function_call_ids: set[str] = field(default_factory=set)
    payload_hash: str = ""
    is_finalized: bool = False

    def add_function_call(self, call_id: str) -> None:
        """Register a function call in this turn."""
        self.function_call_ids.add(call_id)

    def has_duplicate(self, call_id: str) -> bool:
        """Check if call_id already exists in this turn."""
        return call_id in self.function_call_ids

    def finalize(self) -> None:
        """Mark turn as finalized to prevent further mutations."""
        self.is_finalized = True


class PayloadDeduplicator:
    """Handles deduplication of function calls in agentic payloads."""

    def __init__(self) -> None:
        """Initialize deduplicator."""
        self.global_call_registry: dict[str, str] = {}
        self.turn_history: dict[str, TurnState] = {}
        self.current_turn: Optional[TurnState] = None

    def start_new_turn(self) -> str:
        """
        Start a new agentic turn with isolated state.

        Returns:
            Unique turn_id for this turn
        """
        turn_id = (
            f"turn_{uuid.uuid4().hex[:8]}_"
        ...
        self.turn_history[turn_id] = self.current_turn
        logger.info(f"🔄 New agentic turn started: {turn_id}")
        return turn_id

    def finalize_turn(self) -> None:
        """Finalize current turn and prepare for next."""
        if self.current_turn:
            self.current_turn.finalize()
            logger.info(f"✅ Turn {self.current_turn.turn_id} finalized")
            self.current_turn = None

    def deduplicate_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        """
        Deduplicate function calls in payload.

        Args:
            payload: Raw agentic payload (potentially with duplicates)

        Returns:
            Cleaned payload with duplicates removed
        """
        if not self.current_turn:
            raise RuntimeError("No active turn. Call start_new_turn() first.")
        ...
        duplicates_in_payload: list[dict] = []
        unique_calls: list[dict] = []

        for call in tool_calls:
            call_id = call.get("id", "")

            if call_id in seen_in_payload:
                logger.warning(f"⚠️  Duplicate within payload detected: {call_id}")
                duplicates_in_payload.append(call)
                continue

            if self.current_turn.has_duplicate(call_id):
                logger.warning(f"⚠️  Duplicate in current turn detected: {call_id}")
                duplicates_in_payload.append(call)
                continue

            if call_id in self.global_call_registry:
                prev_turn_id = self.global_call_registry[call_id]
                logger.error(
                    f"🚨 CRITICAL: Cross-turn contamination detected! "
                    f"Call {call_id} from turn {prev_turn_id} re-appeared "
                    f"in {self.current_turn.turn_id}"
                )
                duplicates_in_payload.append(call)
                continue

            seen_in_payload.add(call_id)
            self.current_turn.add_function_call(call_id)
            self.global_call_registry[call_id] = self.current_turn.turn_id
            unique_calls.append(call)

        if duplicates_in_payload:
            logger.warning(
                f"🧹 Removed {len(duplicates_in_payload)} duplicate(s) from payload"
            )

        cleaned_payload = payload.copy()
        cleaned_payload["tool_calls"] = unique_calls

        payload_hash = hashlib.sha256(
            json.dumps(unique_calls, sort_keys=True, default=str).encode()
        ).hexdigest()[:16]
        self.current_turn.payload_hash = payload_hash

        logger.info(
            f"✅ Payload deduplicated: {len(unique_calls)} calls, hash={payload_hash}"
        )
        return cleaned_payload

    def get_turn_stats(self, turn_id: str) -> dict[str, Any]:
        """Get statistics for a turn."""
        turn = self.turn_history.get(turn_id)
        if not turn:
        ...
        return {
            ...
            "function_calls": len(turn.function_call_ids),
            "payload_hash": turn.payload_hash,
            "is_finalized": turn.is_finalized,
        }
...
class KnowledgeHungerEngine:
    ...
    ) -> list[IntelligentQuestion]:
        """Generate intelligent questions from gaps."""
        questions: list[IntelligentQuestion] = []
        prioritized = sorted(
            gaps, key=lambda g: g.impact * g.confidence, reverse=True
        )

        for gap in prioritized[:10]:
    ...
    def _formulate_question(self, gap: KnowledgeGap) -> IntelligentQuestion:
        ...
        templates = {
            "conceptual": (
                f"What is the fundamental concept behind {gap.concept} in {gap.domain}?"
            ),
            "technical": (
                f"How is {gap.concept} implemented in production {gap.domain} systems?"
            ),
            "practical": (
                f"What are real-world examples of {gap.concept} in {gap.domain}?"
            ),
            "integration": (
                f"How can {gap.concept} be integrated with existing systems?"
            ),
        }

        question_text = templates.get(q_type, f"What is {gap.concept}?")
        hints = gap.question_hints or self._generate_question_hints(
            gap.concept, gap.context
        )
        hints.extend(self._get_domain_hints(gap.domain))
    ...
    def _generate_curiosity_question(
        ...
        if len(domains) >= 2:
            question_text = (
                f"How might {domains[0]} principles enhance {domains[1]} implementations?"
            )
        else:
            question_text = (
                "What emerging patterns in 2024 could revolutionize this approach?"
            )
        ...
        return IntelligentQuestion(
            ...
            follow_ups=[
                ...
                "How might this evolve in 5 years?",
            ],
        )
    ...
    def _generate_gap_id(self, gap_type: str, content: str) -> str:
        ...
        return (
            f"gap_{gap_type}_"
            f"{hashlib.md5(str(content).encode(), usedforsecurity=False).hexdigest()[:8]}"
        )
...
class QuestionPresenter:
    ...
    def present(
        self, questions: list[IntelligentQuestion], context: dict
    ) -> str:
        """Present questions in research-friendly format."""
        sections: list[str] = []
        sections.append(
            f"## 🧠 Knowledge Hunger: {len(questions)} Questions for Growth\n"
        )
        sections.append(
            f"**Context**: {context.get('current_task', 'General learning')}"
        )
        sections.append(
            f"**Domains**: {', '.join(context.get('domains', ['general']))}\n"
        )
        sections.append("---\n")
        ...
        by_type: dict[str, list[IntelligentQuestion]] = defaultdict(list)
        for q in questions:
    ...
    def _generate_integration_guidance(self) -> str:
        ...
        # NOTE: backtick fences inside the triple-quoted string are intentional.
        # Do NOT replace them with triple-quotes (that would terminate the string).
        return (
            "## 🔄 How to Feed Answers Back\n\n"
            "### Quick Answer Format\n"
            "```\n"
            ...
            "}\n"
            "```\n\n"
            "**Integration Ready**: I can immediately integrate any format of answer "
            "you provide!"
        )
...
class KnowledgeIntegrator:
    ...
    def __init__(self, evolution_state: EvolutionState) -> None:
    ...
    def _update_evolution_state(self, knowledge: list[str]) -> None:
        ...
        self.evolution_state.fitness = min(
            1.0, self.evolution_state.fitness + 0.01
        )
    ...
    def _generate_new_questions(self, knowledge: list[str]) -> list[str]:
        ...
        if knowledge:
            ...
            questions.append(
                f"What are advanced applications of this: {first_item[:50]}...?"
            )
    ...
    def _generate_summary(self, result: dict) -> str:
        ...
        summary = [
            "✅ Knowledge integrated successfully!",
            f"📚 Learned {len(result['knowledge_gained'])} concepts",
        ]
        if result["capabilities_enhanced"]:
            summary.append(
                f"⚡ Enhanced: {', '.join(result['capabilities_enhanced'])}"
            )
...
class IntegratedEvolutionSystem:
    ...
    async def process_task_with_learning(self, task: dict) -> dict:
        ...
        try:
            ...
            if gaps:
                ...
                result["continuation"] = self._generate_continuation(
                    task, gaps, formatted
                )
            else:
                result["continuation"] = (
                    "✅ No knowledge gaps detected. Ready to proceed!"
                )
            ...
            self.deduplicator.finalize_turn()
    ...
    def deduplicate_agentic_payload(
        self, payload: dict[str, Any]
    ) -> dict[str, Any]:
        """Public method to deduplicate agentic payloads."""
        if not self.deduplicator.current_turn:
            self.deduplicator.start_new_turn()
        return self.deduplicator.deduplicate_payload(payload)
    ...
    def _generate_continuation(
        ...
        if task.get("incomplete"):
            sections.append(
                f"🔄 **Task Continuation**: {task['description']}"
            )
            sections.append(f"**Progress**: {task.get('progress', 0)}%\n")
        if len(gaps) > 3:
            sections.append("🧠 **Strong Knowledge Hunger Detected!**")
            sections.append(
                f"I encountered {len(gaps)} concepts I'd love to understand better.\n"
            )
        elif gaps:
        ...
        sections.append(
            f"- Patterns Learned: {self.evolution_state.patterns_learned}"
        )
        return "\n".join(sections)

    def _generate_error_continuation(
        self, error: Exception, task: dict
    ) -> str:
        """Generate continuation with actionable recovery suggestions."""
        ...
        return (
            ...
            f"**Recovery Actions**:\n{recovery}\n\n"
            "**Next Steps**:\n"
            "1. Review the error details above and apply the matching recovery action\n"
            "2. Re-run this task after applying the fix\n"
            "3. If the error persists, escalate via `@copilot fix` with this error context"
        )
...
async def main() -> None:
    ...
    answer = {
        "answer": (
            "Quantum key distribution uses quantum states to distribute encryption "
            "keys securely. Recent implementations include BB84 and E91 protocols."
        ),
        "examples": [
            "BB84 protocol implementation",
            "Hybrid QKD+classical crypto",
        ],
    ...
    print(
        f"\n Evolution Status: "
        f"{json.dumps(knowledge_result['evolution_status'], indent=2)}"
    )
