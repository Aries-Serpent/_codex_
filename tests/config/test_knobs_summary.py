...
def test_knobs_summary_sidecar():
    ...
    Path("audit_artifacts/capabilities_scored.json").write_text(json.dumps({"capabilities":[]}), encoding="utf-8")
    Path("audit_artifacts/context_index.json").write_text(json.dumps({"files":[]}), encoding="utf-8")
