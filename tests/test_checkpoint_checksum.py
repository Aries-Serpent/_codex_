...
def test_save_load_checkpoint_with_integrity(tmp_path, mock_model, mock_optimizer):
    ...
    assert torch.equal(new_model.weights["layer.weight"], mock_model.weights["layer.weight"])
    assert new_optimizer.state["lr"] == mock_optimizer.state["lr"]


def test_checksum_verification_edge_cases(tmp_path):
    """Test edge cases in checksum verification."""
    ckpt_path = tmp_path / "edge_case.pt"
    ...
    large_data = {"data": torch.randn(1000, 1000)}
    save_ckpt(large_data, str(ckpt_path))
    verify_ckpt_integrity(str(ckpt_path))
    
    # Verify checksum reflects the large file
    meta = json.loads((tmp_path / "checksums.json").read_text())
    assert meta["bytes"] > 1000  # Should be substantial size


def test_concurrent_checksum_operations(tmp_path):
    """Test checksum operations with multiple files."""
    files = ["model1.pt", "model2.pt", "model3.pt"]
    states = [
        {"weights": torch.tensor([1.0])},
        {"weights": torch.tensor([2.0, 3.0])},
        {"weights": torch.tensor([4.0, 5.0, 6.0])}
    ]
    
    # Save multiple checkpoints
    for file, state in zip(files, states):
        save_ckpt(state, str(tmp_path / file))
    
    # Verify all checksums
    for file in files:
        verify_ckpt_integrity(str(tmp_path / file))
    
    # Each should have its own checksum file
    for file in files:
        expected_checksum = tmp_path / "checksums.json"
        # Note: save_ckpt creates checksums.json in the same directory
        # Multiple files would overwrite each other's checksums.json
        # This test demonstrates the limitation and expected behavior


def test_checksum_with_special_characters(tmp_path):
    """Test checksum handling with special filename characters."""
    special_files = ["model-v1.0.pt", "model_backup.pt", "model (copy).pt"]
    
    for filename in special_files:
        file_path = tmp_path / filename
        state = {"data": torch.tensor([1.0, 2.0])}
        
        save_ckpt(state, str(file_path))
        verify_ckpt_integrity(str(file_path))
        
        # Verify checksum file contains correct filename
        meta = json.loads((tmp_path / "checksums.json").read_text())
        assert meta["file"] == filename


def test_checksum_error_conditions(tmp_path):
    """Test error conditions in checksum verification."""
    ckpt_path = tmp_path / "test.pt"
    
    # Test with non-existent file
    verify_ckpt_integrity(str(tmp_path / "nonexistent.pt"))  # Should not raise
    
    # Test with malformed checksum file
    torch.save({"data": torch.tensor([1.0])}, ckpt_path)
    checksums_file = tmp_path / "checksums.json"
...
@pytest.mark.parametrize("file_size", [0, 1, 100, 1000, 10000])
def test_checksum_different_sizes(tmp_path, file_size):
    """Parametrized test for different file sizes."""
    ckpt_path = tmp_path / f"size_{file_size}.pt"
    
    # Create data of specified size
    if file_size == 0:
        data = {}
    else:
        # Create tensor with approximately target byte size
        elements = max(1, file_size // 4)  # float32 = 4 bytes
        data = {"weights": torch.randn(elements)}
    
    save_ckpt(data, str(ckpt_path))
    verify_ckpt_integrity(str(ckpt_path))
    
    # Verify checksum metadata reflects actual file size
    meta = json.loads((tmp_path / "checksums.json").read_text())
    assert meta["bytes"] == ckpt_path.stat().st_size
    assert meta["file"] == f"size_{file_size}.pt"


__all__ = [
    "MockModel",
    "MockOptimizer",
    "test_checkpoint_checksum_verify",
    "test_checksum_roundtrip",
    "test_checksum_missing_file",
    "test_checksum_file_mismatch",
    "test_save_load_checkpoint_with_integrity",
    "test_checksum_verification_edge_cases",
    "test_concurrent_checksum_operations",
    "test_checksum_with_special_characters",
    "test_checksum_error_conditions",
    "test_checksum_different_sizes",
]
