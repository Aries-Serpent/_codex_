...
class TestArithmeticOperatorMutations:
    ...
    def test_addition_subtraction_mutation(self):
        ...
        # Addition test
        sum_result = a + b
        assert sum_result == 15, "Result must not be empty"
        assert sum_result != 5, "Result must not be empty"

        # Subtraction test
        diff_result = a - b
        assert diff_result == 5, "Result must not be empty"
        assert diff_result != 15, "Result must not be empty"
    ...
    def test_multiplication_division_mutation(self):
        ...
        # Multiplication test
        mul_result = a * b
        assert mul_result == 50, "Result must not be empty"
        assert mul_result != 2, "Result must not be empty"

        # Division test
        div_result = a / b
        assert div_result == 2.0, "Result must not be empty"
        assert abs(div_result - 50) > 0.001, "Value must be greater than zero"

    def test_addition_with_zero(self):
        """Addition with identity element (0)"""
        x = 42
        assert x + 0 == 42, "0 is not valid"
        assert 0 + x == 42, "x is not valid"

        # Mutation detection: 0 - x would give different result
        assert x - 0 == x, "0 is not valid"
        assert 0 - x == -x, "x is not valid"
        assert x + 0 != 0 + (-x), "0 is not valid"

    def test_multiplication_with_zero(self):
        """Multiplication with zero"""
        x = 42
        assert x * 0 == 0, "0 is not valid"
        assert 0 * x == 0, "x is not valid"

        # Mutation detection: / 0 would error or differ
        assert x * 1 == x, "1 is not valid"
        assert 1 * x == x, "x is not valid"
    ...
    def test_division_by_one(self):
        ...
        assert (x / 1) == (x * 1), "Condition must be true"
    ...
    def test_operator_commutativity(self, x):
        ...
        if x != y:
            assert (x - y) == -(y - x), "Condition must be true"
    ...
    def test_operator_associativity(self):
        ...
        assert (a - b) - c != a - (b - c), "c is not valid"
        assert (a - b) - c == a - b - c, "c is not valid"
        assert a - (b - c) == a - b + c, "Condition must be true"
...
class TestModuloAndSignOperations:
    ...
    def test_modulo_basic_operations(self):
        ...
        # x % y gives remainder
        assert 10 % 3 == 1, "3 is not valid"
        assert 10 % 5 == 0, "5 is not valid"
        assert 15 % 4 == 3, "4 is not valid"

        # Mutation detection: % becomes / or other operators
        assert 10 % 3 != 10 / 3, "3 is not valid"
        assert 10 % 3 != 10 - 3, "3 is not valid"

    def test_modulo_with_negative_numbers(self):
        """Modulo with negative operands"""
        assert -10 % 3 == 2, "3 is not valid"
        assert 10 % -3 == -2, "3 is not valid"
        assert -10 % -3 == -1, "3 is not valid"

        # Consistency checks
        x, y = -10, 3
        assert (x // y) * y + (x % y) == x, "Condition must be true"

    def test_negative_number_arithmetic(self):
        """Arithmetic with negative numbers"""
        assert -5 + 10 == 5, "10 is not valid"
        assert -5 - 10 == -15, "10 is not valid"
        assert -5 * -10 == 50, "10 is not valid"
        assert -10 / 2 == -5, "2 is not valid"

        # Mutation detection
        assert -5 + 10 != -5 - 10, "10 is not valid"
        assert -5 * 2 == -(5 * 2), "2 is not valid"

    def test_absolute_value_consistency(self):
        """Test absolute value and sign handling"""
        for x in [10, -10, 0]:
            abs_x = abs(x)
            assert abs_x >= 0, "abs_x must be greater than zero"

            if x >= 0:
                assert abs_x == x, "abs_x is not valid"
            else:
                assert abs_x == -x, "abs_x is not valid"
    ...
    def test_sign_function_behavior(self, x):
        ...
        if x > 0:
            assert x + 1 > 0, "1 must be greater than zero"
            assert (x - 1 >= 0 or x - 1 < 0), "1 must be greater than zero"
        elif x == 0:
            assert x + 1 > 0, "1 must be greater than zero"
            assert x - 1 < 0, "1 is not valid"
        else:  # x < 0
            assert x - 1 < 0, "1 is not valid"
            assert (x + 1 <= 0 or x + 1 > 0), "1 must be greater than zero"
...
class TestReturnValueMutations:
    ...
    def test_return_true_false_mutation(self):
        ...
        # Test True return
        assert is_positive(5), "Condition must be true"
        assert is_positive(5), "Condition must be true"

        # Test False return
        assert not is_positive(-5), "not is not valid"
        assert not is_positive(-5), "not is not valid"

        # Test return value consistency
        assert is_positive(5) == (not is_negative(5)), "not is not valid"
    ...
    def test_return_type_consistency(self):
        ...
        assert get_first([]) is None, "Condition must be true"
    ...
    def test_return_early_vs_late(self):
        ...
        # Early return paths
        assert process_with_early_return(None) is None, "Condition must be true"
        assert process_with_early_return(-5) == 0, "Condition must be true"

        # Normal return
        assert process_with_early_return(5) == 10, "Condition must be true"

        # Mutation detection: missing early returns
        assert process_with_early_return(None) != 0, "Condition must be true"
        assert process_with_early_return(-5) != -10, "Condition must be true"
...
class TestComplexArithmeticExpressions:
    ...
    def test_order_of_operations(self):
        ...
        assert 2 + 3 * 4 == 14, "4 is not valid"
        assert (2 + 3) * 4 == 20, "4 is not valid"

        # Division before subtraction
        assert 20 - 8 / 2 == 16, "2 is not valid"
        assert (20 - 8) / 2 == 6, "2 is not valid"
    ...
    def test_chained_arithmetic(self):
        ...
        assert x + 5 + 3 == x + 8, "3 is not valid"
        assert x * 2 * 3 == x * 6, "3 is not valid"

        # Mixed operations
        result = 10 + 5 * 2 - 3 / 3
        assert result == 10 + 10 - 1, "Result must not be empty"
        assert result == 19, "Result must not be empty"

    def test_integer_vs_float_division(self):
        """Integer division vs float division"""
        assert 10 // 3 == 3, "3 is not valid"
        assert 10 / 3 != 3, "3 is not valid"
        assert abs(10 / 3 - 3.333) < 0.01, "Condition must be true"
