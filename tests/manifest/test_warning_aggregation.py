...
def setup():
    ...
    (ART / "content_filter_report.json").write_text(json.dumps({
        "mode":"pii","warnings":["invalid_regex:1"],"invalid_patterns":["("]
    }), encoding="utf-8")
    # Pointer with warnings
    BUNDLES.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d_%H%M%S")
    (BUNDLES / f"bundle_{stamp}.pointer.json").write_text(json.dumps({
        "warnings":["pointer_style_degraded:embedded"]
    }), encoding="utf-8")
