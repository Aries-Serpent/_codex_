...
class TestPlanset009Integration:
    ...
    def test_adapt_decision_for_ensemble(self, engine, adapter):
        ...
        assert ensemble_input.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
        assert ensemble_input.confidence_score == decision.confidence, "confidence_score is not valid"
        assert ensemble_input.confidence_level == decision.confidence_level.value, "Value must be initialized"
        assert ensemble_input.decision_option == decision.option, "decision_option is not valid"
        assert ensemble_input.strategy_used == decision.strategy.value, "Value must be initialized"
        assert ensemble_input.candidate_count == len(decision.candidates), "Candidate_count must not be empty"
        assert ensemble_input.domain_validation_passed == decision.domain_validation, "domain_validation_passed is not valid"
        assert ensemble_input.latency_ms == decision.latency_ms, "latency_ms is not valid"
        assert ensemble_input.category == "performance", "category is not valid"
    ...
    def test_ensemble_feature_vector_structure(self, engine, adapter):
        ...
        assert "confidence" in fv, "Condition must be true"
        assert "confidence_normalized" in fv, "Condition must be true"
        assert "latency_factor" in fv, "Condition must be true"
        assert "ensemble_bonus" in fv, "Condition must be true"
        assert "domain_validation_factor" in fv, "Condition must be true"
    ...
    def test_ensemble_input_json_serialization(self, engine, adapter):
        ...
        assert parsed["reasoning_decision_id"] == decision.id, "Condition must be true"
        assert parsed["confidence_score"] == decision.confidence, "Condition must be true"
        assert "feature_vector" in parsed, "Condition must be true"
...
class TestPlanset010Integration:
    ...
    def test_adapt_decision_for_multi_tenant(self, engine, adapter):
        ...
        assert tenant_input.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
        assert tenant_input.confidence_score == decision.confidence, "confidence_score is not valid"
        assert tenant_input.tenant_id == "tenant-123", "tenant_id is not valid"
    ...
    def test_isolation_decision_logic(self, engine, adapter):
        ...
        if not tenant_input.safe_for_production:
            assert tenant_input.isolation_required is True, "isolation_required is not valid"
        ...
        if tenant_input.domain_validation_passed and tenant_input.decision_latency_ms < 100 and tenant_input.confidence_score >= 0.80:
            assert tenant_input.safe_for_production is True, "safe_for_production is not valid"
    ...
    def test_confidence_threshold_evaluation(self, engine, adapter):
        ...
        if decision.confidence >= 0.75:
            assert threshold_met is True, "threshold_met is not valid"
        else:
            assert threshold_met is False, "threshold_met is not valid"
    ...
    def test_tenant_input_json_serialization(self, engine, adapter):
        ...
        parsed = json.loads(json_str)
        assert parsed["reasoning_decision_id"] == decision.id, "Condition must be true"
        assert parsed["tenant_id"] == "test-tenant", "Condition must be true"
...
class TestPlanset011Integration:
    ...
    def test_adapt_decision_for_root_cause(self, engine, adapter):
        ...
        assert rca_input.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
        assert rca_input.decision_option == decision.option, "decision_option is not valid"
        assert rca_input.confidence_score == decision.confidence, "confidence_score is not valid"
        assert rca_input.reasoning_text == decision.reasoning, "reasoning_text is not valid"
        assert rca_input.strategy_used == decision.strategy.value, "Value must be initialized"
    ...
    def test_candidate_options_extraction(self, engine, adapter):
        ...
        assert len(rca_input.candidate_options) == len(decision.candidates), "Collection must not be empty"

        # Each should match a candidate
        for i, option in enumerate(rca_input.candidate_options):
            assert option == decision.candidates[i].option, "option is not valid"
    ...
    def test_validation_rules_aggregation(self, engine, adapter):
        ...
        assert actual_rules.issubset(expected_rules) or actual_rules == expected_rules, "actual_rules is not valid"
    ...
    def test_anomaly_indicators_tracking(self, engine, adapter):
        ...
        assert rca_input.anomaly_indicators == anomaly_indicators, "anomaly_indicators is not valid"
    ...
    def test_historical_decisions_tracking(self, engine, adapter):
        ...
        assert len(rca_input.historical_decisions) == len(decisions) - 1, "Decisions must not be empty"

        # Most recent should be last decision (excluding current)
        # The historical decisions should reference the decisions we added
        assert rca_input.historical_decisions[-1] == decisions[-2].id, "Condition must be true"
    ...
    def test_rca_input_json_serialization(self, engine, adapter):
        ...
        parsed = json.loads(json_str)
        assert parsed["reasoning_decision_id"] == decision.id, "Condition must be true"
        assert "anomaly_indicators" in parsed, "Condition must be true"
        assert "historical_decisions" in parsed, "Condition must be true"
...
class TestCrossPlansetsConsistency:
    ...
    def test_all_adapters_same_decision_id(self, engine, adapter):
        ...
        assert p009.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
        assert p010.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
        assert p011.reasoning_decision_id == decision.id, "reasoning_decision_id is not valid"
    ...
    def test_all_adapters_consistent_confidence(self, engine, adapter):
        ...
        assert p009.confidence_score == decision.confidence, "confidence_score is not valid"
        assert p010.confidence_score == decision.confidence, "confidence_score is not valid"
        assert p011.confidence_score == decision.confidence, "confidence_score is not valid"
    ...
    def test_adapter_history_isolation(self, engine):
        ...
        assert len(adapter2.decision_history) == 0, "Collection must not be empty"
        assert len(adapter1.decision_history) == 5, "Collection must not be empty"
    ...
    def test_all_adapters_json_compatible(self, engine, adapter):
        ...
        assert "reasoning_decision_id" in json009, "Condition must be true"
        assert "reasoning_decision_id" in json010, "Condition must be true"
        assert "reasoning_decision_id" in json011, "Condition must be true"

        # All should deserialize back
        assert json009["reasoning_decision_id"] == decision.id, "Condition must be true"
        assert json010["reasoning_decision_id"] == decision.id, "Condition must be true"
        assert json011["reasoning_decision_id"] == decision.id, "Condition must be true"
