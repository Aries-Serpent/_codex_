...
class TestDenyPatterns:
    def test_sudo_always_denied(self) -> None:
        ...
        assert d.verdict == PolicyVerdict.DENY, "verdict is not valid"
...
class TestWorkingDirectoryConstraint:
    ...
    def test_disallowed_cwd_denied(self) -> None:
        ...
        assert d.verdict == PolicyVerdict.DENY, "verdict is not valid"
        assert "cwd_violation" in d.risk_flags, "Condition must be true"
...
class TestRiskAudit:
    def test_rm_rf_subdir_is_audit(self) -> None:
        ...
        assert d.verdict == PolicyVerdict.AUDIT, "verdict is not valid"
        assert "recursive_delete" in d.risk_flags, "Condition must be true"
    ...
    def test_output_redirect_is_denied(self) -> None:
        ...
        assert d.verdict == PolicyVerdict.DENY, "verdict is not valid"
        assert "shell metacharacter" in d.reason.lower(), "Condition must be true"

    def test_force_flag_is_audit(self) -> None:
        p = _policy()
        d = p.gate("git push --force")
        assert d.verdict == PolicyVerdict.AUDIT, "verdict is not valid"
        assert "force_flag" in d.risk_flags, "Condition must be true"
# ---------------------------------------------------------------------------
# Token redaction
# ---------------------------------------------------------------------------
...
class TestRedaction:
    def test_github_pat_redacted(self) -> None:
        ...
        safe = p.redact(raw)
        assert "ghp_" not in safe, "Condition must be true"
        assert "[REDACTED]" in safe, "Condition must be true"

    def test_codex_master_key_redacted(self) -> None:
        p = _policy()
        raw = "curl -H CODEX_MASTER_KEY=supersecret123 https://api.example.com"
        safe = p.redact(raw)
        assert "supersecret123" not in safe, "Condition must be true"

    def test_password_flag_redacted(self) -> None:
        p = _policy()
        raw = "mysql --password myS3cr3tP@ss"
        safe = p.redact(raw)
        assert "myS3cr3tP@ss" not in safe, "Condition must be true"

    def test_clean_command_unchanged(self) -> None:
        p = _policy()
        raw = "git status --short"
        assert p.redact(raw) == raw, "Condition must be true"
    ...
    def test_gate_safe_command_is_redacted(self) -> None:
        ...
        d = p.gate(raw_cmd)
        assert "[REDACTED]" in d.safe_command, "Condition must be true"
...
class TestPolicyProperties:
    def test_default_timeout_ceiling(self) -> None:
        p = _policy()
        assert p.timeout_ceiling_s > 0, "timeout_ceiling_s must be greater than zero"

    def test_custom_timeout_ceiling(self) -> None:
        p = _policy(timeout_ceiling_s=30.0)
        assert p.timeout_ceiling_s == 30.0, "timeout_ceiling_s is not valid"

    def test_default_max_retries(self) -> None:
        p = _policy()
        assert p.max_retries >= 1, "max_retries must be greater than zero"

    def test_custom_max_retries(self) -> None:
        p = _policy(max_retries=0)
        assert p.max_retries == 0, "max_retries is not valid"

    def test_denied_command_has_zero_retries(self) -> None:
        p = _policy()
        d = p.gate("sudo rm -rf /")
        assert d.max_retries == 0, "max_retries is not valid"

    def test_allowed_command_timeout_matches_policy(self) -> None:
        p = _policy(timeout_ceiling_s=60.0)
        d = p.gate("git status")
        assert d.timeout_s == 60.0, "timeout_s is not valid"
...
class TestGateDecision:
    def test_allowed_property_true_for_allow(self) -> None:
        p = _policy()
        d = p.gate("git status")
        assert d.allowed is True, "allowed is not valid"
    ...
    def test_allowed_property_false_for_deny(self) -> None:
        ...
        assert d.allowed is False, "allowed is not valid"

    def test_to_dict_contains_verdict(self) -> None:
        p = _policy()
        d = p.gate("git status")
        data = d.to_dict()
        assert "verdict" in data, "Data must not be empty"
        assert data["verdict"] == PolicyVerdict.ALLOW.value, "Data must not be empty"
...
class TestDefaultPolicySingleton:
    ...
    def test_get_default_policy_singleton(self) -> None:
        ...
        assert p1 is p2, "p1 is not valid"
...
class TestShellMetacharacterBlocking:
    ...
    def test_all_adversarial_vectors_denied(self, label: str, command: str) -> None:
        ...
        assert "shell metacharacter" in d.reason.lower(), "Condition must be true"
        assert "shell_metacharacter_detected" in d.risk_flags, "Condition must be true"
