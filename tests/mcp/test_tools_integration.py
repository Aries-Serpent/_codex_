...
def test_tool_registration_with_confirmation():
    ...
    registry.register_tool(
        "dangerous",
        lambda x: "executed",
        require_confirm=True  # Offline mode auto-confirms
    )
...
def test_tool_versioning():
    ...
    registry.register_tool(
        "old",
        lambda x: x,
        metadata={"version": "0.9", "deprecated": True}
    )
...
def test_tool_integration_end_to_end():
    ...
    registry.register_tool("upper", lambda p: {"result": p["text"].upper()}, metadata={"type": "transform"})
