...
class TestTarfileExtractionSecurity:
    ...
    def test_safe_extraction_valid_archive(self, tmp_path, cleanup_bootstrapper):
        ...
        assert bootstrapper.wheelhouse_dir.exists(), "Condition must be true"
        assert (bootstrapper.wheelhouse_dir / "test.whl").exists(), "Condition must be true"
    ...
    def test_validate_and_extract_safely_valid_members(self, tmp_path):
        ...
        assert (extract_dir / "file1.txt").exists(), "Condition must be true"
        assert (extract_dir / "file2.txt").exists(), "Condition must be true"
        assert (extract_dir / "file1.txt").read_text() == "content1", "Content must not be empty"
    ...
    def test_validate_and_extract_safely_rejects_traversal(self, tmp_path):
        ...
        with tarfile.open(archive_path, "w:gz") as tar:
            pass  # removed redundant `import io` (top-level import used)
    ...
    def test_extraction_preserves_legitimate_subdirectories(self, tmp_path, cleanup_bootstrapper):
        ...
        assert (bootstrapper.wheelhouse_dir / "subdir" / "package.whl").exists(), "Condition must be true"
    ...
    def test_extraction_handles_special_characters(self, tmp_path, cleanup_bootstrapper):
        ...
        assert (bootstrapper.wheelhouse_dir / "package-1.0-py3-none-any.whl").exists(), "Condition must be true"
...
class TestExtractWhelhouseIntegration:
    ...
    def test_full_bootstrap_workflow_valid_archive(self, tmp_path, cleanup_bootstrapper):
        ...
        assert success, "Extraction should succeed"
        assert len(list(bootstrapper.wheelhouse_dir.glob("*.whl"))) == 2, "Collection must not be empty"
