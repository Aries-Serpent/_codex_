...
class StressTestResult:
    ...
    def calculate_statistics(self):
        ...
        self.throughput_tasks_per_sec = (
            ...
            if self.execution_latencies_ms and sum(self.execution_latencies_ms) > 0 else 0
        )
        ...
        if self.agent_utilization:
            self.utilization_std_dev = statistics.stdev(self.agent_utilization.values()) if len(self.agent_utilization) > 1 else 0.0
        
        # Determine pass/fail
        self.test_passed = (
            self.routing_latency_p99 <= 500 and
            self.routing_accuracy >= 0.95 and
            self.throughput_tasks_per_sec >= 10
        )
...
class StressTestRunner:
    ...
    def test_concurrent_routing(self, num_tasks: int = 100, num_workers: int = 10) -> StressTestResult:
    ...
    def test_agent_dynamics(self, num_tasks: int = 100, churn_rate: float = 0.1) -> StressTestResult:
    ...
    def test_cache_effectiveness(self, num_tasks: int = 100, cache_hit_rate: float = 0.5) -> StressTestResult:
    ...
    def test_load_distribution(self, num_tasks: int = 100, num_agents: int = 10) -> StressTestResult:
        ...
        for agent_id, load in agent_load.items():
            result.agent_utilization[str(agent_id)] = (load / total_load) * 100 if total_load > 0 else 0
    ...
    def run_all_scenarios(self) -> Dict:
        ...
        for scenario_name, test_func in scenarios:
            ...
            print(f"  Latency (p50/p95/p99): {result.routing_latency_p50:.0f}ms / {result.routing_latency_p95:.0f}ms / {result.routing_latency_p99:.0f}ms")
