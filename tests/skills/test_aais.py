...
class TestAcronymDiscipline:
    def test_no_acronyms_returns_perfect_score(self, scorer):
        ...
        assert result.acronym_discipline == 1.0
    ...
    def test_defined_acronym_scores_higher(self, scorer):
        ...
        assert defined_score >= undefined_score
...
class TestStructure:
    def test_heading_adds_to_score(self, scorer):
        ...
        assert scorer.score(with_heading).structure > scorer.score(without_heading).structure

    def test_bullets_add_to_score(self, scorer):
        with_bullets = "Items:\n- first\n- second\n- third"
        without_bullets = "Items: first, second, third."
        assert scorer.score(with_bullets).structure > scorer.score(without_bullets).structure

    def test_multiple_headings_bonus(self, scorer):
        rich = "# H1\n\nText.\n\n## H2\n\nText.\n\n### H3\n\nText."
        single = "# H1\n\nText."
        assert scorer.score(rich).structure > scorer.score(single).structure

    def test_code_block_adds_to_score(self, scorer):
        with_code = "Example:\n```python\nprint('hello')\n```"
        without_code = "Example: print hello"
        assert scorer.score(with_code).structure > scorer.score(without_code).structure
...
class TestClarity:
    ...
    def test_hedge_words_penalised(self, scorer):
        ...
        assert scorer.score(direct).clarity >= scorer.score(hedged).clarity
