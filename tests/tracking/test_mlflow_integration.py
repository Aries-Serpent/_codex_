...
class TestMLflowWriter:
    ...
    @pytest.mark.skipif(
        not pytest.importorskip("mlflow", reason="MLflow not available"),
        reason="Requires MLflow"
    )
...
class TestTrackingConfiguration:
    ...
    def test_tracking_config_file_exists(self):
        ...
        config_path = Path(__file__).parent.parent.parent / "configs" / "base" / "tracking" / "default.yaml"
