...
class TestDecisionVisualization:
    ...
    def test_submit_decision_success(self, client):
        ...
        data = response.json()
        assert data["lane_name"] == "security", "Data must not be empty"
        assert data["candidate"] == "Fix SQL injection in auth.py", "Data must not be empty"
        assert data["confidence_score"] == 0.95, "Data must not be empty"
        assert "decision_id" in data, "Data must not be empty"
        assert data["submitted_at"] is not None, "Value must be initialized"
    ...
    def test_submit_decision_minimal(self, client):
        ...
        data = response.json()
        assert data["lane_name"] == "coverage", "Data must not be empty"
        assert data["k1_factor"] is None, "Data must not be empty"
        assert data["outcome"] is None, "Data must not be empty"
    ...
    def test_get_decision_success(self, client):
        ...
        data = response.json()
        assert data["decision_id"] == decision_id, "Data must not be empty"
        assert data["lane_name"] == "stability", "Data must not be empty"
    ...
    def test_get_recent_decisions(self, client):
        ...
        data = response.json()
        assert len(data) <= 10, "Data must not be empty"
    ...
    def test_get_decision_history(self, client):
        ...
        data = response.json()
        assert data["page"] == 1, "Data must not be empty"
        assert data["page_size"] == 5, "Data must not be empty"
        assert data["total"] == 9, "Data must not be empty"
        assert len(data["decisions"]) <= 5, "Collection must not be empty"
    ...
    def test_get_decision_history_filtered_by_lane(self, client):
        ...
        data = response.json()
        assert data["total"] == 3, "Data must not be empty"
        assert all(d["lane_name"] == "security" for d in data["decisions"]), "Data must not be empty"
...
class TestMemoryManagement:
    ...
    def test_retrieve_memory_success(self, client):
        ...
        assert data[0]["lane_name"] == "stability", "Data must not be empty"
        assert data[0]["pattern_type"] == "flaky_test", "Data must not be empty"
    ...
    def test_retrieve_memory_by_lane(self, client):
        ...
        data = response.json()
        assert all(item["lane_name"] == "security" for item in data), "Data must not be empty"
    ...
    def test_retrieve_memory_updates_usage_count(self, client):
        ...
        assert data[0]["usage_count"] >= 1, "Value must be greater than zero"
    ...
    def test_stm_push_success(self, client):
        ...
        data = response.json()
        assert data["success"] is True, "Data must not be empty"
        assert data["key"] == "test_stm_entry", "Data must not be empty"
        assert "timestamp" in data, "Data must not be empty"
    ...
    def test_stm_push_auto_key(self, client):
        ...
        assert data["key"].startswith("stm_"), "Data must not be empty"
    ...
    def test_get_memory_stats(self, client):
        ...
        data = response.json()
        assert "stm_count" in data, "Data must not be empty"
        assert "ltm_count" in data, "Data must not be empty"
        assert "lte_patterns_count" in data, "Data must not be empty"
        assert "capacity" in data, "Data must not be empty"
        assert "cache_hit_rate" in data, "Data must not be empty"
        assert "compression_rate" in data, "Data must not be empty"
        assert "timestamp" in data, "Data must not be empty"
...
class TestWorkflowMonitoring:
    ...
    def test_get_workflow_status(self, client):
        ...
        assert "total_workflows" in data, "Data must not be empty"
        assert "successful" in data, "Data must not be empty"
        assert "failed" in data, "Data must not be empty"
        assert "cancelled" in data, "Data must not be empty"
        assert "in_progress" in data, "Data must not be empty"
        assert "timestamp" in data, "Data must not be empty"
    ...
    def test_check_workflow_gate_success(self, client):
        ...
        data = response.json()
        assert data["pr_number"] == 42, "Data must not be empty"
        assert "compliant" in data, "Data must not be empty"
        assert "wec_items" in data, "Data must not be empty"
        assert "message" in data, "Data must not be empty"
        assert "checked_at" in data, "Data must not be empty"
    ...
    def test_get_rate_limit_status(self, client):
        ...
        data = response.json()
        assert "remaining" in data, "Data must not be empty"
        assert "limit" in data, "Data must not be empty"
        assert "reset_at" in data, "Data must not be empty"
        assert "percentage_used" in data, "Data must not be empty"
    ...
    def test_get_rate_limit_values(self, client):
        ...
        assert data["remaining"] <= data["limit"], "Data must not be empty"
        # Percentage should be between 0 and 100
        assert 0 <= data["percentage_used"] <= 100, "Data must not be empty"
        ...
        assert reset_time >= now, "reset_time must be greater than zero"
...
class TestIntegration:
    ...
    def test_decision_and_memory_workflow(self, client):
        ...
        assert decision_response.status_code == 201, "Response must not be empty"
        decision_id = decision_response.json()["decision_id"]

        # Retrieve the decision
        get_response = client.get(f"/api/decisions/{decision_id}")
        assert get_response.status_code == 200, "Response must not be empty"
        ...
        assert store_response.status_code == 201, "Response must not be empty"

        # Retrieve memory stats
        stats_response = client.get("/api/memory/stats")
        assert stats_response.status_code == 200, "Response must not be empty"
        stats = stats_response.json()
        assert stats["lte_patterns_count"] >= 1, "Value must be greater than zero"
