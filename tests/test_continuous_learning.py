...
def test_registry_persistence(tmp_path):
    ...
    pipeline = ContinuousLearningPipeline("demo", registry_path=registry_path, drift_threshold=0.1, min_samples_retrain=1)
...
def test_model_comparison_and_rollback(tmp_path):
    ...
    registry_path = tmp_path / "registry.json"
    pipeline = ContinuousLearningPipeline("demo", registry_path=registry_path, drift_threshold=0.0, min_samples_retrain=1)

    v1 = ModelVersion(version="v1", model_path=Path("/tmp/model1"), metrics={"accuracy": 0.8}, trained_at="now")
