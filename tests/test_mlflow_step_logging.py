...
class DummyMLFlowBackend:
    ...
    def log_metric(self, key: str, value: float, step: Optional[int] = None, timestamp: Optional[int] = None):
...
def test_log_metrics_type_conversion(patched_mlflow):
    ...
    # Verify values are passed as-is (float conversion happens in real MLflow)
...
def test_log_metrics_large_step(patched_mlflow):
    ...
    large_step = 999999
    MU.log_metrics({"metric": 1.0}, step=large_step, enabled=True)
    
    assert dummy.logged_metrics[0] == ("metric", 1.0, large_step)


def test_log_metrics_special_float_values(patched_mlflow):
    """Test logging with special float values."""
    dummy = patched_mlflow
    ...
    MU.log_metrics(special_metrics, step=1, enabled=True)
    
    assert len(dummy.logged_metrics) == 5
    # Verify all metrics were logged (handling of special values is MLflow's responsibility)
    logged_keys = [metric[0] for metric in dummy.logged_metrics]
    assert set(logged_keys) == set(special_metrics.keys())


@pytest.mark.parametrize("step_value", [0, 1, 10, 100, 1000])
def test_log_metrics_parametrized_steps(patched_mlflow, step_value):
    """Parametrized test for various step values."""
    dummy = patched_mlflow
    
    MU.log_metrics({"param_test": 1.0}, step=step_value, enabled=True)
    
    assert dummy.logged_metrics[0] == ("param_test", 1.0, step_value)


def test_log_metrics_error_handling(monkeypatch):
    """Test error handling when MLflow operations fail."""
    
    class FailingMLFlow:
        def log_metric(self, *args, **kwargs):
            raise RuntimeError("MLflow operation failed")
    
    failing_mlf = FailingMLFlow()
    monkeypatch.setattr(MU, "_mlf", failing_mlf)
    monkeypatch.setattr(MU, "_HAS_MLFLOW", True)
    monkeypatch.setattr(MU, "_mlflow_noop_or_raise", lambda enabled: failing_mlf if enabled else None)
    
    with pytest.raises(RuntimeError, match="Failed to log metrics to MLflow"):
        MU.log_metrics({"loss": 1.0}, step=1, enabled=True)


__all__ = [
    "DummyMLFlowBackend",
    "test_log_metrics_enforces_step",
    "test_log_metrics_multiple_values",
    "test_log_metrics_disabled_noop",
    "test_log_metrics_none_enabled_noop",
    "test_log_metrics_step_extraction",
    "test_log_metrics_empty_dict",
    "test_log_metrics_type_conversion",
    "test_log_metrics_step_zero",
    "test_log_metrics_negative_step",
    "test_log_metrics_large_step",
    "test_log_metrics_special_float_values",
    "test_log_metrics_parametrized_steps",
    "test_log_metrics_error_handling",
]