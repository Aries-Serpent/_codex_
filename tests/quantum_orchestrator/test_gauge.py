...
def test_translation_momentum_conservation(multi_task_state):
    ...
    result = trans.verify_momentum_conservation(
        multi_task_state, state_after, tolerance=1e-6
    )
...
def test_time_translation_energy_conservation(multi_task_state):
    ...
    result = time_trans.verify_energy_conservation(
        multi_task_state, state_after, tolerance=1e-6
    )
...
def test_noether_momentum_current(simple_task):
    ...
    assert np.allclose(
        momentum_curr,
        simple_task.spinor.total_probability * simple_task.velocity,
        rtol=1e-6
    )
...
def test_noether_continuity_equation(multi_task_state):
    ...
    result = noether.verify_continuity(
        multi_task_state, state_after, dt=0.01, tolerance=1e-3
    )
...
def test_gauge_checker_verify_all(multi_task_state):
    ...
    results = checker.verify_all(
        multi_task_state, state_after, dt=0.1, tolerance=1e-3
    )
