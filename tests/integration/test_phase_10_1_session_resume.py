...
class TestCheckpointManager:
    ...
    def test_checkpoint_manager_init(self, checkpoint_manager, temp_checkpoint_dir):
        ...
        assert checkpoint_manager.schema_version == 1
        assert checkpoint_manager.retention_count == 5
        assert (temp_checkpoint_dir / "v1").exists()
        assert (temp_checkpoint_dir / "metadata").exists()
    ...
    def test_create_checkpoint_minimal(self, checkpoint_manager):
        ...
        assert checkpoint_manager.verify_checkpoint_integrity(checkpoint_id)
    ...
    def test_create_checkpoint_with_session_state(self, checkpoint_manager):
        ...
        assert content["session_state"]["agent_state"]["agent_id"] == "test_agent"
    ...
    def test_list_checkpoints(self, checkpoint_manager):
        ...
        assert checkpoints[0]["checkpoint_id"] == cp_ids[-1]  # Most recent first
    ...
    def test_get_latest_checkpoint(self, checkpoint_manager):
        ...
        assert checkpoint_manager.get_latest_checkpoint() == cp1

        time.sleep(0.1)
        cp2 = checkpoint_manager.create_checkpoint(session_id="S002")
        assert checkpoint_manager.get_latest_checkpoint() == cp2
    ...
    def test_checkpoint_integrity_verification(self, checkpoint_manager):
        ...
        assert checkpoint_manager.verify_checkpoint_integrity(cp_id)
        ...
        assert not checkpoint_manager.verify_checkpoint_integrity(cp_id)

    def test_checkpoint_deletion(self, checkpoint_manager):
        """Test checkpoint deletion."""
        cp_id = checkpoint_manager.create_checkpoint(session_id="S001")
        assert checkpoint_manager.get_checkpoint_content(cp_id) is not None

        # Delete checkpoint
        assert checkpoint_manager.delete_checkpoint(cp_id)
    ...
    def test_retention_policy(self, checkpoint_manager):
        ...
        assert checkpoints[0]["checkpoint_id"] == cp_ids[-1]
        assert checkpoints[-1]["checkpoint_id"] == cp_ids[-5]
    ...
    def test_checkpoint_compression(self, checkpoint_manager):
        ...
        assert (
            metadata.compressed_size_bytes < metadata.uncompressed_size_bytes * 0.5
    ...
    def test_maybe_checkpoint_commit_trigger(self, checkpoint_manager):
        ...
        # Should not trigger
        assert checkpoint_manager.maybe_checkpoint(commit_count_delta=1) is None
...
class TestSessionSerializer:
    ...
    def test_serializer_init(self, serializer):
        ...
        assert serializer.SERIALIZER_VERSION == "1.0.0"
    ...
    def test_serialize_empty_session(self, serializer):
        ...
        assert "agent_state" in state_dict
    ...
    def test_serialize_complete_session(self, serializer):
        ...
        assert state_dict["agent_state"]["agent_id"] == "test_agent"
        assert len(state_dict["decision_history"]) == 1
        assert state_dict["memory_snapshot"]["total_patterns"] == 10
        assert len(state_dict["execution_progress"]["completed_tasks"]) == 2
    ...
    def test_json_roundtrip(self, serializer):
        ...
        assert deserialized["agent_state"]["agent_id"] == "agent_1"
        assert len(deserialized["decision_history"]) == 1
        assert deserialized["decision_history"][0]["confidence"] == 0.92
    ...
    def test_binary_roundtrip(self, serializer):
        ...
        assert deserialized["agent_state"]["agent_id"] == "agent_2"
        assert deserialized["memory_snapshot"]["total_patterns"] == 50
...
class TestSessionResume:
    ...
    def test_validate_checkpoint(self, checkpoint_manager, session_resume):
        ...
        cp_id = checkpoint_manager.create_checkpoint(session_id="S001")
        assert session_resume.validate_checkpoint(cp_id)
    ...
    def test_load_checkpoint(self, checkpoint_manager, session_resume):
        ...
        assert content is not None
        assert content["session_state"]["test_key"] == "test_value"
    ...
    def test_resume_session(self, checkpoint_manager, session_resume):
        ...
        assert result.state["agent_state"]["agent_id"] == "agent_1"
    ...
    def test_get_progress_snapshot(self, checkpoint_manager, session_resume):
        ...
        assert progress["current_task"] == "t3"
...
class TestEndToEndIntegration:
    ...
    def test_complete_checkpoint_resume_cycle(self, checkpoint_manager, session_resume, serializer):
        ...
        result = session_resume.resume_session(cp_id)
        assert result.success
        assert result.agent_id == "semantic_search"
        assert result.state["memory_snapshot"]["total_patterns"] == 47
        assert len(result.state["execution_progress"]["completed_tasks"]) == 2
    ...
    def test_multiple_checkpoints_with_progression(self, checkpoint_manager, session_resume):
        ...
        assert len(progress["completed_tasks"]) == 2
        assert len(progress["pending_tasks"]) == 2

        # Resume from latest (cp3) and verify state
        result_latest = session_resume.resume_latest_session()
        assert result_latest.success
        progress_latest = result_latest.state["execution_progress"]
        assert len(progress_latest["completed_tasks"]) == 3
        assert len(progress_latest["pending_tasks"]) == 1
    ...
    def test_large_session_state_persistence(self, checkpoint_manager, session_resume):
        ...
        result = session_resume.resume_session(cp_id)
        assert result.success
        assert len(result.state["decision_history"]) == 100
        assert result.state["memory_snapshot"]["total_patterns"] == 200
