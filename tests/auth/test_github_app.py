...
class TestGitHubAppConfig:
    ...
    def test_custom_api_base_url(self, rsa_private_key_pem):
        ...
        assert cfg.api_base_url == "https://github.example.com/api/v3"
...
class TestGenerateJWT:
    ...
    def test_jwt_payload_contains_iss(self, github_app):
        ...
        assert payload["iss"] == str(github_app._config.app_id)
...
class TestInstallationToken:
    ...
    def test_fetch_installation_token(self, github_app):
        ...
        assert token.installation_id == 999
...
class TestInstallationTokenExpiry:
    ...
    def test_expires_within_buffer(self):
        ...
        assert t.is_expired(buffer_seconds=60) is True
...
class TestBuildAppManifest:
    ...
    def test_required_fields_present(self):
        ...
        assert m["name"] == "codex-bot"
        assert m["url"] == "https://example.com"
        assert m["hook_attributes"]["url"] == "https://example.com/webhook"
        assert "default_events" in m
        assert "default_permissions" in m

    def test_default_events_non_empty(self):
        m = build_app_manifest("x", "https://x.com", "https://x.com/wh")
        assert len(m["default_events"]) > 0
...
class TestUtilities:
    ...
    def test_parse_iso8601_empty_fallback(self):
        ts = _parse_iso8601("")
        assert ts > time.time()  # fallback is future timestamp
