...
class TestPathConstants:
    ...
    def test_session_logs_db_value(self):
        ...
        assert SESSION_LOGS_DB == Path(".codex/session_logs.db")
    ...
    def test_analysis_db_value(self):
        ...
        assert ANALYSIS_DB == Path(".codex/analysis.db")
    ...
    def test_metrics_db_value(self):
        ...
        assert METRICS_DB == Path(".codex/metrics.db")
    ...
    def test_cache_dir_value(self):
        ...
        assert CACHE_DIR == Path(".codex/cache")
    ...
    def test_reports_dir_value(self):
        ...
        assert REPORTS_DIR == Path(".codex/reports")
    ...
    def test_config_dir_value(self):
        ...
        assert CONFIG_DIR == Path(".codex/config")
    ...
    def test_parsed_trees_cache_value(self):
        ...
        assert PARSED_TREES_CACHE == Path(".codex/cache/parsed_trees")
    ...
    def test_similarity_cache_value(self):
        ...
        assert SIMILARITY_CACHE == Path(".codex/cache/similarity")
...
class TestEnsureCodexStructure:
    ...
    def test_ensure_codex_structure_creates_cache_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/cache").exists()
    ...
    def test_ensure_codex_structure_creates_parsed_trees_cache(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/cache/parsed_trees").exists()
    ...
    def test_ensure_codex_structure_creates_similarity_cache(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/cache/similarity").exists()
    ...
    def test_ensure_codex_structure_creates_reports_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/reports").exists()
    ...
    def test_ensure_codex_structure_creates_reports_archive(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/reports/archive").exists()
    ...
    def test_ensure_codex_structure_creates_config_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/config").exists()
    ...
    def test_ensure_codex_structure_creates_readme(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/README.md").exists()
    ...
    def test_ensure_codex_structure_readme_content(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "Codex Local Data Directory" in readme_content
            assert "session_logs.db" in readme_content
    ...
    def test_ensure_codex_structure_creates_gitignore(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.chdir(tmpdir)
            ensure_codex_structure()
            assert Path(".codex/.gitignore").exists()
    ...
    def test_get_db_path_env_var_priority(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert str(path) == "/custom/path/db.db"
            finally:
                del os.environ["TEST_DB_PATH"]
...
class TestGetCachePath:
    ...
    def test_get_cache_path_parsed_trees(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "parsed_trees" in str(path)
            assert ".codex/cache" in str(path)
    ...
    def test_get_cache_path_custom_type(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "custom_cache" in str(path)
...
class TestGetReportPath:
    ...
    def test_get_report_path_default(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert ".codex/reports" in str(path)
            assert "report.html" in str(path)
    ...
    def test_get_report_path_archived(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert ".codex/reports/archive" in str(path)
            assert "report.html" in str(path)
    ...
    def test_get_report_path_not_archived(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert ".codex/reports" in str(path)
            assert "archive" not in str(path) or ".codex/reports/archive" not in str(path)
...
class TestConvenienceFunctions:
    ...
    def test_get_session_logs_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert isinstance(path, Path)
            assert "session_logs.db" in str(path)
    ...
    def test_get_analysis_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert isinstance(path, Path)
            assert "analysis.db" in str(path)
...
class TestEnvironmentVariableHandling:
    ...
    def test_codex_log_db_path_env(self):
        ...
        try:
            path = get_session_logs_db()
            assert str(path) == "/tmp/custom_logs.db"
        finally:
    ...
    def test_codex_analysis_db_path_env(self):
        ...
        try:
            path = get_analysis_db()
            assert str(path) == "/tmp/custom_analysis.db"
        finally:
    ...
    def test_codex_metrics_db_path_env(self):
        ...
        try:
            path = get_metrics_db()
            assert str(path) == "/tmp/custom_metrics.db"
        finally:
...
class TestPathsIntegration:
    ...
    def test_all_paths_consistent(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert ".codex" in str(metrics_path)
