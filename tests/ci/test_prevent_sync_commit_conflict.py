"""
Unit tests for scripts/ci/prevent_sync_commit_conflict.py
...
Representative diffs / staged-file lists are fed in directly so the tests
are fully offline and require no git repo state.
"""
...
"""
    ...
    # A diff with only development (S-session) work near [Unreleased]
    DEV_ONLY_NEAR_UNRELEASED = """
diff --git a/CHANGELOG.md b/CHANGELOG.md
--- a/CHANGELOG.md
+++ b/CHANGELOG.md
@@ -1,2 +1,5 @@
...
"""
    ...
    # A diff with only auto-generated content (no dev markers)
    AUTO_ONLY_DIFF = """
diff --git a/CHANGELOG.md b/CHANGELOG.md
--- a/CHANGELOG.md
+++ b/CHANGELOG.md
...
"""
    ...
    # A clean diff with no CHANGELOG-relevant additions
    CLEAN_DIFF = """
diff --git a/CHANGELOG.md b/CHANGELOG.md
--- a/CHANGELOG.md
+++ b/CHANGELOG.md
...
"""
    ...
    def test_mixed_auto_and_dev_raises_error(self):
        ...
        assert (
            "auto-generated" in errors[0].reason.lower() or "auto_gen" in errors[0].reason.lower()
        )
...
+- iterative-self-healing-ci Phase 5 implementation
"""
        # The header `## [Unreleased]` is not in the diff context, so the
...
class TestCheckAutoGeneratedFiles:
    ...
    def test_manifest_plus_dev_files_emits_warning(self):
        ...
        assert any("CODEX_MANIFEST.json" in w.file for w in warnings)
    ...
    def test_empty_staged_list_no_warning(self):
        assert check_auto_generated_files([]) == []
