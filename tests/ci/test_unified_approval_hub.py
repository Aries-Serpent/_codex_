...
class TestIntegrationPoints:
    ...
    def _dispatch_hub(self, approval_source, target_pr=None, approval_ttl_hours=None, approval_reason=''):
...
class TestIntegrationScenarios:
    ...
    def test_scenario_12_audit_trail_completeness(self):
        ...
        approvals = [
            {'approval_id': f'id-{i}', 'action': 'approved'}
            for i in range(12)
        ]
...
class TestParametrizedApprovalDecisions:
    ...
    @pytest.mark.parametrize('rule_name,input_data,expected', [
        ('persistent_label', {'labels': ['wec:auto-approve']}, 'APPROVE'),
        ('persistent_label_absent', {'labels': []}, 'SKIP'),
        ('single_session_unexpired', {'labels': ['wec:auto-approve-once'], 'age_hours': 0.5, 'ttl_hours': 1}, 'APPROVE'),
        ('single_session_expired', {'labels': ['wec:auto-approve-once'], 'age_hours': 2, 'ttl_hours': 1}, 'DENY'),
