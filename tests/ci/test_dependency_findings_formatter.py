...
class TestLoadFindings:
    ...
    def test_load_valid_findings(self, findings_json_file):
        ...
        assert len(findings) == 4, "Findings must not be empty"
        assert findings[0]["tool"] == "pip-audit", "Condition must be true"
...
class TestExtractPackageName:
    ...
    def test_extract_from_simple_description(self):
        ...
        assert extract_package_name(desc) == "django", "Condition must be true"
    ...
    def test_extract_from_update_phrase(self):
        ...
        assert extract_package_name(desc) == "requests", "Condition must be true"

    def test_extract_none_when_not_found(self):
        """Test None return when package not found."""
        desc = "Some random security issue"
        assert extract_package_name(desc) is None, "Condition must be true"
...
class TestFilterDependencyFindings:
    ...
    def test_filter_includes_pip_audit(self, sample_findings):
        ...
        assert any(f["tool"] == "pip-audit" for f in filtered), "Condition must be true"

    def test_filter_includes_safety(self, sample_findings):
        """Test safety findings included."""
        filtered = filter_dependency_findings(sample_findings)
        assert any(f["tool"] == "safety" for f in filtered), "Condition must be true"

    def test_filter_excludes_codeql(self, sample_findings):
        """Test CodeQL findings excluded."""
        filtered = filter_dependency_findings(sample_findings)
        assert not any(f["tool"] == "CodeQL" for f in filtered), "Condition must be true"
...
class TestGroupByPackage:
    ...
    def test_group_by_package_name(self, sample_findings):
        ...
        assert "django" in grouped, "Condition must be true"
        assert "requests" in grouped, "Condition must be true"
    ...
    def test_group_preserves_findings(self, sample_findings):
        ...
        assert total_grouped == len(filtered), "Filtered must not be empty"
    ...
    def test_group_multiple_findings_per_package(self):
        ...
        assert len(grouped["requests"]) == 2, "Collection must not be empty"
...
class TestCalculateUpgradePath:
    ...
    def test_upgrade_path_extracted(self):
        ...
        assert path["target_version"] == "3.2.15", "Condition must be true"
    ...
    def test_low_risk_minor_upgrade(self):
        ...
        assert path["risk_level"] == "LOW", "Condition must be true"
        assert not path["is_major_upgrade"], "Condition must be true"
    ...
    def test_high_risk_major_upgrade(self):
        ...
        path = calculate_upgrade_path("package", "1.2.3", findings)
        assert path["risk_level"] == "HIGH", "Condition must be true"
        assert path["is_major_upgrade"], "Condition must be true"
    ...
    def test_breaking_changes_detected(self):
        ...
        assert path["breaking_changes"] is True, "Condition must be true"
...
class TestExtractVersionFromFinding:
    ...
    def test_extract_explicit_version(self):
        ...
        assert extract_version_from_finding(finding) == "1.2.3", "Condition must be true"
    ...
    def test_extract_from_description(self):
        ...
        assert "1.2.3" in version, "Condition must be true"
    ...
    def test_returns_unknown_when_missing(self):
        ...
        assert extract_version_from_finding(finding) == "unknown", "Condition must be true"
...
class TestExtractCveId:
    ...
    def test_extract_explicit_cve(self):
        ...
        assert extract_cve_id(finding) == "CVE-2023-1234", "Condition must be true"

    def test_extract_cve_from_description(self):
        """Test extraction from description."""
        finding = {"description": "Vulnerability CVE-2023-5678"}
        assert extract_cve_id(finding) == "CVE-2023-5678", "Condition must be true"

    def test_returns_empty_when_missing(self):
        """Test returns empty string when no CVE found."""
        finding = {"description": "Some vulnerability"}
        assert extract_cve_id(finding) == "", "Condition must be true"
...
class TestFormatDependencyVulnerabilities:
    ...
    def test_format_returns_valid_structure(self, findings_json_file):
        ...
        assert "vulnerable_packages" in result, "Result must not be empty"
    ...
    def test_format_metadata_complete(self, findings_json_file):
        ...
        assert "total_vulnerabilities" in metadata, "Data must not be empty"
        assert "critical_count" in metadata, "Data must not be empty"
        assert "safe_upgrades" in metadata, "Data must not be empty"
        assert "risky_upgrades" in metadata, "Data must not be empty"
        assert "packages_affected" in metadata, "Data must not be empty"
        assert "generated_at" in metadata, "Data must not be empty"
    ...
    def test_format_package_has_required_fields(self, findings_json_file):
        ...
        if result["vulnerable_packages"]:
            ...
            assert "package" in pkg, "Condition must be true"
            assert "current_version" in pkg, "Condition must be true"
            assert "vulnerability" in pkg, "Condition must be true"
            assert "severity" in pkg, "Condition must be true"
            assert "safe_upgrade" in pkg, "Condition must be true"
    ...
    def test_format_counts_critical(self, findings_json_file):
        ...
        assert result["metadata"]["critical_count"] >= 1, "Value must be greater than zero"
    ...
    def test_format_empty_findings(self):
        ...
        try:
            ...
            assert result["metadata"]["total_vulnerabilities"] == 0, "Result must not be empty"
            assert len(result["vulnerable_packages"]) == 0, "Collection must not be empty"
        finally:
            Path(temp_path).unlink()
...
class TestGenerateMarkdownReport:
    ...
    def test_markdown_includes_summary_table(self, findings_json_file):
        ...
        assert "| Metric | Count |" in markdown, "Count must be greater than zero"
        assert "Total Vulnerabilities" in markdown, "Condition must be true"
...
class TestEdgeCases:
    ...
    def test_handle_special_characters_in_names(self):
        ...
        grouped = group_by_package(findings)
        assert len(grouped) > 0, "Grouped must not be empty"
