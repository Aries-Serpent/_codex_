...
""")
        ...
        assert len(issues) >= 2
        assert any("Missing assertion message" in issue for issue in issues)
...
""")
        ...
        assert assert_line.startswith('        ')  # 8 spaces
...
""")
        ...
        content = test_file.read_text()
        assert 'Response must not be empty' in content
        assert 'Result must not be empty' in content
        assert 'Content must not be empty' in content
