...
""")
        ...
        assert len(issues) >= 1
        assert any("Async test missing timeout" in issue for issue in issues)
...
""")
        ...
        assert '@pytest.mark.timeout(30)' in content
        assert len(issues) > 0
...
""")
        ...
        assert timeout_line == asyncio_line + 1
...
""")
        ...
        content = test_file.read_text()
        assert '@pytest.mark.timeout' not in content
        assert len(issues) == 0
...
""")
        ...
        assert asyncio_indent == timeout_indent
...
""")
        ...
        assert content.count('@pytest.mark.timeout') == 1
        assert '@pytest.mark.timeout(60)' in content
        assert len(issues) == 0
