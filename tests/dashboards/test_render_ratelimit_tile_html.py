...
def test_render_tile_html(tmp_path):
    tile = {
        ...
        "summary": {"core": {"min": 1, "avg": 1.5, "max": 2}, "search": {"min": 1, "avg": 1.5, "max": 2}, "graphql": {"min": 1, "avg": 1.5, "max": 2}},
    }
    ...
    code = subprocess.call([sys.executable, str(renderer), "--tile", str(tile_p), "--out", str(out_p)])
    assert code == 0
    assert out_p.exists()
