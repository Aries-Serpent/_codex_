...
class TestRedisHostEnvVar:
    ...
    def test_redis_host_env_var_override(self, monkeypatch):
        ...
        assert cache.host == "custom-redis.example.com", "host is not valid"
    ...
    def test_redis_host_fallback(self, monkeypatch):
        ...
        cache = RedisCache(host=None)
        assert cache.host == "localhost", "host is not valid"
    ...
    def test_redis_host_explicit_param_wins(self, monkeypatch):
        ...
        assert cache.host == "explicit-redis", "host is not valid"
...
class TestOllamaHostEnvVar:
    ...
    def test_ollama_host_env_var_override(self, monkeypatch):
        ...
        assert provider.host == "http://custom-ollama.example.com", "host is not valid"
...
class TestMasterAddrPortEnvVars:
    ...
    def test_master_addr_env_var_override(self, monkeypatch):
        ...
        assert config.master_addr == "master-node.example.com", "master_addr is not valid"
...
class TestInferenceServiceEnvVars:
    ...
    def test_server_config_host_env_var(self, monkeypatch):
        ...
        config = ServerConfig()
        assert config.host == "0.0.0.0", "host is not valid"
    ...
    def test_server_config_port_env_var(self, monkeypatch):
        ...
        config = ServerConfig()
        assert config.port == 8001, "port is not valid"
    ...
    def test_server_config_host_fallback(self, monkeypatch):
        ...
        config = ServerConfig()
        assert config.host == "127.0.0.1", "host is not valid"
    ...
    def test_server_config_port_fallback(self, monkeypatch):
        ...
        config = ServerConfig()
        assert config.port == 8000, "port is not valid"
...
class TestTrustedHostsEnvVar:
    ...
    def test_trusted_hosts_fallback(self, monkeypatch):
        ...
        assert "localhost" in DEFAULT_TRUSTED_HOSTS or "127.0.0.1" in DEFAULT_TRUSTED_HOSTS, "Condition must be true"
...
class TestLocalLoopbackFeatureGate:
    ...
    def test_loopback_enabled_network_policy(self, monkeypatch):
        ...
        assert "localhost" in _DEFAULT_LOCALHOSTS, "Condition must be true"
        assert "127.0.0.1" in _DEFAULT_LOCALHOSTS, "Condition must be true"
        assert "::1" in _DEFAULT_LOCALHOSTS, "Condition must be true"
    ...
    def test_loopback_disabled_network_policy(self, monkeypatch):
        ...
        assert len(_DEFAULT_LOCALHOSTS) == 0, "_default_localhosts must not be empty"
    ...
    def test_loopback_default_enabled(self, monkeypatch):
        ...
        assert len(_DEFAULT_LOCALHOSTS) > 0, "_default_localhosts must not be empty"
        assert "localhost" in _DEFAULT_LOCALHOSTS or "127.0.0.1" in _DEFAULT_LOCALHOSTS, "Condition must be true"
    ...
    def test_loopback_mlflow_guard(self, monkeypatch):
        ...
        assert reason == "non_local_host", "reason is not valid"
    ...
    def test_loopback_guards(self, monkeypatch):
        ...
        assert parsed.netloc == "", "netloc is not valid"
...
class TestEnvVarIntegration:
    ...
    def test_all_env_vars_set(self, monkeypatch):
        ...
        assert os.environ.get("CODEX_REDIS_HOST") == "test-redis", "Condition must be true"
        assert os.environ.get("CODEX_OLLAMA_HOST") == "http://test-ollama", "Condition must be true"
        assert os.environ.get("CODEX_MASTER_ADDR") == "test-master", "Condition must be true"
        assert os.environ.get("CODEX_MASTER_PORT") == "29501", "Condition must be true"
        assert os.environ.get("CODEX_INFERENCE_SERVICE_HOST") == "test-inference", "Condition must be true"
        assert os.environ.get("CODEX_INFERENCE_SERVICE_PORT") == "8001", "Condition must be true"
        assert os.environ.get("CODEX_TRUSTED_HOSTS") == "test.local", "Condition must be true"
        assert os.environ.get("CODEX_LOCAL_LOOPBACK") == "false", "Condition must be true"
    ...
    def test_all_env_vars_unset_defaults(self, monkeypatch):
        ...
        for var in [
            ...
            assert os.environ.get(var) is None, "Condition must be true"
