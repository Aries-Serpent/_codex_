...
class TestTypeCoercionEdgeCases:
    ...
    def test_string_to_bool_truthiness(self):
        ...
        assert not bool(""), "Empty string must be falsy"
        assert bool("0"), "String '0' must be truthy (non-empty string)"
        assert bool("False"), "String 'False' must be truthy"
        assert bool("True"), "String 'True' must be truthy"
...
class TestDatetimeBoundaryConditions:
    ...
    def test_datetime_comparison_boundaries(self):
        ...
        assert (dt_late - dt_early) == timedelta(seconds=1), (
            "Boundary times must differ by exactly 1 second"
        )
...
class TestNullAndUndefinedHandling:
    ...
    def test_none_identity_checks(self):
        ...
        assert value is not False, "None must remain distinct from False"
    ...
    def test_none_vs_false_distinction(self):
        ...
        assert False is not None, "False must NOT be None"
