...
class TestPreservedContexts:
    ...
    def test_version_date_preserved(self):
        ...
        assert "2026-01-05" in sanitized
    ...
    def test_session_date_preserved(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "2026-01-06" in sanitized
        assert len(replacements) == 0
    ...
    def test_timestamp_preserved(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "2026-01-06T12:34:56Z" in sanitized
        assert len(replacements) == 0
    ...
    def test_report_generated_date_preserved(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "2026-01-04" in sanitized
        assert len(replacements) == 0
...
class TestPlanningTerminologyReplacement:
    ...
    def test_quarter_reference_replaced(self):
        ...
        assert "Current Cycle" in sanitized
        assert len(replacements) > 0
    ...
    def test_phase_with_quarter_replaced(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "Q2 2026" not in sanitized
        assert "Current Cycle" in sanitized
        assert len(replacements) > 0
    ...
    def test_month_name_in_planning_replaced(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "January 2026" not in sanitized
        assert "Current Cycle" in sanitized
        assert len(replacements) > 0
    ...
    def test_grace_period_through_quarter(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "Q4 2026" not in sanitized
        assert "Current Cycle" in sanitized
        assert len(replacements) > 0
    ...
    def test_deadline_by_quarter(self):
        ...
        sanitized, replacements = sanitize_planning_dates(text)
        assert "Q3 2026" not in sanitized
        assert "Current Cycle" in sanitized
        assert len(replacements) > 0
...
"""
        ...
        assert "2026-01-05" in sanitized  # Last Updated
        assert "2026-01-03" in sanitized  # Version release
        assert "2026-01-04" in sanitized  # Session date
...
class TestRealWorldExamples:
    ...
    def test_ai_agent_utilities_registry(self):
        ...
        assert "2026-01-05" in sanitized
        assert "Current Cycle" not in sanitized
        assert len(replacements) == 0
...
"""
        ...
        assert "2026-01-06" in sanitized
        assert sanitized.count("2026-01-06") >= 3
        assert len(replacements) == 0
