...
def test_policy_skips_without_policy_file(tmp_path):
    code = subprocess.call([sys.executable, str(TOOL)], cwd=str(tmp_path))
    assert code == 0
...
def test_policy_schema_validation_runs(tmp_path, monkeypatch):
    ...
    (tmp_path / "configs/schemas/security_policy.schema.json").write_text(json.dumps({"type":"object"}), encoding="utf-8")
