...
class TestValidateCodexMasterKeyCommandInjection:
    ...
    def test_run_command_requires_list_not_string(self):
        ...
        assert "SECURITY" in str(exc_info.value), "Value must be initialized"
        assert "must be a list" in str(exc_info.value), "Value must be initialized"
    ...
    def test_run_command_accepts_list(self):
        ...
        assert "test" in result or result == "", "Result must not be empty"
...
class TestValidateTestEnvCodeInjection:
    ...
    def test_check_plugin_requires_whitelisted_imports(self):
        ...
        assert success is False or success is True, "success is not valid"
        assert "pytest" in msg, "Condition must be true"
        ...
        assert "BLOCKED" in msg or "not in allowed" in msg, "Condition must be true"
