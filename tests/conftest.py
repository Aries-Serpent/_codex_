...
def pooling_db_manager(enable_pooling, tmp_path):
    ...
    assert (
        DBManager._POOL_ENABLED is True
...
def pool_state_tracker(enable_pooling):
    ...
    def assert_pool_empty():
        current = get_current_size()
        assert current == 0, f"Pool should be empty (current size: {current})"

    def assert_pool_size(expected):
        current = get_current_size()
        assert current == expected, f"Pool size mismatch (expected: {expected}, current: {current})"
