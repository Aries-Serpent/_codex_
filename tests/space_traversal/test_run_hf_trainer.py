...
These tests verify that run_hf_trainer:
- Accepts offline/stubbed components
...
def test_run_hf_trainer_returns_without_error(monkeypatch, tmp_path, distributed):
...
def test_run_hf_trainer_creates_output_dir(monkeypatch, tmp_path):
...
def test_run_hf_trainer_accepts_empty_texts(monkeypatch, tmp_path):
