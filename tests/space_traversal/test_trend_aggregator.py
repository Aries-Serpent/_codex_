...
def test_aggregate_trends_basic(tmp_path: Path):
    ...
    assert result["summary_stats"]["capabilities_tracked"] == 3
    assert "cap1" in result["capability_trends"]
    assert "cap2" in result["capability_trends"]
    assert "cap3" in result["capability_trends"]
    
    # Check cap1 trend
    cap1_trend = result["capability_trends"]["cap1"]
    assert len(cap1_trend) == 2
    assert cap1_trend[0]["score"] == 0.7
    assert cap1_trend[1]["score"] == 0.75
...
def test_aggregate_trends_lookback(tmp_path: Path):
    ...
    run3 = {
        "timestamp": now,  # now
        "capabilities": [{"id": "cap1", "score": 0.7}]
    }
    ...
    assert result["run_count"] == 2  # Only run2 and run3
    cap1_trend = result["capability_trends"]["cap1"]
    assert len(cap1_trend) == 2
    assert cap1_trend[0]["score"] == 0.6  # run2
...
def test_aggregate_trends_manifest_paths(tmp_path: Path):
    ...
    assert "external_cap" in result["capability_trends"]
...
def test_generate_trend_report(tmp_path: Path):
    ...
    assert "Capability Audit Trend Report" in content
    assert "**Runs Analyzed**: 3" in content or "Runs Analyzed: 3" in content
...
def test_load_manifest_or_scored(tmp_path: Path):
    ...
    assert result["capabilities"][0]["id"] == "cap1"
...
def test_trending_detection(tmp_path: Path):
    ...
    assert "improving" in trending_up_ids
    
    # Check trending down
    trending_down_ids = [item["id"] for item in stats["trending_down"]]
    assert "declining" in trending_down_ids
    
    # Check stable
    assert "stable" in stats["stable"]
