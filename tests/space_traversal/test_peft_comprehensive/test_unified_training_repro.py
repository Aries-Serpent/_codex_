...
class _DummyStrategy:
    ...
    def run(self, config, callbacks, resume_from=None):  # type: ignore[override]
        return TrainingResult(status="ok", backend=self.backend_name, final_epoch=0, output_dir=config.output_dir, extra={})
...
def test_unified_training_captures_environment(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(unified_training, "resolve_strategy", lambda *_args, **_kwargs: _DummyStrategy())
