...
def test_validate_command_fails_when_required_artifact_is_missing():
    """Test validate fails when an explicitly required artifact is absent."""
    audit_runner = load_audit_runner()
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        (artifacts_dir / "capabilities_scored.json").write_text(
            json.dumps(
                {
                    "capabilities": [
                        {"id": "cap1", "score": 0.80, "components": {}, "meta": {}},
                    ]
                }
            )
        )
        ...
        cfg = {
            "output": {"artifacts_dir": str(artifacts_dir)},
            "options": {"fail_on_low_maturity": True},
            "scoring": {"thresholds": {"low": 0.70}},
        }
        args = type(
            "Args",
            (),
            {
                ...
                "baseline": None,
                "max_regression": 0.02,
                "required_artifacts": [str(artifacts_dir / "gaps.json")],
                "allow_low_maturity": False,
            },
        )()

        with pytest.raises(SystemExit) as exc_info:
            audit_runner.command_validate(cfg, args)

        assert exc_info.value.code == EXIT_MISSING_ARTIFACTS


def test_validate_command_fails_on_score_regression():
    """Test validate fails with exit code 3 when score regression exceeds the limit."""
    audit_runner = load_audit_runner()
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        (artifacts_dir / "capabilities_scored.json").write_text(
            json.dumps(
                {
                    "capabilities": [
                        {"id": "cap1", "score": 0.90, "components": {}, "meta": {}},
                        {"id": "cap2", "score": 0.74, "components": {}, "meta": {}},
                    ]
                }
            )
        )
        ...
        cfg = {
            "output": {"artifacts_dir": str(artifacts_dir)},
            "options": {"fail_on_low_maturity": False},
            "scoring": {"thresholds": {"low": 0.70}},
        }
        args = type(
            "Args",
