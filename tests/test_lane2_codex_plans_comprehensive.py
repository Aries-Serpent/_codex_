...
class TestListPlanDocuments:
    ...
    def test_list_plan_documents_custom_base_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert all(p.parent == tmp_path for p in result), "Result must not be empty"
    ...
    def test_list_plan_documents_only_direct_children(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result[0].name == "root_plan.md", "Result must not be empty"
    ...
    def test_list_plan_documents_returns_path_objects(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result[0].is_file(), "Result must not be empty"
    ...
    def test_list_plan_documents_path_absolute(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(result) == 1, "Result must not be empty"
            assert result[0].is_absolute(), "Result must not be empty"
    ...
    def test_list_plan_documents_large_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(result) == expected_count, "Result must not be empty"
            assert all(p.name.startswith("plan_") for p in result), "Result must not be empty"
    ...
    def test_list_plan_documents_special_characters_in_names(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(result) == len(special_names), "Result must not be empty"
            names = {p.name for p in result}
            assert names == set(special_names), "names is not valid"
    ...
    def test_list_plan_documents_case_sensitivity(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            names = [p.name for p in result]
            assert "plan.md" in names, "Condition must be true"
    ...
    def test_list_plan_documents_none_base_dir(self):
        ...
        assert all(p.is_file() for p in result), "Result must not be empty"
...
class TestListPlanDocumentsIntegration:
    ...
    def test_list_plan_documents_from_package(self):
        ...
        for plan in result:
            assert plan.parent == pkg_dir or plan.parent.parent == pkg_dir, "parent is not valid"
    ...
    def test_list_plan_documents_consistency_across_calls(self):
        ...
        for result in results[1:]:
            assert result == first_result, "Result must not be empty"
