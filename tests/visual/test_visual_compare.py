...
def test_visual_compare_identical(tmp_path):
    ...
    write_img(base, 200)
    write_img(cand, 200)
    code = subprocess.call([sys.executable, "tools/visual_compare.py", "--baseline", str(base), "--candidate", str(cand), "--threshold", "0.99"])
    assert code == 0
...
def test_visual_compare_different(tmp_path):
    ...
    code = subprocess.call([sys.executable, "tools/visual_compare.py", "--baseline", str(base), "--candidate", str(cand), "--threshold", "0.999"])
    assert code in (0, 1)
