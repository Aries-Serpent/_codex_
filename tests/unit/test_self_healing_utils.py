...
class TestFailureType:
    ...
    def test_oom_value(self):
        assert FailureType.OOM.value == "out_of_memory"

    def test_metric_regression_value(self):
        assert FailureType.METRIC_REGRESSION.value == "metric_regression"

    def test_checkpoint_corruption_value(self):
        assert FailureType.CHECKPOINT_CORRUPTION.value == "checkpoint_corruption"

    def test_config_drift_value(self):
        assert FailureType.CONFIG_DRIFT.value == "config_drift"
...
class TestOOMHandlerInit:
    def test_initial_batch_size_stored(self):
        h = OOMHandler(64)
        assert h.initial_batch_size == 64
        assert h.current_batch_size == 64

    def test_default_min_batch_size(self):
        h = OOMHandler(32)
        assert h.min_batch_size == 1

    def test_custom_min_batch_size(self):
        h = OOMHandler(32, min_batch_size=4)
        assert h.min_batch_size == 4
...
class TestOOMHandlerReset:
    def test_restores_batch_size(self):
        ...
        h.reduce_batch_size()
        h.reset()
        assert h.current_batch_size == 32
    ...
    def test_initial_batch_size_unchanged(self):
        ...
        h.reduce_batch_size()
        h.reset()
        assert h.initial_batch_size == 32
...
class TestSelfHealingContextInit:
    ...
    def test_default_oom_recovery(self):
        ctx = SelfHealingContext()
        assert ctx.enable_oom_recovery is True

    def test_default_checkpoint_rollback(self):
        ctx = SelfHealingContext()
        assert ctx.enable_checkpoint_rollback is True
    ...
    def test_custom_flags(self):
        ...
        assert ctx.enable_oom_recovery is False
        assert ctx.enable_checkpoint_rollback is False

    def test_oom_handler_created(self):
        ctx = SelfHealingContext(batch_size=16)
        assert isinstance(ctx.oom_handler, OOMHandler)
        assert ctx.oom_handler.initial_batch_size == 16
...
class TestSelfHealingClassifyFailure:
    ...
    def test_checkpoint_corruption_via_corrupt(self):
        ...
        assert ft == FailureType.CHECKPOINT_CORRUPTION
    ...
    def test_config_drift(self):
        ...
        assert ft == FailureType.CONFIG_DRIFT
