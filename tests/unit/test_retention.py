...
def test_epoch_dir_re_matches_four_digit():
    assert EPOCH_DIR_RE.match("epoch-0001")
    assert EPOCH_DIR_RE.match("epoch-9999")


def test_epoch_dir_re_matches_more_digits():
    assert EPOCH_DIR_RE.match("epoch-00100")
    assert EPOCH_DIR_RE.match("epoch-12345")


def test_epoch_dir_re_no_match_short():
    assert not EPOCH_DIR_RE.match("epoch-001")
    assert not EPOCH_DIR_RE.match("epoch-12")


def test_epoch_dir_re_no_match_non_epoch():
    assert not EPOCH_DIR_RE.match("best")
    assert not EPOCH_DIR_RE.match("latest")
    assert not EPOCH_DIR_RE.match("epoch_0001")
...
def test_prune_checkpoints_keep_last_deletes_dirs(tmp_path: Path):
    ...
    assert "epoch-0003" in remaining_names
    assert "epoch-0001" not in remaining_names
    assert "epoch-0002" not in remaining_names
...
def test_prune_checkpoints_keep_every(tmp_path: Path):
    ...
    assert 10 in result["kept"]
    assert 1 in result["pruned"] or 2 in result["pruned"]
...
def test_prune_checkpoints_protects_latest(tmp_path: Path):
    ...
    assert 2 in result["kept"]
    assert result["protected_latest"] == 2


def test_prune_checkpoints_protected_latest_none_when_no_latest_json(tmp_path: Path):
    _make_epoch_dirs(tmp_path, [1, 2])
    result = prune_checkpoints(tmp_path, keep_last=1)
    assert result["protected_latest"] is None
...
def test_prune_checkpoints_dry_run_does_not_delete(tmp_path: Path):
    ...
    for epoch in [1, 2, 3]:
        assert (tmp_path / f"epoch-{epoch:04d}").exists()
...
def test_prune_checkpoints_result_has_expected_keys(tmp_path: Path):
    ...
    assert "protected_latest" in result
...
def test_prune_checkpoints_kept_plus_pruned_equals_total(tmp_path: Path):
    ...
    assert len(result["kept"]) + len(result["pruned"]) == result["total"]
