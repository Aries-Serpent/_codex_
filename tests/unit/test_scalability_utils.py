...
class TestRateLimiter:
    ...
    def test_wait_for_token_timeout(self):
        ...
        assert elapsed < 1.0  # must not hang
...
class TestCircuitBreaker:
    ...
    def test_decorator_records_success(self):
        ...
        fn()
        assert cb._failures == 1  # decremented by record_success
...
class TestEndpoint:
    def test_default_values(self):
        ...
        assert ep.current_connections == 0
    ...
    def test_custom_values(self):
        ...
        assert ep.url == "http://host"
        assert ep.weight == 2.5
...
class TestLoadBalancer:
    ...
    def test_get_endpoint_round_robin_cycles(self):
        ...
        assert urls[:3] == [e.url for e in eps]
        assert urls[3:] == [e.url for e in eps]
    ...
    def test_get_endpoint_least_connections(self):
        ...
        assert lb.get_endpoint().url == eps[1].url
    ...
    def test_mark_healthy(self):
        ...
        assert lb.get_endpoint().url == eps[0].url
...
class TestMetricPoint:
    ...
    def test_default_timestamp_is_recent(self):
        ...
        after = time.time()
        assert before <= mp.timestamp <= after

    def test_custom_tags(self):
        mp = MetricPoint(name="req", value=1.0, tags={"service": "api"})
        assert mp.tags == {"service": "api"}
...
class TestPerformanceMonitor:
    ...
    def test_record_caps_at_1000_points(self):
        ...
        assert len(mon._metrics["metric"]) == 1000
    ...
    def test_timed_decorator_preserves_name(self):
        ...
        assert my_timed_fn.__name__ == "my_timed_fn"
