...
class TestShouldRetrain:
    ...
    def test_score_above_threshold_returns_true(self):
        ...
        assert pipeline.should_retrain({"score": 0.35}) is True

    # Test 2 — score below threshold
    def test_score_below_threshold_returns_false(self):
        pipeline = _make_pipeline(drift_threshold=0.2)
        assert pipeline.should_retrain({"score": 0.10}) is False
...
class TestTriggerRetrain:
    ...
    def test_config_snapshot_stored(self):
        ...
        assert job.trigger.config_snapshot == cfg
    ...
    def test_retraining_trigger_round_trip(self):
        ...
        assert d["drift_score"] == pytest.approx(0.32)
        assert "2024-06-01" in d["timestamp"]

        restored = RetrainingTrigger.from_dict(d)
        assert restored.reason == trigger.reason
        assert restored.drift_score == pytest.approx(trigger.drift_score)
        assert restored.config_snapshot == trigger.config_snapshot
...
class TestEvalGate:
    ...
    def test_accuracy_below_minimum_fails(self):
        ...
        assert any("min_accuracy" in r for r in result.reasons)
    ...
    def test_loss_above_maximum_fails(self):
        ...
        assert result.passed is False
        assert any("max_loss" in r for r in result.reasons)
    ...
    def test_insufficient_improvement_fails(self):
        ...
        assert result.passed is False
        assert any("min_improvement_pct" in r for r in result.reasons)
...
class TestPromote:
    ...
    def test_promote_blocked_when_gate_fails(self):
        ...
        assert "model_path" not in registry
    ...
    def test_registry_updated_on_success(self):
        ...
        assert registry["model_path"] == "/models/v3.pt"
        assert "promoted_at" in registry
    ...
    def test_promote_without_metrics_always_succeeds(self):
        ...
        assert result is True
        assert registry["model_path"] == "/models/v4.pt"
...
class TestPipelineEndToEnd:
    ...
    def test_end_to_end_with_dict_drift_result(self):
        ...
        assert pipeline.should_retrain(drift_result) is True
        ...
        assert registry["model_path"] == "/models/new.pt"
        assert pipeline.last_job is not None
        assert pipeline.last_job.status == "done"
    ...
    def test_end_to_end_with_object_drift_result(self):
        ...
        assert pipeline.should_retrain(fake_drift) is True
        job = pipeline.trigger_retrain()
        assert job.trigger.reason == "drift_threshold_exceeded"

        metrics = {"accuracy": 0.80}
        assert pipeline.eval_gate(metrics) is True
