    ...
    def test_csv_with_missing_values(self):
        ...
        try:
            with open(temp_path, 'r') as f:
                reader = csv.reader(f)
                rows = list(reader)
            assert rows[1][1] == ''  # Missing value preserved
        finally:
            os.remove(temp_path)
...
class TestDataTransforms:
    ...
    def test_categorical_encoding(self):
        ...
        assert len(encoding) == 3  # 3 unique categories
    ...
    def test_stratified_sampling(self):
        ...
        assert abs(original_ratio - 4/7) < 0.01
...
class TestBatchCreation:
    ...
    def test_batch_size_configuration(self):
        ...
        assert expected_batches == 4  # 3 * 32 + 1 * 4
    ...
    def test_shuffle_batching(self):
        ...
        assert set(shuffled) == set(indices)
