...
class TestIncidentResponsePlanning:
    ...
    def test_incident_detection_and_alerting(self):
        ...
        assert not detector.detect_suspicious_activity(normal), "Condition must be true"

        # Suspicious activity (alert)
        suspicious = {"failed_logins": 20, "data_exfiltration_bytes": 1000}
        assert detector.detect_suspicious_activity(suspicious), "detect is not valid"
        assert len(detector.alerts) > 0, "Collection must not be empty"
    ...
    def test_disaster_recovery_backup_verification(self):
        ...
        assert recovery.verify_backup_recovery(0), "Condition must be true"
