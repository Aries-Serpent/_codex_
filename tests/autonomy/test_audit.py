...
class TestAuditRecord:
    ...
    def test_mode_serialised_as_string(self):
        ...
        assert rec.to_dict()["mode"] == "DRY_RUN"
...
class TestAuditLogger:
    def test_record_writes_ndjson(self, tmp_path):
        ...
        assert data["surface_id"] == "AUT-007"
    ...
    def test_audit_coverage_zero_when_no_runs(self, tmp_path):
        al = _logger(tmp_path)
        assert al.audit_coverage(total_runs=0) == 0.0

    def test_audit_coverage_capped_at_one(self, tmp_path):
        al = _logger(tmp_path)
        al.record(AuditRecord())
        al.record(AuditRecord())
        assert al.audit_coverage(total_runs=1) == 1.0

    def test_audit_coverage_partial(self, tmp_path):
        al = _logger(tmp_path)
        for _ in range(8):
            al.record(AuditRecord())
        assert al.audit_coverage(total_runs=10) == pytest.approx(0.8)
