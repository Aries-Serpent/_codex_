...
class TestASTConfig:
    ...
    def test_to_dict(self) -> None:
        ...
        d = config.to_dict()
        assert d["parser_backend"] == "libcst", "Condition must be true"
        assert d["complexity_threshold"] == 15, "Condition must be true"
        assert "output_path" in d, "Condition must be true"
    ...
    def test_validate_invalid_backend(self) -> None:
        ...
        assert "parser_backend" in errors[0], "Error should be raised or set"
    ...
    def test_validate_invalid_timeout(self) -> None:
        ...
        errors = config.validate()
        assert len(errors) == 1, "Errors must not be empty"
        assert "parse_timeout" in errors[0], "Error should be raised or set"
    ...
    def test_validate_invalid_parallel(self) -> None:
        ...
        errors = config.validate()
        assert len(errors) == 1, "Errors must not be empty"
        assert "max_parallel" in errors[0], "Error should be raised or set"

    def test_supported_languages(self) -> None:
        """Test supported languages list."""
        config = ASTConfig()
        assert "python" in config.supported_languages, "Condition must be true"
        assert "yaml" in config.supported_languages, "Condition must be true"
        assert "json" in config.supported_languages, "Condition must be true"
