...
class TestRunBenchmarks:
    ...
    def test_cpu_suite_runs_all_benchmarks(self):
        ...
        assert "sha256_1MB" in benchmarks
        assert "int_sort_10k" in benchmarks
        assert "math_sqrt_loop" in benchmarks
        assert "json_roundtrip_100k_chars" in benchmarks
    ...
    def test_io_suite_completes(self):
        ...
        assert "io" in results["suites"]
        assert results["suites"]["io"]["elapsed_s"] >= 0

    def test_import_suite_completes(self):
        results = run_benchmarks(["import"])
        assert "import" in results["suites"]
    ...
    def test_all_suites_run_without_error(self):
        ...
        assert len(results["suites"]) >= 3  # import, cpu, io at minimum

    def test_json_serialisable(self):
        results = run_benchmarks(["cpu"])
        dumped = json.dumps(results)
        loaded = json.loads(dumped)
        assert loaded["suites"]["cpu"]["benchmarks"]["sha256_1MB"]["status"] == "ok"
