...
class TestPatternLibrary:
    ...
    def test_confidence_distribution(self, validator):
        ...
        assert (high_conf + medium_high + medium) >= len(scores) * 0.85, "Scores must not be empty"
    ...
    def test_phase_17_report_exists(self):
        ...
        assert "COMPLETE" in content, "Content must not be empty"
        assert "40+" in content, "Content must not be empty"
