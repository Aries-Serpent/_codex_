...
class TestInputValidation:
    ...
    def test_sql_injection_payload_rejected(self):
        ...
        # Valid usernames
        assert validate_username("alice_123") == "alice_123", "Condition must be true"
        assert validate_username("bob-user") == "bob-user", "Condition must be true"
    ...
    def test_xss_payload_rejected_in_user_input(self):
        ...
        # Valid comments
        assert validate_comment("This is a safe comment") == "This is a safe comment", "This is not valid"
    ...
    def test_command_injection_prevented(self):
        ...
        # Valid filenames
        assert validate_filename("report_2026.pdf") == "report_2026.pdf", "Condition must be true"
        assert validate_filename("data-file_v1.csv") == "data-file_v1.csv", "Data must not be empty"
    ...
    def test_path_traversal_prevented(self):
        ...
        assert "/data/documents" in result, "Result must not be empty"
    ...
    def test_email_validation(self):
        ...
        assert validate_email("alice.bob+tag@company.co.uk") == "alice.bob+tag@company.co.uk", "Condition must be true"
    ...
    def test_integer_overflow_prevented(self):
        ...
        # Valid values
        assert validate_quantity(100) == 100, "Condition must be true"
        assert validate_quantity(1000000) == 1000000, "Condition must be true"
...
class TestCryptographicOperations:
    ...
    def test_salt_length_for_password_hashing(self):
        ...
        assert validate_salt(good_salt), "Condition must be true"

        # Strong salt
        strong_salt = os.urandom(32)
        assert validate_salt(strong_salt), "Condition must be true"
    ...
    def test_key_derivation_function_iterations(self):
        ...
        assert validate_kdf_params(100000), "Condition must be true"

        # Strong: exceeds standard
        assert validate_kdf_params(600000), "Condition must be true"
    ...
    def test_hmac_key_length(self):
        ...
        assert validate_hmac_key(good_key), "Condition must be true"

        # Strong key
        strong_key = os.urandom(64)
        assert validate_hmac_key(strong_key), "Condition must be true"
...
class TestDataProtection:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        # Safe logs
        assert log_message("User alice logged in"), "Condition must be true"
        assert log_message("API request to /users endpoint"), "Condition must be true"
    ...
    def test_api_key_not_in_source_code(self):
        ...
        assert contains_hardcoded_secret(code_snippets[0]), "Condition must be true"
        assert not contains_hardcoded_secret(code_snippets[1]), "Condition must be true"
        assert contains_hardcoded_secret(code_snippets[2]), "Condition must be true"
        assert not contains_hardcoded_secret(code_snippets[3]), "Condition must be true"
    ...
    def test_database_connection_uses_tls(self):
        ...
        # Valid: TLS required
        assert validate_db_connection_string("******host/db?sslmode=require"), "Condition must be true"
...
class TestErrorHandling:
    ...
    def test_error_messages_dont_leak_system_info(self):
        ...
        assert "error occurred" in result or len(result) < len(str(leaky_error)), "Result must not be empty"
