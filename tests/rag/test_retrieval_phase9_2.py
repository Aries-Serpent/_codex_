...
class TestRetrievalPipelineInit:
    ...
    def test_pipeline_default_init(self) -> None:
        ...
        assert pipeline.config.top_k == DEFAULT_TOP_K
        assert pipeline.embedding_pipeline is not None
    ...
    def test_pipeline_custom_config(self) -> None:
        ...
        # Assert
        assert pipeline.config.top_k == 20
        assert pipeline.config.similarity_threshold == 0.7
    ...
    def test_pipeline_custom_embedding_pipeline(self) -> None:
        ...
        # Assert
        assert pipeline.embedding_pipeline is mock_embedding
...
class TestAddDocuments:
    ...
    def test_add_multiple_documents(self) -> None:
        ...
        # Assert
        assert added == 3
        assert pipeline.get_document_count() == 3
...
class TestRetrieveBounds:
    ...
    def test_retrieve_max_query_length(self) -> None:
        ...
        assert len(response.query) <= MAX_QUERY_LENGTH
...
class TestRetrieveMetadata:
    ...
    def test_metadata_included_by_default(self) -> None:
        ...
        if len(response.results) > 0:
            assert "key" in response.results[0].metadata or response.results[0].metadata == {}
...
class TestRetrieveSearchTime:
    ...
    def test_search_time_reasonable(self) -> None:
        ...
        assert response.search_time_ms < 1000.0
