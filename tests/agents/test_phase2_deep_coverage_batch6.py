...
class TestPhase2_ChaoticAttractors:
    ...
    def test_lorenz_attractor(self):
        ...
        assert attractor.parameters.get("sigma") == 10.0
        assert attractor.parameters.get("rho") == 28.0
        final_state = attractor.iterate(steps=5)
        assert final_state is not None
        assert len(final_state) == 3  # 3D system
    ...
    def test_henon_map(self):
        ...
        assert attractor.parameters.get("a") == 1.4
        assert attractor.parameters.get("b") == 0.3
        final_state = attractor.iterate(steps=5)
        assert final_state is not None
        assert len(final_state) == 2  # 2D system
    ...
    def test_attractor_history_tracking(self):
        ...
        assert len(attractor.history) == 20
    ...
    def test_generate_test_parameters(self):
        ...
        assert all(len(tc) == 2 for tc in test_cases)
        # Check ranges
        for tc in test_cases:
            assert 0.0 <= tc[0] <= 1.0
            assert 10.0 <= tc[1] <= 20.0
    ...
    def test_inject_chaos(self):
        ...
        assert abs(perturbed - decision) < 0.3
...
class TestPhase2_FluidDynamics:
    ...
    def test_continuity_equation(self):
        ...
        A2 = 1.0
        v2 = (A1 * v1) / A2
        assert v2 == 6.0  # Velocity increases as area decreases
...
class TestPhase2_ElectromagneticFields:
    ...
    def test_poynting_vector(self):
        ...
        S = np.cross(E, B) / mu0
        assert len(S) == 3
        assert S[2] != 0  # Should point in z direction
...
class TestPhase2_WavePropagation:
    ...
    def test_wave_interference_destructive(self):
        ...
        psi_total = psi1 + psi2
        assert abs(psi_total) < 0.1  # Nearly zero
...
class TestPhase2_RelativisticEffects:
    ...
    def test_relativistic_addition_of_velocities(self):
        ...
        c = 1.0
        u = (v + w) / (1.0 + v * w / c**2)
        assert u < c  # Result must be less than c
        assert u > v and u > w  # But greater than components
...
class TestPhase2_NumericalMethods:
    ...
    def test_finite_difference_derivative(self):
        ...
        df_exact = 2 * x
        assert abs(df_numeric - df_exact) < 0.01
    ...
    def test_trapezoidal_integration(self):
        ...
        assert abs(integral_numeric - integral_exact) < 0.01
    ...
    def test_bisection_root_finding(self):
        ...
        while (b - a) / 2 > tol:
            ...
            if f(a) * f(c) < 0:
                ...
                a = c
        root = (a + b) / 2
        assert abs(root - np.sqrt(2)) < 1e-5
