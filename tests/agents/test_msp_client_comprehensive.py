...
class TestMSPClientMocked:
    ...
    def test_client_handles_connection_error(self, mock_client):
        ...
        if hasattr(mock_client, 'request'):
            with patch.object(mock_client, 'request', side_effect=ConnectionError("Connection failed")):
    ...
    def test_client_handles_timeout(self, mock_client):
        ...
        if hasattr(mock_client, 'request'):
            with patch.object(mock_client, 'request', side_effect=socket.timeout("Request timed out")):
...
class TestMSPClientAuthentication:
    ...
    def test_api_key_in_headers(self):
        ...
        if hasattr(client, '_build_headers'):
            ...
            has_auth = any(key.lower() in ['authorization', 'x-api-key', 'api-key'] 
    ...
    def test_bearer_token_format(self):
        ...
        if hasattr(client, '_build_headers'):
            ...
            if 'Authorization' in headers:
                assert 'Bearer' in headers['Authorization'] or 'bearer_token' in headers['Authorization']
