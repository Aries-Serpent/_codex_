...
class TestHybridPhysicsOrchestrator:
    ...
    @pytest.mark.skipif(not PHYSICS_ORCHESTRATOR_AVAILABLE, reason="Physics orchestrator not available")
    def test_classical_physics_integration(self, orchestrator):
        """Test integration with classical physics orchestrator."""
        assert orchestrator.classical_orchestrator is not None
        
        decision_space = {
            'current_position': 'state_1',
            'goal_position': 'state_2'
        }
    ...
    def test_result_structure_consistency(self, orchestrator):
        ...
        required_fields = ['paradigms_used', 'classical_physics', 'advanced_physics', 'recommendations']
