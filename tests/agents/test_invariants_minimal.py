...
class TestPhysicsIntegrationInvariants:
    ...
    def test_capabilities_keys_valid(self):
        ...
        assert any(key in caps for key in [
            'classical_physics', 'chaos_theory', 'fluid_dynamics',
            'electromagnetic_fields', 'wave_propagation', 'fractal_geometry'
        ])
...
class TestSelfHealingCorrected:
    ...
    def test_engine_detect_issues_method_exists(self):
        ...
        assert hasattr(engine, 'detect_issues') or hasattr(engine, 'detect') or hasattr(engine, 'analyze')
