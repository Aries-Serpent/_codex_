...
class TestMemorySystemProperties:
    ...
    @given(
        memory_id=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),
    ...
    @given(
        access_count=st.integers(min_value=0, max_value=1000000)
    )
...
class TestMathematicalProperties:
    ...
    @given(
        temperature=st.floats(min_value=0.01, max_value=100.0, allow_nan=False, allow_infinity=False),
...
class TestPropertyBasedSuite:
    ...
    def test_composite_properties(self, data):
        ...
        energy = data.draw(st.floats(min_value=0.0, max_value=100.0, allow_nan=False, allow_infinity=False))
        temperature = data.draw(st.floats(min_value=0.1, max_value=10.0, allow_nan=False, allow_infinity=False))
