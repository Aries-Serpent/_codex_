...
class TestVectorEncoder:
    ...
    def test_encode_observation_returns_vector(self):
        ...
        assert len(vector) == 128, "Vector must not be empty"
...
async def test_get_patterns_retrieves_similar_patterns():
    ...
    assert all("similarity" in p for p in patterns), "Condition must be true"
...
async def test_get_sessions_retrieves_successful_sessions():
    ...
    assert len(sessions) > 0, "Sessions must not be empty"
...
async def test_inject_context_full_context():
    ...
    assert "patterns" in context, "Condition must be true"
    assert "sessions" in context, "Condition must be true"
    assert "external" in context, "Condition must be true"
...
def test_calculate_confidence_full_context():
    ...
    assert confidence.overall_confidence > 0.5, "overall_confidence must be greater than zero"
    assert confidence.pattern_similarity_score > 0, "pattern_similarity_score must be greater than zero"
    assert confidence.session_recency_score > 0, "session_recency_score must be greater than zero"
    assert confidence.external_reliability > 0, "external_reliability must be greater than zero"
...
def test_calculate_confidence_patterns_only():
    ...
    assert confidence.overall_confidence > 0, "overall_confidence must be greater than zero"
    assert confidence.pattern_similarity_score > 0, "pattern_similarity_score must be greater than zero"
    assert confidence.session_recency_score == 0, "session_recency_score is not valid"
...
def test_calculate_confidence_empty_context():
    ...
    assert confidence.overall_confidence == 0.0, "overall_confidence is not valid"
...
def test_confidence_metrics_breakdown():
    ...
    assert "breakdown" in confidence.__dict__, "Condition must be true"
    assert isinstance(confidence.breakdown, dict)
    assert len(confidence.breakdown) > 0, "Collection must not be empty"
...
def test_fuse_patterns_sorts_by_score():
    ...
    assert fused[0]["pattern_id"] == "pat_002", "Condition must be true"
...
def test_fuse_patterns_respects_top_k():
    ...
    assert len(fused) == 3, "Fused must not be empty"
...
def test_fuse_sessions_filters_successful():
    ...
    assert all(s.get("success") for s in fused), "Condition must be true"
    assert len(fused) == 2, "Fused must not be empty"
...
async def test_metrics_aggregation():
    ...
    assert metrics["total_context_requests"] == 5, "Condition must be true"
    assert "avg_confidence" in metrics, "Condition must be true"
    assert "median_confidence" in metrics, "Condition must be true"
    assert "p99_confidence" in metrics, "Condition must be true"
