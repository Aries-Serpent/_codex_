...
class SelfHealingEngine:
    ...
    def _register_default_handlers(self) -> None:
        ...
        self.remediation_handlers[IssueType.DEPENDENCY_CONFLICT] = self._remediate_dependency_conflict
        ...
        for issue_type, patterns in self.issue_patterns.items():
            for pattern, description_template in patterns:
                ...
                for match in matches:
                    issue_counter += 1
                    groups = match.groups()
                    description = description_template.format(*groups) if groups else description_template
