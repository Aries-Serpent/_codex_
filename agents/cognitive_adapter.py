...
"""
MIGRATION GUIDE: Legacy Agent to Cognitive Architecture

Step 1: Wrap existing agent
------------------------------
...
# After (wrapped):
agent = wrap_legacy_agent(MyLegacyAgent())
result = agent.ooda_loop(data)

Step 2: Gradual refactoring
------------------------------
Instead of wrapping, inherit from Planner ABC:

from cognitive_brain.base import Planner, ObservationData, OrientationResult, Decision, ActionResult

class MyModernizedAgent(Planner):
...
Step 3: Use through cognitive_app
------------------------------
from cognitive_app.src.orchestrator import process_through_cognitive_app, initialize_cognitive_app

agent = MyModernizedAgent()
memory = SimpleDictMemory()
initialize_cognitive_app(agent, memory)

result = process_through_cognitive_app({"input": "data"})
"""
