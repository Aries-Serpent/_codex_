...
- Made Chronicle check-only diagnostics read-only for tracked metrics/state with a 120-second bound, and prevented Rust CI from running on `.codex/**`-only changes.
- Focused regression batches passed with 9 and 24 tests respectively; the modified-file secret scan was clean.

### Fixed — federated release and trust-boundary hardening
- Removed matrix expressions from PyPI job-level conditions, kept package selection on operational steps, and separated unprivileged validation from OIDC publication.
- Locked the release build toolchain, rejected generated bytecode in distribution archives, and retained isolated installation, tag, provenance, and attestation checks.
- Made audit-log appends cross-process atomic, treated malformed records as invalid chains, and enforced plugin execution deadlines with process termination and quarantine.
