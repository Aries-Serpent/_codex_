...
- **`.github/workflows/iterative-self-healing-ci.yml`**: Fixed self-healing cascade root cause (SELF_HEALING_001) — both `triage` and `heal` jobs now use resilient pip fallback (`(.venv_ci/bin/pip install ... 2>/dev/null) || pip install ...`). When `.venv_ci/` cache misses, the system pip is used instead of failing the workflow. Expected to reduce CI failure rate from 13.3% → <1%.
