...
- **lint/sec:** Add Ruff formatting, `pip-audit` hook, and automatic detect-secrets baseline hint.
- **nox:** Reuse venvs, stop on first error, and enforce coverage fail-under.
- **trainer:** Harden early-stop metric wiring and scheduler hookup.
- **tokenizer:** Safe `pad_id` fallback when SPM model lacks pad token.
- **tracking:** Helpers for W&B offline and local MLflow logging.
