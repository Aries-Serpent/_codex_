...
from performance_optimizations import LazyImportCache, ModuleLoadOptimizer, PerformanceMonitor
from regression_detector import RegressionDetector, PerformanceGate
...
def test_module_load_optimizer():
    ...
    print(f"✅ Batch import {len([m for m in modules_dict.values() if m])} modules in {elapsed:.2f}ms")
...
def test_performance_monitor():
    ...
    print(f"✅ Operation A: mean={stats_a['mean']:.2f}ms, min={stats_a['min']:.2f}ms, max={stats_a['max']:.2f}ms")
    
    stats_b = monitor.get_stats("operation_b")
    print(f"✅ Operation B: mean={stats_b['mean']:.2f}ms, min={stats_b['min']:.2f}ms, max={stats_b['max']:.2f}ms")
...
def main():
    ...
    results = {
        'optimization_1_lazy_cache': test_lazy_import_cache(),
        'optimization_2_module_optimizer': test_module_load_optimizer(),
        'performance_monitoring': test_performance_monitor(),
        'regression_detection': test_regression_detection(),
        'optimization_impact': test_optimization_impact(),
    }
