...
ROOT = Path(__file__).resolve().parents[1]  # repo root (assumes .codex/run_db_utils_workflow.py)
...
def append_change(file_path: Path, action: str, rationale: str, before: str, after: str):
    diff = "\n".join(
...
def write_if_changed(path: Path, content: str, rationale: str):
    ...
    if before != after:
        ...
        append_change(path, "write" if not before else "update", rationale, before, after)
...
def safe_inject_imports(pth: Path):
    ...
    if "from .db_utils import" in before or "from src.codex.logging.db_utils import" in before:
