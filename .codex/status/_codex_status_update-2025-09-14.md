...
**Risk:** Forcing gradient accumulation to 1 when LoRA is enabled Phase 5 slow training if users wanted accumulation. It ensures LoRA layers receive full gradients, but may not be necessary for all adapter implementations. To mitigate, warn rather than force the value.
