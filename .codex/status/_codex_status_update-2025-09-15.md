# _codex_: Status Update (Previous Cycle-09-15)
===================================
...
15. **Optional dependencies** – Many features rely on optional libraries (MLflow, W&B, Pynvml). Without proper dependency management or clear error messages, features Phase 5 silently fail.
...
**Risk:** If hyper-parameters mismatch existing defaults, performance Phase 5 change. YAML loading errors could break training.
...
**Risk:** Introduces CLI dependencies (click) and Phase 5 break for missing config files. Changing function signatures could require refactoring tests.
...
**Risk:** Sanitisation Phase 5 remove legitimate content; performance overhead from regex checks. Users expecting raw output Phase 5 be surprised.
...
**Risk:** If checkpoint loading fails or mismatched hyper-parameters, training Phase 5 crash. Resume Phase 5 inadvertently load stale checkpoints.
...
| **Deterministic datasets** | ✗ | Data splitting is deterministic when seeds provided[GitHub](https://github.com/Aries-Serpent/_codex_/blob/9c76af46886b0aa06944992086a904384f63e304/src/codex_ml/data_utils.py#L1-L143), but the absence of config/seed means splits Phase 5 vary. |
...
```vbnet
Question for ChatGPT @codex Previous Cycle-09-15T14:00:00Z: While performing Step 3: Loading Hydra config, encountered the following error: FileNotFoundError: configs/training/base.yaml not found
