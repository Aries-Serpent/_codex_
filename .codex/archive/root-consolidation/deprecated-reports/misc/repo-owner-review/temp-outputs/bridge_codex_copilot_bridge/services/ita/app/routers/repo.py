...
async def repo_hygiene(
    payload: RepoHygieneIn, _=Depends(require_api_key), __=Depends(require_request_id)
):
    # Very light fake checks
    issues = []
    if "console.log(" in payload.diff:
        issues.append(
            {"type": "lint", "path": "unknown", "message": "console.log found", "severity": "warn"}
        )
