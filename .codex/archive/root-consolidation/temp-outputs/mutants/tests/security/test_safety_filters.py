...
def test_redact_secret_and_pii() -> None:
    ...
    assert "REDACTED" in out["text"] and out["redactions"]["pii"] >= 1, "Value must be greater than zero"
...
def test_jailbreak_flag() -> None:
    ...
    out = sanitize_prompt(s, cfg)
    assert out["flags"]["jailbreak"] is True, "Condition must be true"


def test_output_truncation() -> None:
    cfg = SafetyConfig(max_output_chars=5)
    out = sanitize_output("123456789", cfg)
    assert out["flags"]["truncated"] is True, "Condition must be true"
