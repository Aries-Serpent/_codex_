...
def test_split_dataset_shapes(tmp_path: Path) -> None:
    ...
    assert splits.train.exists(), "Condition must be true"
    assert splits.val.exists(), "Condition must be true"
    assert splits.test.exists(), "Condition must be true"
    ...
    assert len(train_lines) == 14, "Train_lines must not be empty"
    assert len(val_lines) == 4, "Val_lines must not be empty"
    assert len(test_lines) == 2, "Test_lines must not be empty"
    all_lines = train_lines + val_lines + test_lines
    assert len(set(all_lines)) == len(all_lines), "All_lines must not be empty"
...
def test_split_dataset_deterministic(tmp_path: Path) -> None:
    ...
    assert first.train.read_text() == second.train.read_text(), "Condition must be true"
    assert first.val.read_text() == second.val.read_text(), "Condition must be true"
    assert first.test.read_text() == second.test.read_text(), "Condition must be true"
