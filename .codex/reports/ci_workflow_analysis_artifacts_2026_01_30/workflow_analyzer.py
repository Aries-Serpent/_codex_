...
class WorkflowAnalyzer:
    ...
    def _check_guards(self, content: str, data: dict) -> bool:
        ...
        if "on" in data:
            ...
            if isinstance(on_triggers, dict):
                for trigger_type, trigger_config in on_triggers.items():
                    if isinstance(trigger_config, dict) and not trigger_config.get(
                        "if"
                    ):
                        return True
        ...
        if "jobs" in data:
            for job_name, job_config in data["jobs"].items():
                if isinstance(job_config, dict):
                    ...
                    if (
                        if_condition in ["false", False]
                        or "false" in str(if_condition).lower()
                    ):
                        return True
        ...
        if (
            ...
            and "#" in content.split("workflow_dispatch:")[0].split("\n")[-1]
        ):
            return True
    ...
    def _extract_dependencies(self, content: str) -> dict[str, bool]:
        ...
        return {
            "docker": bool(
                re.search(r"docker (build|run|push)", content, re.IGNORECASE)
            ),
    ...
    def generate_markdown_report(self) -> str:
        ...
        md.append(
            f"- **Guarded Workflows**: {summary['guarded_workflows']} (if: false or disabled)"
        )
        md.append(
        ...
        for workflow_name in sorted(self.workflows.keys()):
            ...
            md.append(
                f"| {workflow_name} | {status} | {jobs_count} | {triggers} | {runner} | {secrets_count} | {deps_str} | {priority} |"
            )
    ...
    def _determine_priority(self, workflow_name: str, info: dict) -> str:
        ...
        if any(
            x in workflow_name.lower() for x in ["ci", "test", "pr-checks", "security"]
        ):
            return "🔴 Critical"

        # High: Build, deploy, release
        if any(
            x in workflow_name.lower()
            for x in ["build", "deploy", "release", "publish"]
        ):
            return "🟠 High"
        ...
        if any(
            x in workflow_name.lower()
            for x in ["automation", "monitor", "audit", "scan"]
        ):
            return "🟡 Medium"
    ...
    def _categorize_workflows(self) -> dict[str, list[str]]:
        ...
        for workflow_name, info in self.workflows.items():
            ...
            if any(x in wf_lower for x in ["test", "pytest", "ci", "pr-check"]):
                ...
                x in wf_lower
                for x in ["security", "scan", "codeql", "bandit", "semgrep"]
            ):
                categories["Security"].append(workflow_name)
            elif any(
                x in wf_lower
                for x in ["build", "docker", "deploy", "publish", "release"]
            ):
...
def main():
    ...
    with open(json_path, "w", encoding="utf-8") as f:  # nosec  # B110 JSON report stores only non-sensitive workflow metadata
        f.write(json_report)
    print(f"📄 JSON report saved to: {json_path}")

    with open(md_path, "w", encoding="utf-8") as f:  # nosec  # codeql[py/clear-text-storage-sensitive-data]
        f.write(md_report)  # nosec  # codeql[py/clear-text-storage-sensitive-data]
