...
## 9. Create `agent-auth-delegation` Environment (one-time)

**Why:** The [`agent-auth-delegation.yml`](../../.github/workflows/agent-auth-delegation.yml)
workflow uses a GitHub Environment protection rule as the approval gate. When a
PR body contains the checked `COPILOT_AGENT_AUTH_ENABLED` checkbox, the workflow
pauses at this environment and shows **"Waiting for approval"** in the Actions UI.
Approving it activates token delegation and automatically posts `@copilot continue`
on the PR.
...
2. Click **"New environment"** → Name it exactly: `agent-auth-delegation`

3. Under **"Deployment protection rules"**:
   - Enable **"Required reviewers"**
   - Add `mbaetiong` as required reviewer
   - Click **"Save protection rules"**

4. Leave all other settings at defaults (no wait timer needed)

### How it works (end-to-end)

```
PR author checks ☑ Enable Agent Token Delegation
        ↓
agent-auth-delegation.yml fires (detect-checkbox job)
        ↓
await-approval job hits environment gate
        ↓  ← GitHub Actions UI shows "Waiting for approval" 🔴
@mbaetiong clicks "Approve" in Actions UI
        ↓
activate-delegation job runs:
  • CODEX_MASTER_KEY sets COPILOT_AGENT_AUTH_ENABLED = true
  • COGNITIVE_BRAIN_ALLOWED_ACTORS updated (Set-based dedup, S110 pattern)
  • GitHub Guru dispatched to verify variable state (maintainer role)
  • @copilot continue posted to PR 💬
```
