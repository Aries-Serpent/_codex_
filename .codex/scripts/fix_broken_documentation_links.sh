...
grep -r "example\.com" "$REPO_ROOT/docs" --include="*.md" -l 2>/dev/null | while read -r file; do
  ...
  # Replace other example.com URLs with localhost
  sed -i 's|https://\([^[:space:]]*\)\.example\.com|http://localhost:8080|g' "$file"
  sed -i 's|http://\([^[:space:]]*\)\.example\.com|http://localhost:8080|g' "$file"
