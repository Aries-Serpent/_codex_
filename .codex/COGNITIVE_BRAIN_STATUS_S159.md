...
| `deferral-language-gate.yml` | ✅ FIXED | Remove `edited` from PR triggers; eliminates cancel race with `synchronize` |
| `pr_comment_consolidator.py` | ✅ FIXED | Check run deduplication by name; GraphQL `reviewThreads.isResolved` for accurate unresolved count |
...
| Agent Token Delegation | Cascading cancellations from `edited` PR trigger | `if: vars != 'true'` guard + remove `edited` | S159 |
