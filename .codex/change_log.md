...
### 2025-08-19T07:42:51Z — /workspace/_codex_/src/codex/logging/db_utils.py
- **Action:** write
- **Rationale:** Introduce shared SQLite discovery helpers (LIKELY_MAP, infer_*, open_db).

```diff
--- a//workspace/_codex_/src/codex/logging/db_utils.py
+++ b//workspace/_codex_/src/codex/logging/db_utils.py
@@ -0,0 +1,72 @@
+# Auto-generated by .codex/run_db_utils_workflow.py
...
### 2025-08-19T07:42:51Z — /workspace/_codex_/src/codex/logging/query_logs.py
- **Action:** update
- **Rationale:** Add helper wrapper to centralize schema inference via db_utils.
...
+codex.logging.query_logs: Query transcripts from a SQLite database.# --- injected by workflow: wrap local inference with shared helpers ---
...
### 2025-08-19T07:42:51Z — /workspace/_codex_/src/codex/logging/query_logs.py
- **Action:** update
- **Rationale:** Inject db_utils imports (safe, dual-namespace).
...
### 2025-08-19T07:42:51Z — /workspace/_codex_/src/codex/logging/export.py
- **Action:** update
- **Rationale:** Add helper wrapper to centralize schema inference via db_utils.
...
+"""codex.logging.export: Dump session events from a SQLite DB.# --- injected by workflow: wrap local inference with shared helpers ---
+def _infer_table_and_columns(db_path=None, explicit_table=None):
+    con = open_db(db_path)
+    try:
+        table = explicit_table or infer_probable_table(con)
...
### 2025-08-19T07:42:51Z — /workspace/_codex_/tests/test_db_utils.py
- **Action:** write
- **Rationale:** Add smoke tests for db_utils inference across common variants.

```diff
--- a//workspace/_codex_/tests/test_db_utils.py
+++ b//workspace/_codex_/tests/test_db_utils.py
@@ -0,0 +1,46 @@
...
### 2025-08-19T07:45:29Z — src/codex/logging/export.py
- **Action:** refactor
- **Rationale:** centralize schema inference via db_utils helpers.

```diff
diff --git a/src/codex/logging/export.py b/src/codex/logging/export.py
index 20c53e3..133c2a3 100644
--- a/src/codex/logging/export.py
+++ b/src/codex/logging/export.py
@@ -22,0 +23,5 @@ from typing import Iterable, List, Dict, Any, Tuple
+try:
+    from .db_utils import open_db, infer_probable_table, infer_columns
+except Exception:  # fallback for alternative namespace
+    from src.codex.logging.db_utils import open_db, infer_probable_table, infer_columns
+
@@ -45,35 +49,0 @@ def _db_path(override: str | None = None) -> str:
-LIKELY_MAP = {
-    "timestamp": [
-        "timestamp",
-        "ts",
...
-
-
@@ -81 +51 @@ def _fetch_events(db_path: str, session_id: str) -> List[Dict[str, Any]]:
-    conn = sqlite3.connect(db_path)
+    conn = open_db(db_path)
@@ -84 +54,4 @@ def _fetch_events(db_path: str, session_id: str) -> List[Dict[str, Any]]:
...
### 2025-08-19T07:45:29Z — src/codex/logging/query_logs.py
- **Action:** refactor
- **Rationale:** use shared db_utils helpers and remove duplicated inference logic.

```diff
diff --git a/src/codex/logging/query_logs.py b/src/codex/logging/query_logs.py
index bb1fb96..57036d4 100644
--- a/src/codex/logging/query_logs.py
+++ b/src/codex/logging/query_logs.py
@@ -28,0 +29,5 @@ from typing import Any, Dict, List, Optional, Tuple
+try:
+    from .db_utils import open_db, infer_probable_table, infer_columns
+except Exception:  # fallback for alternative namespace
+    from src.codex.logging.db_utils import open_db, infer_probable_table, infer_columns
+
@@ -46,18 +50,0 @@ def parse_when(s: Optional[str]) -> Optional[str]:
-LIKELY_MAP = {
-    "timestamp": [
-        "created_at",
-        "timestamp",
...
-
-
@@ -75,28 +61,0 @@ def _resolve_db_path(path: str) -> str:
...
-
-
@@ -200 +159,2 @@ def main(argv: Optional[List[str]] = None) -> int:
-        conn = open_db(args.db)
+        db_path = _resolve_db_path(args.db) if args.db else args.db
+        conn = open_db(db_path)
@@ -202 +162,4 @@ def main(argv: Optional[List[str]] = None) -> int:
