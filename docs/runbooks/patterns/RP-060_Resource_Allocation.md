...
**Problem**: Automated detection of resource allocation in CI workflows.
...
Resource Allocation failures typically result from:
...
```python
def detect_rp_060(log_text: str) -> bool:
...
```python
def analyze_rp_060(context: str) -> Dict:
    return {
        "pattern": "RP-060",
        "confidence": 0.85,
        "category": "infrastructure",
