...
**Problem**: Automated detection of secret injection in CI workflows.
...
Secret Injection failures typically result from:
...
```python
def detect_rp_064(log_text: str) -> bool:
...
```python
def analyze_rp_064(context: str) -> Dict:
    return {
        "pattern": "RP-064",
        "confidence": 0.92,
        "category": "multi-environment",
