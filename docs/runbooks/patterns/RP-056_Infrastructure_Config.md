...
**Problem**: Automated detection of infrastructure config in CI workflows.
...
Infrastructure Config failures typically result from:
...
```python
def detect_rp_056(log_text: str) -> bool:
...
```python
def analyze_rp_056(context: str) -> Dict:
    return {
        "pattern": "RP-056",
        "confidence": 0.92,
        "category": "infrastructure",
