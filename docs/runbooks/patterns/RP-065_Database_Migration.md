...
**Problem**: Automated detection of database migration in CI workflows.
...
Database Migration failures typically result from:
...
```python
def detect_rp_065(log_text: str) -> bool:
...
```python
def analyze_rp_065(context: str) -> Dict:
    return {
        "pattern": "RP-065",
        "confidence": 0.9,
        "category": "multi-environment",
