...
**Problem**: Automated detection of memory leak in CI workflows.
...
Memory Leak failures typically result from:
...
```python
def detect_rp_047(log_text: str) -> bool:
...
```python
def analyze_rp_047(context: str) -> Dict:
    return {
        "pattern": "RP-047",
        "confidence": 0.93,
        "category": "performance-regression",
        "action": "remediate" if 0.93 >= 0.95 else "review"
    }
```
