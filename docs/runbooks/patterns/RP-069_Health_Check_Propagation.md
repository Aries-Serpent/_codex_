# RP-069: Health Check Propagation
...
**Problem**: Automated detection of health check propagation in CI workflows.
...
Health Check Propagation failures typically result from:
...
```python
def detect_rp_069(log_text: str) -> bool:
...
```python
def analyze_rp_069(context: str) -> Dict:
    return {
        "pattern": "RP-069",
        "confidence": 0.83,
        "category": "multi-environment",
...
```
ERROR: Health Check Propagation
Context: multi
Severity: low
```
...
```
✅ RP-069 remediation applied
✅ Health Check Propagation resolved
