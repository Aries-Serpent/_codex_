...
**Problem**: Automated detection of feature flag in CI workflows.
...
Feature Flag failures typically result from:
...
```python
def detect_rp_025(log_text: str) -> bool:
...
```python
def analyze_rp_025(context: str) -> Dict:
    return {
        "pattern": "RP-025",
        "confidence": 0.9,
        "category": "deployment-pipeline",
