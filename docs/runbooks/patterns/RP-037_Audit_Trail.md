...
**Problem**: Automated detection of audit trail in CI workflows.
...
Audit Trail failures typically result from:
...
```python
def detect_rp_037(log_text: str) -> bool:
...
```python
def analyze_rp_037(context: str) -> Dict:
    return {
        "pattern": "RP-037",
        "confidence": 0.83,
        "category": "security-scanning",
