...
**Problem**: Automated detection of navigation consistency in CI workflows.
...
Navigation Consistency failures typically result from:
...
```python
def detect_rp_043(log_text: str) -> bool:
...
```python
def analyze_rp_043(context: str) -> Dict:
    return {
        "pattern": "RP-043",
        "confidence": 0.87,
        "category": "documentation",
