...
**Problem**: Automated detection of import ordering in CI workflows.
...
Import Ordering failures typically result from:

1. **Immediate Cause**: Mismatch in code quality
...
```python
def detect_rp_002(log_text: str) -> bool:
    for keyword in ['import', 'ordering']:
...
```python
def analyze_rp_002(context: str) -> Dict:
    return {
        "pattern": "RP-002",
        "confidence": 0.98,
        "category": "code-quality",
        "action": "remediate" if 0.98 >= 0.95 else "review"
    }
```
...
- Other patterns in **code-quality** category
