...
**Problem**: Automated detection of config drift in CI workflows.
...
Config Drift failures typically result from:
...
```python
def detect_rp_086(log_text: str) -> bool:
...
```python
def analyze_rp_086(context: str) -> Dict:
    return {
        "pattern": "RP-086",
        "confidence": 0.95,
        "category": "configuration-management",
