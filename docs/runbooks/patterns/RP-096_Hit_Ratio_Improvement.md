...
**Problem**: Automated detection of hit ratio improvement in CI workflows.
...
Hit Ratio Improvement failures typically result from:
...
```python
def detect_rp_096(log_text: str) -> bool:
...
```python
def analyze_rp_096(context: str) -> Dict:
    return {
        "pattern": "RP-096",
        "confidence": 0.91,
        "category": "cache-optimization",
        "action": "remediate" if 0.91 >= 0.95 else "review"
    }
```
...
```
✅ RP-096 remediation applied
✅ Hit Ratio Improvement resolved
