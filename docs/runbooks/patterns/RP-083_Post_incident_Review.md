...
**Problem**: Automated detection of post-incident review in CI workflows.
...
Post-incident Review failures typically result from:
...
```python
def detect_rp_083(log_text: str) -> bool:
...
```python
def analyze_rp_083(context: str) -> Dict:
    return {
        "pattern": "RP-083",
        "confidence": 0.87,
        "category": "incident-response",
...
```
✅ RP-083 remediation applied
✅ Post-incident Review resolved
