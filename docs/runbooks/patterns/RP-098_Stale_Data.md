...
**Problem**: Automated detection of stale data in CI workflows.
...
Stale Data failures typically result from:
...
```python
def detect_rp_098(log_text: str) -> bool:
...
```python
def analyze_rp_098(context: str) -> Dict:
    return {
        "pattern": "RP-098",
        "confidence": 0.88,
        "category": "cache-optimization",
