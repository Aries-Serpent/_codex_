...
**Problem**: Automated detection of memory pressure in CI workflows.
...
Memory Pressure failures typically result from:
...
```python
def detect_rp_097(log_text: str) -> bool:
...
```python
def analyze_rp_097(context: str) -> Dict:
    return {
        "pattern": "RP-097",
        "confidence": 0.89,
        "category": "cache-optimization",
        "action": "remediate" if 0.89 >= 0.95 else "review"
    }
```
