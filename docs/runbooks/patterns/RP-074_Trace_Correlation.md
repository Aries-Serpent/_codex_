...
**Problem**: Automated detection of trace correlation in CI workflows.
...
Trace Correlation failures typically result from:
...
```python
def detect_rp_074(log_text: str) -> bool:
...
```python
def analyze_rp_074(context: str) -> Dict:
    return {
        "pattern": "RP-074",
        "confidence": 0.88,
        "category": "monitoring",
