...
**Problem**: Automated detection of api null handling in CI workflows.
...
API Null Handling failures typically result from:

1. **Immediate Cause**: Mismatch in error prevention
...
```python
def detect_rp_001(log_text: str) -> bool:
    for keyword in ['NoneType', 'null']:
...
```python
def analyze_rp_001(context: str) -> Dict:
    return {
        "pattern": "RP-001",
        "confidence": 0.99,
        "category": "error-prevention",
        "action": "remediate" if 0.99 >= 0.95 else "review"
    }
```
...
```
✅ RP-001 remediation applied
✅ API Null Handling resolved
...
- Other patterns in **error-prevention** category
