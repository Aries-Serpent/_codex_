...
**Problem**: Automated detection of environment variables in CI workflows.
...
Environment Variables failures typically result from:
...
```python
def detect_rp_093(log_text: str) -> bool:
...
```python
def analyze_rp_093(context: str) -> Dict:
    return {
        "pattern": "RP-093",
        "confidence": 0.83,
        "category": "configuration-management",
