...
**Problem**: Automated detection of compliance check in CI workflows.
...
Compliance Check failures typically result from:
...
```python
def detect_rp_036(log_text: str) -> bool:
...
```python
def analyze_rp_036(context: str) -> Dict:
    return {
        "pattern": "RP-036",
        "confidence": 0.85,
        "category": "security-scanning",
