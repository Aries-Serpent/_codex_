# RP-009: Test Execution Order Dependency
...
**Problem**: Automated detection of test execution order dependency in CI workflows.
...
Test Execution Order Dependency failures typically result from:

1. **Immediate Cause**: Mismatch in test infrastructure
...
```python
def detect_rp_009(log_text: str) -> bool:
    for keyword in ['test order', 'isolation']:
...
```python
def analyze_rp_009(context: str) -> Dict:
    return {
        "pattern": "RP-009",
        "confidence": 0.9,
        "category": "test-infrastructure",
...
```
ERROR: Test Execution Order Dependency
Context: test order
Severity: high
```
...
```
✅ RP-009 remediation applied
✅ Test Execution Order Dependency resolved
...
- Other patterns in **test-infrastructure** category
