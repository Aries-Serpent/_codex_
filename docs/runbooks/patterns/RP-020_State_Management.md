...
**Problem**: Automated detection of state management in CI workflows.
...
State Management failures typically result from:
...
```python
def detect_rp_020(log_text: str) -> bool:
...
```python
def analyze_rp_020(context: str) -> Dict:
    return {
        "pattern": "RP-020",
        "confidence": 0.85,
        "category": "workflow-orchestration",
