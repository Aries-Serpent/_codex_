...
**Problem**: Automated detection of schema evolution in CI workflows.
...
Schema Evolution failures typically result from:
...
```python
def detect_rp_088(log_text: str) -> bool:
...
```python
def analyze_rp_088(context: str) -> Dict:
    return {
        "pattern": "RP-088",
        "confidence": 0.92,
        "category": "configuration-management",
