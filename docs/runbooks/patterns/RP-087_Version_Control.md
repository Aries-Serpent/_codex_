...
**Problem**: Automated detection of version control in CI workflows.
...
Version Control failures typically result from:
...
```python
def detect_rp_087(log_text: str) -> bool:
...
```python
def analyze_rp_087(context: str) -> Dict:
    return {
        "pattern": "RP-087",
        "confidence": 0.93,
        "category": "configuration-management",
