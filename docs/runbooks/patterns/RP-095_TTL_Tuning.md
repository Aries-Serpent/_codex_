...
**Problem**: Automated detection of ttl tuning in CI workflows.
...
TTL Tuning failures typically result from:
...
```python
def detect_rp_095(log_text: str) -> bool:
...
```python
def analyze_rp_095(context: str) -> Dict:
    return {
        "pattern": "RP-095",
        "confidence": 0.93,
        "category": "cache-optimization",
