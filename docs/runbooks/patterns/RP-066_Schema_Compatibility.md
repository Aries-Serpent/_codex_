...
**Problem**: Automated detection of schema compatibility in CI workflows.
...
Schema Compatibility failures typically result from:
...
```python
def detect_rp_066(log_text: str) -> bool:
...
```python
def analyze_rp_066(context: str) -> Dict:
    return {
        "pattern": "RP-066",
        "confidence": 0.88,
        "category": "multi-environment",
