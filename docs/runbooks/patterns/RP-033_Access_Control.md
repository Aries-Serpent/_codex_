...
**Problem**: Automated detection of access control in CI workflows.
...
Access Control failures typically result from:
...
```python
def detect_rp_033(log_text: str) -> bool:
...
```python
def analyze_rp_033(context: str) -> Dict:
    return {
        "pattern": "RP-033",
        "confidence": 0.9,
        "category": "security-scanning",
