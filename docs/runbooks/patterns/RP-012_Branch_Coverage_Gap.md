...
**Problem**: Automated detection of branch coverage gap in CI workflows.
...
Branch Coverage Gap failures typically result from:
...
```python
def detect_rp_012(log_text: str) -> bool:
    for keyword in ['branch', 'coverage']:
...
```python
def analyze_rp_012(context: str) -> Dict:
    return {
        "pattern": "RP-012",
        "confidence": 0.83,
        "category": "test-coverage",
...
```
✅ RP-012 remediation applied
✅ Branch Coverage Gap resolved
