# RP-006: Parameterized Test Flakiness
...
**Problem**: Automated detection of parameterized test flakiness in CI workflows.
...
Parameterized Test Flakiness failures typically result from:
...
```python
def detect_rp_006(log_text: str) -> bool:
    for keyword in ['parametrize', 'fixture']:
...
```python
def analyze_rp_006(context: str) -> Dict:
    return {
        "pattern": "RP-006",
        "confidence": 0.92,
        "category": "test-infrastructure",
...
```
ERROR: Parameterized Test Flakiness
Context: parametrize
Severity: high
```
...
```
✅ RP-006 remediation applied
✅ Parameterized Test Flakiness resolved
