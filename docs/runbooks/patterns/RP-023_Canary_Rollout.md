...
**Problem**: Automated detection of canary rollout in CI workflows.
...
Canary Rollout failures typically result from:
...
```python
def detect_rp_023(log_text: str) -> bool:
...
```python
def analyze_rp_023(context: str) -> Dict:
    return {
        "pattern": "RP-023",
        "confidence": 0.93,
        "category": "deployment-pipeline",
...
```
✅ RP-023 remediation applied
✅ Canary Rollout resolved
