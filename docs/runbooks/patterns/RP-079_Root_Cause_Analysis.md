...
**Problem**: Automated detection of root cause analysis in CI workflows.
...
Root Cause Analysis failures typically result from:
...
```python
def detect_rp_079(log_text: str) -> bool:
...
```python
def analyze_rp_079(context: str) -> Dict:
    return {
        "pattern": "RP-079",
        "confidence": 0.93,
        "category": "incident-response",
