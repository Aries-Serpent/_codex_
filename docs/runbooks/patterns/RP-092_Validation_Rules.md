...
**Problem**: Automated detection of validation rules in CI workflows.
...
Validation Rules failures typically result from:
...
```python
def detect_rp_092(log_text: str) -> bool:
...
```python
def analyze_rp_092(context: str) -> Dict:
    return {
        "pattern": "RP-092",
        "confidence": 0.85,
        "category": "configuration-management",
