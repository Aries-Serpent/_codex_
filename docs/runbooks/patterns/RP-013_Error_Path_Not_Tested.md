...
**Problem**: Automated detection of error path not tested in CI workflows.
...
Error Path Not Tested failures typically result from:
...
```python
def detect_rp_013(log_text: str) -> bool:
    for keyword in ['error path', 'exception']:
...
```python
def analyze_rp_013(context: str) -> Dict:
    return {
        "pattern": "RP-013",
        "confidence": 0.84,
        "category": "test-coverage",
        "action": "remediate" if 0.84 >= 0.95 else "review"
    }
```
...
```
✅ RP-013 remediation applied
✅ Error Path Not Tested resolved
