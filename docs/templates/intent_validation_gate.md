...
2) PLAN OF ACTION (structured, reviewable):
 ...
 • Assumptions: List key assumptions (derived from my request, conventions, or your inference). Mark each as (confirmed), ? (uncertain), or (needs clarification).
...
Reply format (Markdown):
 ...
 - **Assumptions** (bulleted list with confidence markers: ? )
