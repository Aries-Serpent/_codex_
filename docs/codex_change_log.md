...
| 2025-12-01T00:45:00Z | 2 | 2.1 | ok | Command: python -c "import yaml, pathlib as p; data=yaml.safe_load(p.Path('codex_task_sequence.yaml').read_text()); assert 'codex_task_sequence' in data" | <!-- pragma: allowlist secret -->
...
| 2025-12-01T00:45:01Z | 2 | 2.5 | ok | Command: python tools/codex_yaml_gap_check.py --gaps codex_gap_registry.yaml --yaml codex_task_sequence.yaml --out codex_yaml_gap_report.md | <!-- pragma: allowlist secret -->
...
| 2025-12-01T00:45:11Z | 6 | 6.2 | ok | Command: python -c "import pathlib as p; assert p.Path('codex_secret_scan_report.json').exists()" | <!-- pragma: allowlist secret -->
