...
| Autonomy control | **ED transition gate** — 5-condition FSM (C1–C5) blocks promotion until all conditions met |
...
### 5. Monitor ED Transition Readiness

The **ED Transition Gate** checks 5 conditions on every PR:
...
| 4 | **ED Transition Gate** | `e-to-d-transition-gate.yml` | PR + push to main | `core.setFailed` when ED score < 5/5 |
...
| `scripts/ci/orchestrator_routing.py` | FAISScapability_tags agent routing |
...
|-------------|-------------|--------|
| **D_CAPABLE promotion workflow** | Create `d-capable-promotion.yml` that runs when ED gate reaches 5/5 consistently | Enables first agent transition from advisory (E) to autonomous (D) |
...
*Source: SoftGROUNDED conversion (Phases 0–6)*
