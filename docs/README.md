# _codex_ Documentation

**Last Updated:** 2026-06-22

> Comprehensive documentation for the _codex_ AI/ML platform

## 📚 Documentation Structure

This documentation is organized into several key sections:

### Getting Started
- [Quickstart Guide](QUICKSTART.md) - Get up and running quickly
- [Installation](setup/environment.md) - Detailed installation instructions
- [Contributing](https://github.com/Aries-Serpent/_codex_/blob/main/CONTRIBUTING.md) - How to contribute to the project

### User Guides
- [CLI Usage](CLI.md) - Command-line interface guide
- [API Reference](API_REFERENCE.md) - Complete API documentation
- [Configuration](configs/OmegaConf_Schema.md) - Configuration management

### Architecture & Design
- [Architecture Overview](./ARCHITECTURE.md) - System architecture
- [Design Decisions](adr/000-mcp-architecture.md) - Architecture decision records
- [Module Structure](modules/cli.md) - Code organization

### Administration
- [Admin Guide](admin/INDEX.md) - Administration and operations
- [Security](https://github.com/Aries-Serpent/_codex_/blob/main/SECURITY.md) - Security policies and procedures
- [Governance](admin/GOVERNANCE.md) - Project governance

### Development
- [Development Guide](development/modernization_guide.md) - Developer documentation
- [Testing](TESTING.md) - Testing strategy and guidelines
- [CI/CD](development/ci_optimization_guide.md) - Continuous integration/deployment

### Operational Templates
- [Template Index](templates/README.md) - Reusable operational templates
- **Migration — Python File Relocation** - Standardized approach for relocating Python modules
- **Migration — CLI Hardening** - Template for improving CLI robustness
- **Planning — Intent Validation** - Framework for validating implementation intent

### Advanced Topics
- [RAG Integration](EXPANDED_CONTEXT_RAG.md) - Retrieval-augmented generation
- [Distributed Training](distributed_training_guide.md) - Multi-node training
- [Performance Optimization](PERFORMANCE_OPTIMIZATION_GUIDE.md)

## 📖 Documentation Indexes

- [Master Index](MASTER_INDEX.md) - Complete documentation index
- [Documentation Index](DOCUMENTATION_INDEX.md) - Organized by topic
- [Archive](archive/INDEX.md) - Archived documentation

## 🔍 Finding What You Need

### For New Users
1. Start with the [Quickstart Guide](QUICKSTART.md)
2. Review [Architecture Overview](./ARCHITECTURE.md)
3. Explore [User Guides](#user-guides)

### For Contributors
1. Read [Contributing Guidelines](https://github.com/Aries-Serpent/_codex_/blob/main/CONTRIBUTING.md)
2. Review [Development Guide](development/modernization_guide.md)
3. Check [Testing Guidelines](TESTING.md)

### For Administrators
1. Review [Admin Guide](admin/INDEX.md)
2. Check [Security Policies](https://github.com/Aries-Serpent/_codex_/blob/main/SECURITY.md)
3. Review [Operations Documentation](ops/)

## 🔗 External Resources

- [GitHub Repository](https://github.com/Aries-Serpent/_codex_)
- [Issue Tracker](https://github.com/Aries-Serpent/_codex_/issues)
- [Discussions](https://github.com/Aries-Serpent/_codex_/discussions)

## 📝 Documentation Maintenance

This documentation is actively maintained. For:
- **Errors or outdated info**: Open an issue
- **Missing documentation**: Check [Documentation Gap Analysis](DOCUMENTATION_GAP_ANALYSIS.json)
- **Contributions**: Follow the [Documentation Standards](CODE_REVIEW_STANDARDS.md)

---

**Last Updated**: 2026-01-26
**Maintained By**: _codex_ team and autonomous agents
