# Codex Remediation Task Queue – Phase 1 Execution Packet (2024-09-17)
...
- **Source references**: Phase 1/U1 in remediation plan; outstanding questions table entries dated 2024-09-13 and 2024-09-17 concerning missing `pre-commit` gating.
...
- **Preconditions**: Access to `configs/development/noxfile.py`, `requirements/dev.txt`, coverage configuration, and test infrastructure.
- **Actions**:
  1. Verify/install `pytest-cov` and related plugins within the testing environment manifest.
  2. Update the `tests` session in `configs/development/noxfile.py` to run pytest with `--cov=src/codex_ml --cov-report=term-missing --cov-fail-under=<target>` (align target with remediation plan suggestions).
...
- **Source references**: Phase 1/U5; outstanding question entry dated 2024-09-13 for `ModuleNotFoundError: torch`.
