...
| W-113 | CI fix: `.secrets.baseline` stale line numbers + CodeQL `javascript` no-code failure (PR #3497, 2026-03-05): **(W-113a)** `.secrets.baseline` line numbers for `agent-auth-delegation.yml` shifted after main merge (c0a71f3) — REQ-8 base64 token moved from line 561→566 (new hash `31a7aa9c...`) and REQ-9 token moved from line 592→604 (new hash `c99b53af...`); updated both entries. `CODEX_MANIFEST.json` entry also refreshed (line 1653, hash `f88d271f...`). **(W-113b)** `codeql-analysis.yml`: reverted `config-file: .codeql/codeql-config.yml` (broke Go analysis — Go code lives in `tools/github-secrets-cli/`, excluded by config path restriction); restored `queries: +security-extended`; added `continue-on-error: ${{ matrix.language == 'javascript' }}` at job level so the JS matrix leg (no JS source files in repo) no longer blocks the workflow while python and go remain required. **(W-113c)** `chatops_copilot_trigger.yml` `timeout-minutes: 30→60` and `tests/security/test_no_hardcoded_secrets.py` pragma suppressor from commit 52afd2d preserved. | ✅ Done (PR #3497) |
