...
| W-100 | Lint fix: `tests/ci/test_auto_promote_tier.py` ruff violations (PR #3494, run 22681530852): removed unused `import pytest` (F401); added `I001` to the `# noqa` comment on the `auto_promote_tier` import line — ruff isort flags it as out-of-order because it follows a `sys.path.insert()` call that is required before the import. Fixes Pre-Merge Validation gate. | ✅ Done (PR #3494) |
