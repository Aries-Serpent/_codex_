...
**Last updated:** 2026-03-06 (W-131 — CI failure sweep: `github-app-manager` registry `handoff_protocol` added; Ruff I001 unsorted imports fixed in `src/codex/auth/__init__.py` + `tests/server/test_webhook_endpoint.py`; pre-flight broad `pytest.raises` match patterns tightened in 5 test locations; `ubuntu-latest-m` added to `actionlint.yaml` self-hosted runner labels; `build-preview-image.yml` `SHORT_SHA` expression fixed)
