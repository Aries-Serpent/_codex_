...
| W-094 | Fix actionlint-audit.yml `ERROR_COUNT` double-zero bug (PR #3492): `grep -c` exits with code 1 on zero matches while still printing `"0"` — the `\|\| echo "0"` fallback then fires a second time producing `ERROR_COUNT="0\n0"`, which causes `Invalid format '0'` in `$GITHUB_OUTPUT` and `integer expression expected` in the `-gt 0` test. Fixed by replacing `\|\| echo "0"` with `2>/dev/null; true` inside the subshell so the exit code is absorbed and only one `"0"` is captured. actionlint scan itself was clean (0 errors across 96 files) — only the output-capture logic was broken. | ✅ Done (PR #3492) | (1) Updated `cognitive-brain-manager.md` v2.0→v3.0 — added RBAC + CI Health tracking subgraphs to Mermaid diagram, PR #3492 metrics table rows (COGNITIVE_BRAIN_ALLOWED_ACTORS active, CODEX_CI_LAST_GREEN_SHA wired, ZendeskAPIClient.update_user added), updated version history; (2) Updated `cognitive-brain-session-injector.md` v1.0→v1.1 — flipped `COGNITIVE_BRAIN_ALLOWED_ACTORS` from ⚠️ pending to ✅ active (4 actors now set via repo variable); (3) Created `docs/cognitive_brain/status/COGNITIVE_BRAIN_STATUS_PR3492.md` with full session summary, all P2.x wiring status table (7/7 complete), repo variable state, and next-phase Mermaid flowchart; (4) Created `.codex/docs/FOLLOWUP_PROMPT_PR3492.md` with P3.x enhancement wiring tasks, D_CAPABLE promotion guide, and session self-review checklist for next session. | ✅ Done (PR #3492) |
