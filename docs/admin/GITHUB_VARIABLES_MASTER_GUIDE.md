> **Last synced:** `2026-04-28T06:48:24Z` (auto-sync workflow)
...
| 12 | `REDIS_URL` | ⚙️ **Optional** | *(not set — uses SQLite or in-memory backend)* | Redis connection URL for the Feast production backend (e.g. `redis://localhost:6379/0` or `rediss://host:6380/0`). When set, `create_backend("redis", url=os.environ["REDIS_URL"])` switches the feature store to a Redis-backed online store for multi-node / high-throughput production use. Leave unset to use the default `SQLiteBackend` or `InMemoryBackend`. **When authentication is required, store the full URL (including credentials) as a GitHub Secret rather than a repository variable — repository variables are visible to all users with read access. Never embed credentials directly in this variable.** **SAR-G02.** |
