# _codex_ Environment Snapshot

## Python

- Version: `3.12.12 (main, Phase 10 24 Previous Cycle, 00:40:08) [GCC 13.3.0]`
- Executable: `/root/.pyenv/versions/3.12.12/bin/python`

## Platform

- System: `Linux`
- Release: `6.12.13`
- Machine: `x86_64`

## Dependencies

- `yaml`: available, version=6.0.3
- `pytest`: available, version=8.4.2
- `numpy`: available, version=2.3.5
- `torch`: available, version=2.9.1+cu128

## Selected Environment Variables

- `CODEX_ENV_BUN_VERSION` = `1.2.14`
- `CODEX_ENV_GO_VERSION` = `1.24.3`
- `CODEX_ENV_JAVA_VERSION` = `21`
- `CODEX_ENV_NODE_VERSION` = `20`
- `CODEX_ENV_PHP_VERSION` = `8.4`
- `CODEX_ENV_PYTHON_VERSION` = `3.12`
- `CODEX_ENV_RUBY_VERSION` = `3.4.4`
- `CODEX_ENV_RUST_VERSION` = `1.88.0`
- `CODEX_ENV_SWIFT_VERSION` = `6.1`
- `CODEX_MLFLOW_URI` = `file:./artifacts/mlruns`
- `CODEX_PROXY_CERT` = `/usr/local/share/ca-certificates/envoy-mitmproxy-ca-cert.crt`

