# RAG Pipeline API
**Last Updated:** 2026-07-11
**Version:** v0.2.1

**Last Updated: 2026-06-22

**Version:** 1.0.0 | **Release Date:** 2026-06-22

For detailed RAG Pipeline documentation, see [RAG Pipelines](rag_pipelines.md).

## Quick Links

- [RAG Pipelines Documentation](rag_pipelines.md) - Complete RAG pipeline API reference
- [API Index](index.md) - Main API documentation index

## Overview

The RAG (Retrieval Augmented Generation) pipeline provides:

- Document ingestion and indexing
- Semantic search and retrieval
- Context-aware response generation
- Multi-tenant support
- Caching and performance optimization

For implementation details, configuration, and usage examples, refer to the [RAG Pipelines documentation](rag_pipelines.md).
