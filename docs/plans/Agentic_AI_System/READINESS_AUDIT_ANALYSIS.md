...
AGENT_REGISTRY.yaml v0.2.1 contains 152 agents with enforcement fields, CODEX_MANIFEST.json
...
|--------|----------------|-------------|
| Agent definitions | 193 agents | 372 `.md` files across `.github/agents/**`; **152 registered in `.github/agents/AGENT_REGISTRY.yaml` v0.2.1** |
...
The canonical active count is **152 agents** registered in AGENT_REGISTRY.yaml v0.2.1.
...
|------|--------------|----------------|
| `AGENT_REGISTRY.yaml` (plan schema) | Phases 1–6 | `.github/agents/AGENT_REGISTRY.yaml` v0.2.1 — extended in-place with 152 agents, all enforcement fields |
...
|---|----------------|--------|----------|--------|
| G-1 | Confirm active agent count (372 files; 128 in AGENT_REGISTRY.yaml reconcile with doc's 193) | Done — 152 agents in v0.2.1 | **P0** | Done |
...
| G-10 | Assess `.github/agents/AGENT_REGISTRY.yaml` (v0.2.1, 128 agents) and determine extend vs. migrate strategy for Phase 1 | Done — Option A (extend in-place) chosen. Now v0.2.1 with 152 agents | **P0** | Done |
...
| D2 | **Agent Ecosystem Maturity** | 20 | 20 | 20 | 152 agents in AGENT_REGISTRY.yaml v0.2.1; GROUNDED=8, PARTIAL=144, SOFT=0; all enforcement fields populated |
...
- `.github/agents/AGENT_REGISTRY.yaml` (128 formally registered agents, v0.2.1)
...
| Phase 1 | 4 | 3–4 | COMPLETE | Registry v0.2.1, manifest, generate_manifest.py, schemas |
