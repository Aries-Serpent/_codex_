...
- [ DOMAIN 1 — Agent Registry YAML Schema Patterns](#-domain-1--agent-registry-yaml-schema-patterns)
...
- [ DOMAIN 5 — ED Tiered Autonomy Architecture](#-domain-5--ed-tiered-autonomy-architecture)
 ...
 - [ED Transition: FSM State Diagram](#ed-transition-fsm-state-diagram)
...
- [Phase 4: ED Transition Readiness Gate](#phase-4-ed-transition-readiness-gate)
...
- [Dynamic Routing for OrchestratorSpecialist Selection (Phase 4)](#dynamic-routing-for-orchestratorspecialist-selection-phase-4)
...
- [ DOMAIN 8 — CODEX_MANIFEST Security & Runtime Injection Hardening](#-domain-8--codex_manifest-security--runtime-injection-hardening)
 ...
 - [ Critical New Finding: Prompt/Context Injection is the #1 Risk](#-critical-new-finding-promptcontext-injection-is-the-1-risk)
...
- [ SECTION 1 — Integrated Implementation Guide](#-section-1--integrated-implementation-guide)
...
- [ ED operating model status ](#-ed-operating-model-status-)
...
- [ SECTION 2 — Revised Risk Table](#-section-2--revised-risk-table)
...
- [Task 4 — ED Transition Map](#task-4--ed-transition-map)
...
- [ SECTION 3 — Physics-Inspired Formulas: Domains 5–8](#-section-3--physics-inspired-formulas-domains-58)
...
- [20. ED Transition — Finite State Machine](#20--ed-transition--finite-state-machine)
- [21. Tiered Role Taxonomy — Potential Energy Ladder](#21--tiered-role-taxonomy--potential-energy-ladder)
...
## DOMAIN 1 — Agent Registry YAML Schema Patterns
### (Phase 1 Validation)
...
|-------|--------|---------|
| `role: specialist\|utility\|orchestrator` | Microsoft Multi-Agent Reference Architecture [[5]](https://microsoft.github.io/multi-agent-reference-architecture/docs/context-engineering/Agents-Orchestration.html) | Enables orchestratorspecialist routing |
| `autonomy_model: E\|D_CAPABLE` | Plan Q1 answer + tiered autonomy research | Gates ED transition per agent |
...
> - Domain 5: ED Tiered Autonomy Architecture (Phase 4 validation)
...
5. **ED Tiered Autonomy Architecture** Phase 4 validation
...
## DOMAIN 5 — ED Tiered Autonomy Architecture
### (Phase 4 Validation)
...
This maps the ED transition directly to a **state machine** where
...
### Critical New Finding: Prompt/Context Injection is the #1 Risk
...
|-------|-----------|----------------|-------------|
| Phase 4 | ED FSM transition | Validated — Agentic Lybic arXiv paper | FSM is the correct model; 5-condition gate is correct |
...
| ALL | CODEX_MANIFEST security | **NEW RISK** — arXiv + CVE-2025-55319/61260 | Context injection is #1 new attack surface; integrity hash + field allowlist required |
...
## SECTION 1 — Integrated Implementation Guide
...
|---|------|----------|-------|-----------|---------|
| R-01 | Premature ED activation before gates ready | High | 4 | `e-to-d-transition-gate.yml` 5-condition Tier-1 block | `AUTONOMY_MODEL=E` repo var |
...
| **Model D** | Target operating model: orchestratorspecialist delegation without human approval | 2,3 | D5 |
| **D_CAPABLE** | `autonomy_model` field value in registry indicating agent is eligible for D-model | 2,3 | D5 |
| **ED Transition Gate** | 5-condition FSM check before `AUTONOMY_MODEL=D_CAPABLE` is activated | 2,3 | D5 |
...
| D8: Security Hardening | 2,3 | All | CVE-2025-55319/61260 — NEW RISK | `generate_manifest.py` security additions |
...
## SECTION 3 — Physics-Inspired Formulas: Domains 5–8
...
| **Chunk 2** | D5–D8 | ED FSM gate, Self-healing CI patterns, workflow_compliance_scan.py, CODEX_MANIFEST security |
