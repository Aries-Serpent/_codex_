...
6. **Finding 3 — Compile regex once**: `compile_replacements()` pre-compiles patterns (list comprehension); `replace_in_file()` accepts compiled list; `update_references()` compiles once before traversal.
