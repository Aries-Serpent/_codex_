...
| Optional high-cost startup failures | | zero-job `startup_failure` states (infra/startup-level, non-code) |
...
- ℹ Startup-failure runs reported in optional heavy suites (`data-quality-suite`,
...
- CodeQL fetcher rerun currently pending due to temporary GitHub API rate-limit
...
|-------|----------|--------|
| Optional high-cost suites with `startup_failure` + zero jobs | infra | monitor; non-code failure mode |
