...
if grep -E "_DEV.*_PRODUCTION|_STAGING.*_PRODUCTION" /tmp/secrets_audit.txt; then
 echo " Cross-environment secrets detected!"
 exit 1
else
...
def analyze_access_patterns():
 ...
 print(f" {len(failed_attempts)} failed access attempts")
 print(" Actors:")
 ...
 print(f" Unexpected access: {actor} to {secret}") # pragma: allowlist secret
...
if [ "$OLD_HASH" != "$CURRENT_HASH" ]; then
 ...
 echo " WARNING: Key hash unchanged"
fi
```
