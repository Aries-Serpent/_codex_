...
class ZendeskKnowledgeSyncService:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if subdomain:
            ...
            logger.info(f"Initialized ZendeskKnowledgeSyncService for {subdomain}")  # codeql[py/clear-text-logging-sensitive-data]
        else:
    ...
    def _load_cache(self) -> dict[str, ArticleMetadata]:
        ...
        if not self.api_index_path.exists():
            logger.info(f"No existing cache at {self.api_index_path}, starting fresh")  # codeql[py/clear-text-logging-sensitive-data]
            return {}
            ...
            if isinstance(data, dict) and "articles" in data:
                for url, meta_dict in data.get("articles", {}).items():
                    try:
                        ...
                        logger.warning(f"Invalid cache entry for {url}: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]

            logger.info(f"Loaded {len(cache)} cached articles from {self.api_index_path}")  # codeql[py/clear-text-logging-sensitive-data]
            return cache
        except (json.JSONDecodeError, OSError) as e:
            error_type = type(e).__name__
            logger.error("Failed to load cache: <ERROR_TYPE>, starting fresh")  # codeql[py/clear-text-logging-sensitive-data]
            return {}
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            logger.info(f"Saved cache with {len(self._cache)} articles to {self.api_index_path}")  # codeql[py/clear-text-logging-sensitive-data]
        except OSError as e:
            error_type = type(e).__name__
            logger.error("Failed to save cache: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    def _fetch(self, url: str) -> tuple[bytes, dict[str, str]]:
        ...
        for attempt in range(self.retries):
            try:
                ...
                if exc.code == 404:
                    logger.warning(f"Article not found (404): {url}")  # codeql[py/clear-text-logging-sensitive-data]
                    raise
    ...
    def _should_update(self, url: str, headers: dict[str, str]) -> bool:
        ...
        if url not in self._cache:
            logger.debug(f"Article not in cache: {url}")  # codeql[py/clear-text-logging-sensitive-data]
            return True
        ...
        if etag and cached.etag and etag == cached.etag:
            logger.debug(f"ETag match, skipping: {url}")  # codeql[py/clear-text-logging-sensitive-data]
            return False
        ...
        if last_modified and cached.last_modified:
            try:
                ...
                if last_modified <= cached.last_modified:
                    logger.debug(f"Not modified since last fetch: {url}")  # codeql[py/clear-text-logging-sensitive-data]
                    return False
        ...
        logger.debug(f"No cache hit or stale, will fetch: {url}")  # codeql[py/clear-text-logging-sensitive-data]
        return True
            ...
            for bucket, urls in buckets.items():
                ...
                for url in urls:
                    ...
                    try:
                        ...
                        if dry_run:
                            logger.info(f"[DRY-RUN] Would check: {section}/{bucket}: {url}")  # codeql[py/clear-text-logging-sensitive-data]
                            checked += 1
                            continue

                        # Fetch with headers for change detection
                        logger.info(f"Checking: {section}/{bucket}: {url}")  # codeql[py/clear-text-logging-sensitive-data]
                        ...
                        if not needs_update:
                            skipped += 1
                            logger.info(f"Skipped (up-to-date): {url}")  # codeql[py/clear-text-logging-sensitive-data]
                            continue
                        ...
                        if any(pii_flags.values()):
                            logger.warning(f"PII detected and scrubbed in {url}: {pii_flags}")  # codeql[py/clear-text-logging-sensitive-data]
                        ...
                        logger.info(f"Updated: {output_path}")  # codeql[py/clear-text-logging-sensitive-data]
                        ...
                        if e.code == 404:
                            logger.warning(f"Article not found (404), skipping: {url}")  # codeql[py/clear-text-logging-sensitive-data]
                            missing_articles.append(
                                {"url": url, "section": section, "bucket": bucket}
                            )
                            skipped += 1
                        else:
                            logger.error(f"HTTP error {e.code} syncing {url}: {e}")  # codeql[py/clear-text-logging-sensitive-data]
                            failed += 1
                    except (ConnectionError, TimeoutError) as e:
                        error_type = type(e).__name__
                        logger.error(f"Failed to sync {url}: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
                        failed += 1
    ...
    def check_and_pull_incremental(
        ...
        *,
        ...
        if since is None:
            ...
            if since is None:
                logger.warning("No previous sync found, performing full sync")  # codeql[py/clear-text-logging-sensitive-data]
                return self.check_and_pull(dry_run=dry_run, force=False)

        logger.info(f"Starting incremental sync from {since}")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        if not zendesk_url:
            logger.error("Could not determine Zendesk URL for API access")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        while api_url:
            try:
                logger.info(f"Fetching page {page_num} from {api_url}")  # codeql[py/clear-text-logging-sensitive-data]
                ...
                for article in articles:
                    ...
                    if dry_run:
                        logger.info(f"[DRY-RUN] Would sync article {article_id}: {title}")  # codeql[py/clear-text-logging-sensitive-data]
                        continue

                    # PII Scrubbing (MANDATORY)
                    scrubbed_body, pii_flags = scrub_pii(body)
                    if any(pii_flags.values()):
                        logger.warning(f"PII detected in article {article_id}: {pii_flags}")  # codeql[py/clear-text-logging-sensitive-data]
                    ...
                    updated += 1
                    logger.info(f"Updated article {article_id}: {output_path}")  # codeql[py/clear-text-logging-sensitive-data]
                ...
                if e.code == 404:
                    logger.warning(f"API endpoint not found (404): {paginated_url}")  # codeql[py/clear-text-logging-sensitive-data]
                    missing_articles.append({"url": paginated_url, "page": page_num})
                    break
                logger.error(f"HTTP error {e.code} fetching page {page_num}: {e}")  # codeql[py/clear-text-logging-sensitive-data]
                failed += len(articles) if "articles" in locals() else 0
                break
            except (ConnectionError, TimeoutError) as e:
                error_type = type(e).__name__
                logger.error(f"Failed to fetch page {page_num}: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
                failed += len(articles) if "articles" in locals() else 0
                break
    ...
    def _export_json_dataset(self, source_dir: Path) -> Path:
        ...
        for html_file in source_dir.rglob("*.html"):
            try:
                ...
                error_type = type(e).__name__
                logger.warning(f"Failed to process {html_file}: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        logger.info(f"Exported {len(articles)} articles to {dataset_path}")  # codeql[py/clear-text-logging-sensitive-data]
        return dataset_path
    ...
    def pipeline_to_codex_digest(self, source_dir: Path | None = None) -> dict[str, Any]:
        ...
        logger.info(f"Pipelining content from {source_dir} to codex_digest")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        logger.info(f"Pipeline preparation complete: {len(html_files)} files ready")  # codeql[py/clear-text-logging-sensitive-data]
        return result
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "incremental":
            logger.info("Running incremental sync (changes only)")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        print(f"Total Articles:    {result.total_articles}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"Checked:           {result.checked}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"Updated:           {result.updated}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"Failed:            {result.failed}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"Skipped:           {result.skipped}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"Timestamp:         {result.timestamp}")  # codeql[py/clear-text-logging-sensitive-data]
        if result.dataset_path:
            print(f"JSON Dataset:      {result.dataset_path}")  # codeql[py/clear-text-logging-sensitive-data]
        print(f"{'=' * 60}\n")  # codeql[py/clear-text-logging-sensitive-data]

        # Pipeline if requested
        if args.pipeline and not args.dry_run and result.updated > 0:
            logger.info("Starting pipeline to codex_digest...")  # codeql[py/clear-text-logging-sensitive-data]
            pipeline_result = service.pipeline_to_codex_digest()
            print(f"Pipeline Result: {json.dumps(pipeline_result, indent=2)}")  # codeql[py/clear-text-logging-sensitive-data]

        return 0 if result.failed == 0 else 1

    except (ValueError, TypeError) as e:
        error_type = type(e).__name__
        logger.error("Sync failed: <ERROR_TYPE>", exc_info=True)  # codeql[py/clear-text-logging-sensitive-data]
        return 2
