...
try:  # pragma: no cover - MLflow is optional for offline smoke tests
    ...
    OSError,
    ModuleNotFoundError,
):  # pragma: no cover - guard offline runs that skip mlflow install
    mlflow = None
...
try:  # pragma: no cover - optional GPU metrics dependency
    ...
    RuntimeError,
    ValueError,
):  # pragma: no cover - allow execution without NVML bindings
    pynvml = None
...
def _create_tensorboard_writer(log_dir: str | Path) -> SummaryWriter | None:
    ...
    try:
        ...
        ModuleNotFoundError,
        ImportError,
    ) as exc:  # pragma: no cover - propagate context
    ...
    try:
        ...
        ModuleNotFoundError,
        ImportError,
    ) as exc:  # pragma: no cover - e.g. tensorboard not installed
...
def _start_mlflow_run(config: LoggingConfig) -> bool:
    ...
    try:
        ...
        ModuleNotFoundError,
        ImportError,
    ) as exc:  # pragma: no cover - offline guard
...
def _create_fallback_writer(config: LoggingConfig) -> FallbackMetricsWriter | None:
    ...
    try:
        ...
        ModuleNotFoundError,
        ImportError,
    ) as exc:  # pragma: no cover - best-effort fallback
        LOGGER.debug(
...
def log_scalar_tb(writer: SummaryWriter | None, tag: str, value: float, step: int) -> None:
    ...
    try:
        ...
        ModuleNotFoundError,
        ImportError,
    ):  # pragma: no cover - robustness guard
...
def shutdown_logging(session: LoggingSession) -> None:
    ...
    if session.tensorboard is not None:
        try:
            ...
            ModuleNotFoundError,
            ImportError,
        ) as exc:  # pragma: no cover - flush errors should not raise
