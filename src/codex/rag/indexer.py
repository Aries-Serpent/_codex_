...
def chunk_text(text: str, chunk_size: int = 1000, overlap: int = 128) -> list[tuple[int, int, str]]:
    ...
    logger.debug(f"Created {len(chunks)} chunks from {text_len} characters")  # codeql[py/clear-text-logging-sensitive-data]
    return chunks
...
def embed_chunks(
    ...
    logger.info(f"Loading embedding model: {model_name}")  # codeql[py/clear-text-logging-sensitive-data]
    try:
        ...
        logger.error("Failed to load embedding model: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise
    ...
    logger.debug(f"Encoding {len(texts_filtered)} texts, first sample: {texts_filtered[0][:100]}")  # codeql[py/clear-text-logging-sensitive-data]

    # Generate embeddings with explicit device parameter and detailed error handling
    logger.info(f"Generating embeddings for {len(texts_filtered)} chunks")  # codeql[py/clear-text-logging-sensitive-data]
    try:
        ...
        error_type = type(e).__name__
        logger.error("IndexError during encoding: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        logger.error(f"Texts count: {len(texts_filtered)}")  # codeql[py/clear-text-logging-sensitive-data]
        logger.error(f"Sample texts: {texts_filtered[:3] if texts_filtered else 'EMPTY'}")  # codeql[py/clear-text-logging-sensitive-data]
        logger.error(f"Model info: {model}")  # codeql[py/clear-text-logging-sensitive-data]
        logger.error(f"Model max_seq_length: {getattr(model, 'max_seq_length', 'NOT SET')}")  # codeql[py/clear-text-logging-sensitive-data]
        raise RuntimeError(
...
def persist_index(
    ...
    if faiss is None:
        logger.error("faiss-cpu not installed. Install with: pip install faiss-cpu")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    logger.info(f"Building FAISS index with dimension {dimension}")  # codeql[py/clear-text-logging-sensitive-data]

    # Use IndexFlatL2 for exact search (can be upgraded to IndexIVFFlat for larger datasets)
    index = faiss.IndexFlatL2(dimension)
    index.add(embeddings.astype(np.float32))

    logger.info(f"Added {index.ntotal} vectors to FAISS index")  # codeql[py/clear-text-logging-sensitive-data]

    # Save index
    faiss_file = index_path / "index.faiss"
    faiss.write_index(index, str(faiss_file))
    logger.info(f"Saved FAISS index to {faiss_file}")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    logger.info(f"Saved chunks metadata to {chunks_file}")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    logger.info(f"Saved index metadata to {metadata_file}")  # codeql[py/clear-text-logging-sensitive-data]

    logger.info(f"✅ Index '{index_name}' persisted to {index_path}")  # codeql[py/clear-text-logging-sensitive-data]
    return index_path
...
def load_index(
    ...
    logger.info(f"Loaded FAISS index with {index.ntotal} vectors")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    logger.info(f"✅ Loaded index '{index_name}' from {index_path}")  # codeql[py/clear-text-logging-sensitive-data]
...
def build_index_from_files(
    ...
    for file_path in files:
        if not file_path.exists():
            logger.warning(f"File not found: {file_path}")  # codeql[py/clear-text-logging-sensitive-data]
            continue
        ...
        try:
            ...
            logger.info(f"Processed {file_path}: {len(chunks)} chunks")  # codeql[py/clear-text-logging-sensitive-data]
            ...
            error_type = type(e).__name__
            logger.error(f"Error processing {file_path}: {error_type}")  # codeql[py/clear-text-logging-sensitive-data]
            processing_errors.append(str(file_path))
    ...
    logger.info(f"Total chunks: {len(all_chunks)} from {len(files)} files")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    **kwargs,
    ...
    if op_enum == IndexOperation.CREATE:
        ...
        for index_name in index_names:
            try:
                ...
                logger.info(f"Created index '{index_name}' at {index_path}")  # codeql[py/clear-text-logging-sensitive-data]
            except (IOError, OSError) as e:
                error_type = type(e).__name__
                logger.error(f"Failed to create index '{index_name}': <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        for index_name in index_names:
            try:
                ...
                if old_path.exists():
                    ...
                    logger.info(f"Removed old index '{index_name}'")  # codeql[py/clear-text-logging-sensitive-data]
                ...
                logger.info(f"Updated index '{index_name}' at {index_path}")  # codeql[py/clear-text-logging-sensitive-data]
            except (IOError, OSError) as e:
                error_type = type(e).__name__
                logger.error(f"Failed to update index '{index_name}': <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        for index_name in index_names:
            try:
                ...
                if index_path.exists():
                    ...
                    logger.info(f"Deleted index '{index_name}' from {tenant_dir}")  # codeql[py/clear-text-logging-sensitive-data]
                else:
                    logger.warning(f"Index '{index_name}' not found for tenant '{tenant_id}'")  # codeql[py/clear-text-logging-sensitive-data]
            except (IOError, OSError) as e:
                error_type = type(e).__name__
                logger.error(f"Failed to delete index '{index_name}': <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        try:
            ...
            for index_name in index_names:
                try:
                    ...
                    logger.info(f"Loaded {index.ntotal} vectors from '{index_name}'")  # codeql[py/clear-text-logging-sensitive-data]
                except (ValueError, TypeError, RuntimeError, IOError, OSError) as e:
                    error_type = type(e).__name__
                    logger.error(f"Failed to load index '{index_name}': {error_type}: {str(e)}")  # codeql[py/clear-text-logging-sensitive-data]
            ...
            error_type = type(e).__name__
            logger.error("Merge operation failed: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
            return TenantOperationResult(
                success=False,
                operation=op_enum,
                tenant_id=tenant_id,
    ...
    # LIST: List all indices for tenant
        ...
        try:
            ...
            error_type = type(e).__name__
            logger.error("List operation failed: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
            return TenantOperationResult(
                success=False,
                operation=op_enum,
                tenant_id=tenant_id,
