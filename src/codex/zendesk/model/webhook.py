...
class Webhook(BaseModel):
    ...
    def diff(self, other: "Webhook") -> list[dict[str, Any]]:
        ...
        if self.endpoint != other.endpoint:
            patches.append(
                {"op": "replace", "path": "/endpoint", "value": self.endpoint}
            )
        if self.http_method != other.http_method:
            patches.append(
                {"op": "replace", "path": "/http_method", "value": self.http_method}
            )
        ...
        if self.subscriptions != other.subscriptions:
            patches.append(
                {"op": "replace", "path": "/subscriptions", "value": self.subscriptions}
            )
        if self.status != other.status:
            patches.append(
                {"op": "replace", "path": "/status", "value": self.status}
            )
        return patches
