...
class PatternGraph:
    ...
    def get_related_patterns(self, pattern_id: str, 
                            depth: int = 1) -> List[PatternNode]:
        """
    ...
    def find_causal_chains(self, pattern_id: str, 
    ...
    def compute_graph_metrics(self) -> Dict[str, Any]:
        ...
        # Average degree
        degrees = [len(self.adjacency.get(nid, [])) + len(self.reverse_adjacency.get(nid, []))
        ...
        return {
            ...
            "avg_node_confidence": sum(n.confidence for n in self.nodes.values()) / num_nodes if num_nodes > 0 else 0,
            "avg_node_frequency": sum(n.frequency for n in self.nodes.values()) / num_nodes if num_nodes > 0 else 0,
        }
    ...
    def export_graphml(self) -> str:
        ...
        lines.append('    <key id="success_rate" for="node" attr.name="success_rate" attr.type="double"/>')
        
        # Define edge attributes
        lines.append('    <key id="relationship" for="edge" attr.name="relationship" attr.type="string"/>')
    ...
    def add_patterns(self, patterns: List[Any]) -> None:
        ...
        for pattern in patterns:
            node = PatternNode(
                ...
                created_at=pattern.first_seen if hasattr(pattern, 'first_seen') else datetime.now(timezone.utc),
            )
