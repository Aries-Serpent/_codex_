...
class MemoryConsolidationEngine:
    ...
    def _observe(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Query STM stats
            stm_stats = conn.execute(
                "SELECT COUNT(*) as count FROM stm_entries"
            ).fetchone()
            
            # Query LTM stats
            ltm_stats = conn.execute(
                "SELECT COUNT(*) as count FROM ltm_entries"
            ).fetchone()
    ...
    def _decide(self, hot_entries: List[PatternEntry], 
                cold_entries: List[Tuple[str, float]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for entry in plan["promote"]:
                ...
                conn.execute(
                    ...
                    """,
                    (
                        entry.key, entry.value, entry.pattern_type.value,
                        entry.frequency, entry.success_rate, confidence,
            ...
            for key in plan["prune"]:
                ...
                conn.execute(
                    "INSERT INTO ltm_archive SELECT * FROM ltm_entries WHERE key = ?",
                    (key,)
                )
    ...
    def _determine_retention_policy(self, entry: PatternEntry, 
                                   confidence: float) -> RetentionPolicy:
    ...
    def _analyze(self, state_before: Dict[str, Any], state_after: Dict[str, Any],
                promoted: int, pruned: int, duration: float) -> ConsolidationMetrics:
