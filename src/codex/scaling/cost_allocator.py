...
class CostReport:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "tenant_costs": {t_id: cost.to_dict() for t_id, cost in self.tenant_costs.items()},
            "total_cost": self.total_cost,
...
class CostAllocator:
    ...
    def add_reserved_instance(
        self, instance_id: str, instance_class: str, quantity: int, hours_remaining: float
    ) -> None:
        ...
        if cost.on_demand_cost > cost.reserved_cost * 0.5:
            ...
            recommendations.append(
                CostOptimizationRecommendation(
                    ...
                    savings_percentage=(savings / cost.total_cost * 100)
        ...
        if cost.on_demand_cost > 0:
            ...
            recommendations.append(
                CostOptimizationRecommendation(
                    ...
                    savings_percentage=(spot_potential / cost.total_cost * 100)
                    if cost.total_cost > 0
                    else 0,
        ...
        if cost.storage_cost > cost.total_cost * 0.15:
            ...
            recommendations.append(
                CostOptimizationRecommendation(
                    ...
                    savings_percentage=(storage_savings / cost.total_cost * 100)
                    if cost.total_cost > 0
                    else 0,
    ...
    def calculate_ri_utilization(self) -> float:
        ...
        total_ri_hours = sum(ri["hours_remaining"] for ri in self.ri_inventory.values())

        if total_ri_hours == 0:
            return 0.0

        # Calculate actual usage
        total_reserved_usage = sum(usage["reserved_hours"] for usage in self.tenant_usage.values())
    ...
    def generate_monthly_report(self, report_month: str) -> CostReport:
        ...
        estimated_savings = sum(r.potential_savings for r in all_recommendations)
