...
class RBACEngine:
    ...
    def grant_role(
        self, user_id: str, tenant_id: str, role_type: RoleType, granted_by: str = "system"
    ) -> bool:
        """
        ...
        self.user_roles[key] = UserRole(
            user_id=user_id, tenant_id=tenant_id, role_type=role_type, granted_by=granted_by
        )

        self._audit_permission_change("role_grant", user_id, tenant_id, role_type.value, granted_by)
        return True
    ...
    def revoke_role(self, user_id: str, tenant_id: str) -> bool:
        ...
        self._audit_permission_change(
            "role_revoke", user_id, tenant_id, self.user_roles[key].role_type.value, "system"
        )
        del self.user_roles[key]
        return True

    def check_permission(
        self, user_id: str, tenant_id: str, resource_type: ResourceType, action: str
    ) -> Tuple[bool, str]:
        """
        ...
        denials = [
            ...
            if (p.resource_type == resource_type or p.resource_type == ResourceType.ALL)
            and p.action == action
            and p.access_level == AccessLevel.DENY
        ]
        if denials:
        ...
        permission_level = max((g.access_level for g in grants), default=AccessLevel.NONE)
    ...
    def add_custom_permission_grant(
        self,
        user_id: str,
        tenant_id: str,
        permission: Permission,
    ...
    def validate_permission_matrix(self) -> Dict[str, any]:
        ...
        # Check hierarchy
        hierarchy_valid = all(r.parent_role is None or r.parent_role in self.roles for r in roles)

        # Check for unintended grants (wildcard + specific resource)
        wildcard_grants = sum(
            sum(1 for p in r.permissions if p.resource_type == ResourceType.ALL) for r in roles
        )
        ...
        report = {
            ...
            "coverage_percent": 100.0 * total_permissions / total_combinations
    ...
    def _audit_permission_change(
        self, change_type: str, user_id: str, tenant_id: str, detail: str, actor: str
    ) -> None:
    ...
    def get_audit_log(self, tenant_id: Optional[str] = None, limit: int = 1000) -> List[Dict]:
