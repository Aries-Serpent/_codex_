...
class AuditTrail:
    ...
    def __init__(self, db_path: Optional[str] = None, retention_years: int = 7):
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_tenant_timestamp ON audit_events (tenant_id, timestamp)"  # noqa: E501
            )
            conn.execute("CREATE INDEX IF NOT EXISTS idx_event_type ON audit_events (event_type)")
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_resource ON audit_events (resource_type, resource_id)"  # noqa: E501
            )
    ...
    def enforce_retention_policy(self) -> Dict[str, any]:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                cursor = conn.execute(
                    "SELECT COUNT(*) FROM audit_events WHERE timestamp < ?", (cutoff_time,)
                )
    ...
    def export_events(
        ...
        events = self.query_events(
            tenant_id=tenant_id, start_time=start_time, end_time=end_time, limit=100000
        )
