...
class FailoverManager:
    ...
    def start_monitoring(self) -> None:
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="FailoverMonitor"
        )
    ...
    def _make_failover_decision(self) -> None:
        ...
        primary_healthy = (
            ...
            and self.region_states.get(self.current_primary) == HealthStatus.HEALTHY
        )
    ...
    def _execute_failover(self, from_region: Optional[str], to_region: str) -> bool:
        """
    ...
    def get_failover_history(self, limit: int = 100) -> List[Dict]:
        ...
        events = sorted(self.failover_events, key=lambda x: x.timestamp, reverse=True)[:limit]
    ...
    def verify_failover_capability(self) -> Dict[str, any]:
        ...
        """
        healthy_regions = sum(
            1 for status in self.region_states.values() if status == HealthStatus.HEALTHY
        )

        avg_detection_time = 0.0
        if self.failover_events:
            avg_detection_time = sum(e.detection_time_ms for e in self.failover_events[-10:]) / min(
                10, len(self.failover_events)
            )
