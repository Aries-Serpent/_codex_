...
class BottleneckAlert:
    ...
    def __post_init__(self):
        ...
        self.risk_score = self.confidence * severity_weights.get(self.severity, 0.5) * days_factor
...
class BottleneckPredictor:
    ...
    def analyze_cascading(self, alerts: List[BottleneckAlert]) -> CascadingAnalysis:
        ...
        if cascading:
            time_to_cascade = (
                cascading[0].predicted_saturation_date - first.predicted_saturation_date
            ).days
        else:
    ...
    def analyze_pareto_frontier(
        ...
        # Normalize
        norm_capacities = (capacities - np.min(capacities)) / (
            np.max(capacities) - np.min(capacities) + 1e-6
        )
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    ...
                    if norm_capacities[j] > norm_capacities[i] and norm_costs[j] < norm_costs[i]:
                        pareto_mask[i] = False
                        break
        ...
        if total_allocation > 0:
            recommended_allocation = {
                r: (v / total_allocation) * 100 for r, v in recommended_allocation.items()
            }
...
class CapexRecommendationEngine:
    ...
    def _recommend_for_resource(
        ...
        return {
            ...
            "roi_months": self._calculate_roi(new_cost - new_cost_optimized, current_cost),
        }
