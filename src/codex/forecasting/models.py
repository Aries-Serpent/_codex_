...
class ProphetModel(BaseForecaster):
    ...
    def _detect_changepoints(
        self, t: np.ndarray, y: np.ndarray, num_changepoints: int = 3
    ) -> np.ndarray:
        ...
        rolling_var = np.convolve((y - np.mean(y)) ** 2, np.ones(window) / window, mode="valid")
    ...
    def forecast(self, periods: int = 7) -> Tuple[np.ndarray, np.ndarray]:
        ...
        confidence = np.maximum(np.abs(forecasts) * 0.15, 1.96 * std_residuals)
    ...
    def forecast(self, steps: int = 7) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if np.abs(self.model_correlation) > self.config.max_correlation:
            ...
            if self.config.voting_strategy == "weighted_average":
                ...
                ensemble_forecast = np.median(np.array([arima_forecast, prophet_forecast]), axis=0)
            else:
    ...
    def get_model_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "diversity_acceptable": (
                self.model_correlation is None
                or np.abs(self.model_correlation) <= self.config.max_correlation
            ),
