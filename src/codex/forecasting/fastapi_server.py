...
class ForecastRequest(BaseModel):
    ...
    dimension: str = Field(
        ...,
        description="Infrastructure dimension (cpu, memory, storage, network, gpu, cache, database)",  # noqa: E501
    )
...
class CapexRequest(BaseModel):
    ...
    optimization_strategy: str = Field(
        default="moderate", pattern="^(aggressive|moderate|conservative)$"
    )
...
def create_app() -> Optional[FastAPI]:
    ...
    async def forecast_dimension(dimension: str, request: ForecastRequest) -> ForecastResponse:
        ...
        try:
            ...
            if len(ts_data) < 7:
                raise HTTPException(
                    status_code=400, detail="At least 7 historical data points required"
                )
            ...
            # Fit Prophet model
            dates = pd.date_range(end=datetime.now(), periods=len(ts_data), freq="D")
    ...
    async def detect_bottlenecks(request: BottleneckRequest) -> BottleneckResponse:
        ...
        try:
            ...
            for alert in alerts:
                ...
                alert_dict["predicted_saturation_date"] = (
                    alert.predicted_saturation_date.isoformat()
                )
            ...
            if len(alerts) > 1:
                try:
                    ...
                    cascading = {
                        ...
                        "cascading_sequence": [
                            a.resource for a in cascade_analysis.cascading_sequence
                        ],
                        "mitigation_urgency": cascade_analysis.mitigation_urgency,
                        "estimated_time_to_cascade_days": cascade_analysis.estimated_time_to_cascade,  # noqa: E501
    ...
    async def run_load_test(request: LoadTestRequest) -> LoadTestResult:
        ...
        try:
            ...
            np.random.seed(42)
            historical_data = (50 + np.arange(100) * 0.5 + np.random.normal(0, 3, 100)).tolist()
