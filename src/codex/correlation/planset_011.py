...
class GateCriterionValidator:
    ...
    def validate_criterion(
        self, criterion_num: int, actual_value: Any, target_value: Any = None
    ) -> GateCriterionResult:
...
class AnomalyDetectionAPI:
    ...
    def __init__(self, correlation_engine: AnomalyCorrelationEngine, max_latency_ms: int = 1000):
        """
        Initialize API.
    ...
    def correlate_anomalies(self, request: AnomalyDetectionRequest) -> AnomalyCorrelationResponse:
        """
        ...
        try:
            ...
            alert_reduction = self.engine.get_alert_reduction_percentage(
                len(request.anomalies), len(correlations)
            )
...
class EnsembleAnomalyDetector:
    ...
    def detect(self, metrics: Dict[str, float], baselines: Dict[str, float]) -> Tuple[bool, float]:
        """
...
class AnomalyCorrelationEngine:
    ...
    def get_alert_reduction_percentage(self, original_count: int, correlated_count: int) -> float:
        """Calculate alert reduction percentage"""
        if original_count == 0:
            return 0.0
        return (original_count - correlated_count) / original_count

    def update_graph_from_predictions(self, predictions: List[Dict[str, Any]]) -> bool:
...
class Planset011Orchestrator:
    ...
    def validate_all_gates(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Gate 2 (Root cause identification): {result2.passed} ({root_cause_accuracy:.1%})"
        )
...
class Planset012IntegrationAdapter:
    ...
    def prepare_feedback(root_causes: List[RootCauseChain]) -> Dict[str, Any]:
        ...
        return {
            ...
            "feedback_confidence": np.mean([rc.confidence for rc in root_causes])
            if root_causes
            else 0.0,
        }
