...
class RootCauseInference:
    ...
    def explanation(self) -> str:
        ...
        if self.causal_path:
            path_str = " → ".join(
                [link.source for link in self.causal_path.path] + [self.causal_path.path[-1].target]
                if self.causal_path.path
                else [self.root_cause]
            )
            return f"{self.root_cause} caused {self.anomaly_id} (confidence: {self.confidence:.1%}). Causal path: {path_str}"  # noqa: E501
        return (
            f"{self.root_cause} likely caused {self.anomaly_id} (confidence: {self.confidence:.1%})"
        )
...
class CausalGraph:
        ...
        for source, targets in self.INITIAL_STRUCTURE.items():
            ...
            for target in targets:
                ...
                self.links[key] = CausalLink(
                    source=source, target=target, conditional_probability=0.6
                )
                self.reverse_links[target].add(source)

    def add_link(self, source: str, target: str, probability: float = 0.5) -> None:
        ...
        if key in self.links:
            ...
            link.conditional_probability = (
                alpha * probability + (1 - alpha) * link.conditional_probability
            )
        else:
            link = CausalLink(source=source, target=target, conditional_probability=probability)
            self.links[key] = link

        self.reverse_links[target].add(source)

    def learn_from_correlation(self, source: str, target: str, success: bool = True) -> None:
    ...
    def get_downstream_effects(self, cause: str) -> List[str]:
        ...
        return [link.target for link in self.links.values() if link.source == cause]
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "learned_links": sum(1 for link in self.links.values() if link.learned_from_count > 0),
            "total_observations": sum(link.learned_from_count for link in self.links.values()),
        }
...
class BackwardChainer:
    ...
    def __init__(
        self, causal_graph: CausalGraph, max_depth: int = 5, confidence_threshold: float = 0.3
    ):
        """
    ...
    def find_root_causes(
        self, anomaly: str, lookback_anomalies: Optional[List[str]] = None
...
class RootCauseEngine:
    ...
    def __init__(self, max_depth: int = 5, confidence_threshold: float = 0.3):
        """Initialize root cause engine"""
        self.causal_graph = CausalGraph()
        self.chainer = BackwardChainer(
            self.causal_graph, max_depth=max_depth, confidence_threshold=confidence_threshold
        )

    def infer_root_cause(
        self, anomaly: str, related_anomalies: Optional[List[str]] = None
    ) -> Optional[RootCauseInference]:
        """
    ...
    def infer_multiple_causes(self, anomaly: str, top_n: int = 5) -> List[RootCauseInference]:
        """Get top N possible root causes"""
        return self.chainer.find_root_causes(anomaly)[:top_n]

    def learn_from_incident(self, root_cause: str, anomaly: str, success: bool = True) -> None:
