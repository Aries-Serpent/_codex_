...
SENSITIVE_PATTERNS = [
    ...
    (r'(?:secret|password|passwd|pwd)["\']?\s*[:=]\s*["\']?([^"\'\s,}]+)', r'\1: [REDACTED_SECRET]'),
    (r'(?:private[_-]?key|privatekey)["\']?\s*[:=]\s*["\']?([^"\'\s,}]+)', r'\1: [REDACTED_PRIVATE_KEY]'),
...
def redact_dict(data: dict, exclude_keys: Optional[list] = None) -> dict:
    """
    ...
    for key, value in data.items():
        if key in exclude_keys:
            ...
            result[key] = [
                redact_sensitive_data(v) if isinstance(v, str) else v
                for v in value
            ]
        else:
            result[key] = value
