    ...
    def load_baseline(self, baseline_file: Path) -> None:
        ...
        try:
            ...
            for metric_name, metric_data in (
                baseline_data.get("baselines", {}).get("test_execution", {}).items()
            ):
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Performance SLA enforcement for GitHub Actions")
    parser.add_argument(
        "--baseline",
        type=Path,
    ...
    parser.add_argument("--metrics", type=Path, required=True, help="Path to current metrics")
    parser.add_argument("--pr-comment", action="store_true", help="Generate PR comment")
    ...
    enforcer.set_github_output(
        results["passed"], enforcer.generate_pr_comment(results) if args.pr_comment else ""
    )
