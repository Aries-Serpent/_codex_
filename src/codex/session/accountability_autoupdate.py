...
_STOPWORDS = frozenset(
    ...
    "over under again further then once that this these those it its not no "
    "nor so if all each every both few more most other some such only own same "
    "than too very just don about up also how what which who whom when where "
    "why".split()
)
...
def tokenize_narrative(
    ...
    result.sort(key=lambda x: x["weight"], reverse=True)  # type: ignore[return-value]
