...
class PriorityPruner:
    ...
    DEFAULT_RULES = [
        ...
        PruneRule(
            pattern=r'^\d{4}-\d{2}-\d{2}.*(?:INFO|DEBUG)',
        ...
        PruneRule(
            pattern=r'(?:\.{10,}|={10,}|-{10,})',
            strategy=PruneStrategy.SUMMARIZE
        ),
    ]
    ...
    def prune(
        self,
        text: str,
        force_strategy: Optional[PruneStrategy] = None
    ) -> PrunedBlock:
        """
        ...
        if strategy == PruneStrategy.KEEP:
            ...
            pruned_text = self._truncate(text, matched_rule)
            key_info_preserved = matched_rule is not None and matched_rule.extract_pattern is not None
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            'avg_tokens_per_block': (
                self._total_pruned / self._blocks_processed
                if self._blocks_processed > 0 else 0
            ),
