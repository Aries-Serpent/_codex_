...
class AgentLoad:
    ...
    def sla_compliant(self, max_latency_ms: float) -> bool:
        ...
        return (
            ...
            and self.health != AgentHealth.UNHEALTHY
        )
...
class AgentLoadTracker:
    ...
    def sla_compliant_agents(self, max_latency_ms: float) -> list[str]:
        ...
        return [
            agent_id for agent_id, load in self._loads.items() if load.sla_compliant(max_latency_ms)
        ]
...
class EnhancedRouter:
    ...
    def _rank_by_utilization(
        self, agent_ids: list[str], task_description: str
    ...
    def _semantic_search(self, task_description: str) -> Optional[str]:
        ...
        try:
            ...
            agent_results = [r for r in results if ".github/agents/" in r.get("source", "")]
            if agent_results:
    ...
    def _save_decision(self, decision: RoutingDecision) -> None:
        ...
        try:
            trace_file.write_text(json.dumps(decision.to_dict(), indent=2), encoding="utf-8")
            self._decision_cache[decision.decision_id] = decision
        except Exception as e:
            logger.error(f"Failed to save decision trace: {e}")

    def _multi_decision(
        self, primary: RoutingDecision, candidates: list[tuple[str, float]], top_k: int
