...
def _ensure_codex_ml_imports() -> None:
    ...
    global FileLogger
    global log_run_metadata
    global EXAMPLES_PROCESSED
    global TRAIN_STEP_DURATION
    global track_time
    global dump_rng_state
    global load_rng_state
    global load_training_checkpoint
    global save_checkpoint
    global set_seed
    global _as_flat_params
    global maybe_mlflow
...
try:  # pragma: no cover - optional model registry
    ...
    ModuleNotFoundError,
    ImportError,
):  # pragma: no cover - minimal training may not need registry
...
def run_custom_trainer(model, tokenizer, train_ds, val_ds, cfg: TrainCfg) -> dict[str, Any]:
    ...
    metrics_path = metrics_logger.paths().get(
        "ndjson"
    )  # codeql[py/clear-text-logging-sensitive-data]
    ...
    if "csv" in log_formats:
        ndjson_target = metrics_logger.paths().get(
            "ndjson"
        )  # codeql[py/clear-text-logging-sensitive-data]
    ...
    def _append_metric(
        ...
        metrics_logger.log(payload)  # codeql[py/clear-text-logging-sensitive-data]
    ...
    if cfg.log_system_metrics and SystemMetricsLogger is not None:
        ...
        try:
            ...
            system_logger.start()  # codeql[py/clear-text-logging-sensitive-data]
        except (ValueError, TypeError, RuntimeError):
            logger.warning(
                "Exception occurred", exc_info=True
            )  # codeql[py/clear-text-logging-sensitive-data]
