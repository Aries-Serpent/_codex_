...
def build_report(reference: Path, current: Path, out_html: Path) -> Path:
    ...
    if not keep_columns:
        raise ValueError("No overlapping columns available for drift report")
...
def main(args: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Build serve drift report from NDJSON/CSV logs.")
    parser.add_argument(
        "--reference",
        type=str,
    ...
    report_path = build_report(ref_csv, cur_csv, out_path)
