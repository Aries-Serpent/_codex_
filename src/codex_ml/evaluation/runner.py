...
class EvaluationRunner:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        """
        print(f"Starting evaluation run with {len(self.metrics)} metrics...")
        ...
        with torch.no_grad() if torch else _nullcontext():
            for batch_idx, batch in enumerate(dataloader):
                ...
                if hasattr(self.model, 'predict'):
                    ...
                    raise ValueError(f"Model {type(self.model)} has no predict/forward/__call__ method")
        ...
        print(f"Evaluation complete: {total_samples} samples in {elapsed_time:.2f}s")
        print(f"Results: {metric_results}")
    ...
    def _log_to_tracking(self) -> None:
        ...
        try:
            ...
            self.tracking_writer.log_metric('throughput', self.results['throughput_samples_per_sec'])
