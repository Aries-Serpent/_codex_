...
try:  # pragma: no cover - optional
    if torch is not None:
        SummaryWriter = torch.utils.tensorboard.SummaryWriter  # type: ignore[assignment]
except (IOError, OSError):  # pragma: no cover - tensorboard not installed
...
def _get_safety_cfg():  # type: ignore[no-untyped-def]
...
def _get_safety_filters():  # type: ignore[no-untyped-def]
