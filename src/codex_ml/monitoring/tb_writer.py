...
class TBWriter:
    ...
    def __init__(self, enabled: bool, logdir: str = "runs/codex") -> None:
        ...
        if self.enabled and SummaryWriter is not None:
            ...
            except (IOError, OSError, ModuleNotFoundError, ImportError):  # pragma: no cover - tensorboard initialisation failures
                self._writer = None
    ...
    def add_scalar(self, tag: str, value: float, step: int) -> None:
        ...
        except (IOError, OSError, ModuleNotFoundError, ImportError):  # pragma: no cover - tensorboard runtime errors
    ...
    def close(self) -> None:
        ...
        try:
            self._writer.flush()
        except (IOError, OSError, ModuleNotFoundError, ImportError):  # pragma: no cover - flushing is best-effort
            logger.debug("Suppressed exception in handler", exc_info=True)
        try:
            self._writer.close()
        except (IOError, OSError, ModuleNotFoundError, ImportError):  # pragma: no cover - closing is best-effort
            logger.debug("Suppressed exception in handler", exc_info=True)
