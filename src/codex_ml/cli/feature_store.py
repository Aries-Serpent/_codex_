...
def register(
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Registered feature group: {name} v{version}")  # codeql[py/clear-text-logging-sensitive-data]

    except (ValueError, TypeError, RuntimeError) as e:
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error registering feature group: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
...
def list(
    ...
    try:
        ...
        if not features:
            console.print("[yellow]No features registered[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
            return
        ...
        console.print(f"\n[dim]Total features: {len(features)}[/dim]")  # codeql[py/clear-text-logging-sensitive-data]

    except (ValueError, TypeError, RuntimeError) as e:
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error listing features: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise typer.Exit(code=1) from e
...
def health(
    ...
    try:
        ...
        features = store.list_features()
        if not features:
            console.print("[yellow]No features to monitor[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
            return
        ...
        if output_file:
            ...
            with open(output_file, "w") as f:
                f.write(report)
            console.print(f"[green]✓[/green] Health report written to: {output_file}")  # codeql[py/clear-text-logging-sensitive-data]
        else:
        ...
        console.print(f"\n[dim]Healthy: {healthy_count}/{total_count}[/dim]")  # codeql[py/clear-text-logging-sensitive-data]

    except (ValueError, TypeError, RuntimeError) as e:
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error generating health report: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise typer.Exit(code=1) from e
...
def materialize(
    ...
    try:
        ...
        console.print(f"Features to materialize: {', '.join(feature_names)}")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        console.print("\n[dim]Use Python API for full materialization functionality[/dim]")  # codeql[py/clear-text-logging-sensitive-data]

    except (ValueError, TypeError, RuntimeError) as e:
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error materializing features: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise typer.Exit(code=1) from e
...
def versions(
    ...
    try:
        ...
        if not versions:
            console.print(f"[yellow]No versions found for feature: {feature_name}[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
            return
        ...
        console.print(table)  # codeql[py/clear-text-logging-sensitive-data]
        console.print(f"\n[dim]Total versions: {len(versions)}[/dim]")  # codeql[py/clear-text-logging-sensitive-data]

    except (ValueError, TypeError, RuntimeError) as e:
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error listing versions: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise typer.Exit(code=1) from e
...
def info(
    ...
    try:
        ...
        if metadata is None:
            ...
            if feature is None:
                console.print(f"[yellow]Feature not found: {feature_name}[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
                return
            console.print(f"[yellow]Feature exists but has no metadata: {feature_name}[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        if metadata:
            ...
            console.print(f"  Version: {metadata.version}")  # codeql[py/clear-text-logging-sensitive-data]
            console.print(f"  Data Type: {metadata.dtype}")  # codeql[py/clear-text-logging-sensitive-data]
            console.print(f"  Description: {metadata.description}")  # codeql[py/clear-text-logging-sensitive-data]
            console.print(f"  Created: {metadata.created_at}")  # codeql[py/clear-text-logging-sensitive-data]
            console.print(f"  Updated: {metadata.updated_at}")  # codeql[py/clear-text-logging-sensitive-data]

            if metadata.tags:
                console.print(f"  Tags: {', '.join(f'{k}={v}' for k, v in metadata.tags.items())}")  # codeql[py/clear-text-logging-sensitive-data]

        console.print("\n[cyan]Health Status:[/cyan]")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        console.print(f"  Freshness: {health.freshness_level} ({health.freshness_minutes:.1f} min)")  # codeql[py/clear-text-logging-sensitive-data]
        console.print(f"  Last Updated: {health.last_updated}")  # codeql[py/clear-text-logging-sensitive-data]
        console.print(f"  Error Count: {health.error_count}")  # codeql[py/clear-text-logging-sensitive-data]

        if health.warnings:
            console.print("\n[yellow]Warnings:[/yellow]")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        error_type = type(e).__name__
        logger.debug("Exception: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        console.print("[red]✗[/red] Error getting feature info: <ERROR_TYPE>")  # codeql[py/clear-text-logging-sensitive-data]
        raise typer.Exit(code=1) from e
