...
class PrometheusMetrics:
    ...
    def export_prometheus_format(self) -> str:
        ...
        for endpoint, hist in self.request_latency.items():
            ...
            lines.append(f'inference_request_latency_milliseconds_count{{endpoint="{endpoint}"}} {stats["count"]}')
            lines.append(f'inference_request_latency_milliseconds_sum{{endpoint="{endpoint}"}} {stats["sum"]}')
        ...
        # Circuit breaker metrics
        lines.append("# HELP circuit_breaker_state Circuit breaker state (0=closed, 1=half_open, 2=open)")
    ...
    def get_summary(self) -> Dict:
        ...
        with self.lock:
            ...
            cache_hit_rate = (
                self.model_cache_hits / cache_total if cache_total > 0 else 0.0
            )
