...
class HAREntry:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'HAREntry':
        return cls(
            ...
            timings=HARTimings(**{
                k: v for k, v in data.get('timings', {}).items()
...
class HARLog:
    ...
    version: str = "1.2"
    creator: Dict[str, str] = field(default_factory=lambda: {
...
class HARRecorder:
    ...
    def __init__(self, name: str = "Codex Recording", auto_save: bool = False, save_path: Optional[Path] = None):
        self.har_log = HARLog()
        self.name = name
    ...
    def record_complete_transaction(
        ...
        response = HARResponse(
            ...
            content={'size': len(response_body) if response_body else 0, 'mimeType': 'application/json', 'text': response_body or ''},
        )
