...
def precision_recall_f1(logits: torch.Tensor, targets: torch.Tensor) -> tuple[float, float, float]:
    ...
    if logits.ndim == 1 or (logits.ndim > 1 and logits.shape[-1] == 1):
        ...
        if logits_1d.dtype.is_floating_point and torch.all((0.0 <= logits_1d) & (logits_1d <= 1.0)):  # type: ignore[operator]
            preds = (logits_1d >= 0.5).to(targets.dtype)  # type: ignore[operator, attr-defined]
        else:
            preds = (logits_1d >= 0).to(targets.dtype)  # type: ignore[operator, attr-defined]
    else:
        preds = torch.argmax(logits, dim=-1)
