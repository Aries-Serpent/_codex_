...
class CacheOptimizationAnalyzer:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        estimated_hit_rate_gain = sum(
            opt.estimated_hit_rate_improvement for opt in recommendations[:3]
        )
        estimated_latency_gain = sum(
            opt.estimated_latency_improvement for opt in recommendations[:3]
        )
    ...
    def _generate_recommendations(
        ...
        sorted_opts = sorted(
            ...
            key=lambda x: (
                x.priority != "HIGH",
                x.implementation_effort == "HIGH",
                -x.estimated_hit_rate_improvement,
            ),
        )
