...
def _validate(config: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if not isinstance(config, dict):
        issues.append({"path": "root", "issue": f"Expected dict, got {type(config).__name__}"})
        return issues
...
def validate_config_file(path: Path) -> dict[str, Any]:
    ...
    """
    report: dict[str, Any] = {"path": str(path), "valid": False, "issues": []}
    ...
    try:
        ...
        report["issues"].append({"path": "root", "issue": f"File not found: {path}"})
    except Exception as e:
        report["issues"].append({"path": "root", "issue": f"Failed to load config: {e}"})
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate Codex config files (offline)")
    parser.add_argument(
        "--path", type=Path, required=True, help="Path to config file (YAML or JSON)"
    )
