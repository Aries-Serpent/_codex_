    ...
    def _compute_advantages(self) -> List[float]:
        ...
        for i in reversed(range(len(self.trajectory))):
            ...
            if transition['done']:
                ...
                next_value = self.trajectory[i + 1]['value'] if i + 1 < len(self.trajectory) else 0.0
                delta = transition['reward'] + self.discount_factor * next_value - transition['value']
    ...
    def _update_policy(self):
        ...
        for _ in range(self.epochs_per_update):
            ...
            for transition, advantage, ret in zip(self.trajectory, advantages, returns):
                ...
                self.value_weights[transition['state']] += self.learning_rate * (ret - current_value)
