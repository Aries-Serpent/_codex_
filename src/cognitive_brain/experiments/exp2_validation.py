...
def run_exp2(
    ...
    compliance_assessor = QuantumComplianceAssessor(config, monitor, repository, enable_superposition=True)
    ...
    for i in range(entangled_count, sample_size):
        ...
        decisions_map = {
            "APPROVE": 1.0, "MONITOR": 0.5, "REJECT": 0.0, "CONDITIONAL": 0.25,
            "BLOCK": 0.0, "ALLOW": 1.0
        }
    ...
    # Success evaluation
    success = (
        redundancy_reduction >= EXP_2_CONFIG.success_criteria["redundancy_reduction"] and
        avg_entangled_correlation >= EXP_2_CONFIG.success_criteria["correlation"] and
        latency_overhead_ms < EXP_2_CONFIG.success_criteria["latency_overhead"]
    )
...
if __name__ == "__main__":
    ...
    print(f"  Avg Entangled Correlation: {results['avg_entangled_correlation']:.3f} (target: >0.80)")
