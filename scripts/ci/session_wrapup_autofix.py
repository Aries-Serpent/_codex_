...
def _compute_merge_readiness_score() -> dict:
    ...
    rc, _ = _run(
        ...
        timeout=120,
        env={"CODEX_SKIP_PATTERN_NUMS": "25,30"},
    )
    ok1 = rc == 0
...
def _accountability_report_is_current() -> bool:
    ...
    if not text.strip():
        return False
    if "## SESSION SUMMARY" not in text and "## Session:" not in text:
        return False
...
def _report_already_has_auto_entry(pr_number: str) -> bool:
    ...
    # Match today's exact heading generated by fix_accountability_report():
    # "## SESSION SUMMARY — YYYY-MM-DD... SESSION AUTO [auto-generated] (CI Auto-Fix — PR #N)"
    today = datetime.now(tz=timezone.utc).strftime("%Y-%m-%d")
    return bool(re.search(
        rf"## SESSION SUMMARY.*{re.escape(today)}.*SESSION AUTO.*{re.escape(_AUTO_ENTRY_SENTINEL)}.*PR #{re.escape(pr_number)}",
        content,
    ))
