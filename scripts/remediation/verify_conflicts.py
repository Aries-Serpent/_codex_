...
def check_import(module_name: str, expected_location_substr: str = None, quiet: bool = False) -> tuple[bool, str]:
...
def main():
    ...
    parser.add_argument("--mode", choices=["strict", "shim-aware", "legacy"], default="legacy", help="Verification mode: strict (fail on non-whitelisted), shim-aware (warn only), legacy (original behavior)")
    ...
    if args.mode in ["strict", "shim-aware"]:
        ...
        yaml_ok, yaml_origin = check_import("yaml", expected_location_substr="site-packages" if args.expect_site_packages else None, quiet=True)
        hydra_ok, hydra_origin = check_import("hydra", expected_location_substr="site-packages" if args.expect_site_packages else None, quiet=True)
        ...
        if args.mode == "strict" and findings["violations"]:
            print(f"\n[FAIL] {len(findings['violations'])} non-whitelisted duplicate(s) found in strict mode.", file=sys.stderr)
            sys.exit(1)
    ...
    yaml_ok, _ = check_import("yaml", expected_location_substr="site-packages" if args.expect_site_packages else None)
    ...
    hydra_ok, _ = check_import("hydra", expected_location_substr="site-packages" if args.expect_site_packages else None)
    ...
    src_train, _ = check_import("src.training", expected_location_substr=str(ROOT / "src" / "training"))
    ...
    src_tok, _ = check_import("src.tokenization", expected_location_substr=str(ROOT / "src" / "tokenization"))
