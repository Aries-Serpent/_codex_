        ...
        items.append(
            {
                ...
                "status": (
                    "shim"
                    if (mod in legacy and mod in canonical)
                    else ("migrated" if mod in canonical else "active")
                ),
                "rationale": "",
...
def main():
    ...
    ap.add_argument(
        ...
        nargs="+",
        required=True,
        help="Root directories to scan (e.g., training src/training)",
    )
    ...
    print(
        f"[INFO] Found {len(inventory)} modules ({len(legacy)} legacy, {len(canonical)} canonical)"
    )
