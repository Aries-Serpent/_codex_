...
def print_progress_report(caps: List[Dict]):
    ...
    print(f"  Critical (<0.40):  {len(categories['critical']):2d} ({len(categories['critical'])/len(caps)*100:5.1f}%)")
    print(f"  Low (0.40-0.69):   {len(categories['low']):2d} ({len(categories['low'])/len(caps)*100:5.1f}%)")
    print(f"  Medium (0.70-0.84): {len(categories['medium']):2d} ({len(categories['medium'])/len(caps)*100:5.1f}%)")
    print(f"  High (≥0.85):      {len(categories['high']):2d} ({len(categories['high'])/len(caps)*100:5.1f}%)")
    ...
    print(f"  Phase 1 (ALL ≥0.70):  {'✅' if phase1_complete else '❌'} {len(categories['critical']) + len(categories['low'])} remaining")
    print(f"  Phase 2 (ALL ≥0.85):  {'✅' if phase2_complete else '❌'} {len(categories['medium'])} remaining")
    print(f"  Phase 3 (AVG ≥0.93):  {'✅' if avg_score >= 0.93 else '❌'} {0.93 - avg_score:+.4f} to target")
    ...
    if low_maturity:
        ...
        for i, cap in enumerate(low_sorted[:8], 1):
            ...
            weakest = min(components.items(), key=lambda x: x[1]) if components else ('unknown', 0.0)
            print(f"  {i}. {cap['id']:30s}  Score: {cap['score']:.4f}  Weakest: {weakest[0]} ({weakest[1]:.2f})")
    ...
    component_scores = {'tests': [], 'documentation': [], 'functionality': [], 'safeguards': [], 'consistency': []}
    for cap in caps:
        components = cap.get('components', {})
