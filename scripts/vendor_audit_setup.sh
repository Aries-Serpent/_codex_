...
# Sampling controls (kept for parity; default disabled in restricted envs)
export CODEX_VENDOR_SAMPLE_ENABLE="${CODEX_VENDOR_SAMPLE_ENABLE:-0}"
export CODEX_VENDOR_SAMPLE_MAX="${CODEX_VENDOR_SAMPLE_MAX:-6}"
export CODEX_VENDOR_SAMPLE_INDEX="${CODEX_VENDOR_SAMPLE_INDEX:-https://pypi.org/simple}"
export CODEX_VENDOR_SAMPLE_KEEP="${CODEX_VENDOR_SAMPLE_KEEP:-0}"
...
# Ensure VAR has a default if it is unset
ensure_default() {
  local var="$1" def="$2"
...
sanitize_env() {
  local v val
  # booleans
  for v in CODEX_FORCE_CPU CODEX_ALLOW_TRITON_CPU CODEX_FAIL_ON_VIOLATION CODEX_AUDIT_BOOTSTRAP CODEX_OFFLINE CODEX_VENDOR_SAMPLE_ENABLE CODEX_VENDOR_SAMPLE_KEEP CODEX_VENDOR_VERBOSE CODEX_ERR_TRAP; do
    val="$(strip_comment "$(get_var "$v")")"
    [[ -n "$val" ]] && export "$v"="$val"
  done
  # numeric
  for v in CODEX_VENDOR_MAX_PACKAGES CODEX_VENDOR_MAX_SIZE_KB CODEX_VENDOR_SAMPLE_MAX CODEX_NET_TRIALS CODEX_CPU_TRIALS CODEX_DISK_BENCH_BYTES CODEX_DISK_TRIALS CODEX_CPU_BENCH_BUF_KB; do
    val="$(strip_comment "$(get_var "$v")")"
    [[ -n "$val" ]] && export "$v"="$val"
  done
  # strings
  for v in CODEX_VENDOR_LOG_AGG CODEX_VENDOR_SAMPLE_INDEX CODEX_TORCH_VERSION_BASE CODEX_NET_TEST_URLS CODEX_CPU_TARGET_SECONDS; do
    val="$(strip_comment "$(get_var "$v")")"
    [[ -n "$val" ]] && export "$v"="$val"
  done
  # enforce defaults if runner blanked them
  ensure_default CODEX_VENDOR_SAMPLE_ENABLE "0"
  ensure_default CODEX_VENDOR_SAMPLE_MAX "6"
  ensure_default CODEX_VENDOR_SAMPLE_INDEX "https://pypi.org/simple"
}
sanitize_env
...
AUDIT_JSON="$CACHE_DIR/vendor_audit.setup.json"
ERROR_JSON="$CACHE_DIR/vendor_audit.setup.error.json"
PY_FILE="$CACHE_DIR/vendor_audit_setup.py"

log(){ printf "[setup-audit] %s %s\n" "$(date -Iseconds)" "$*"; }
die(){ printf "[setup-audit][ERROR] %s\n" "$*" >&2; exit 1; }
...
# uv bootstrap (optional, for sampling/bootstrap)
if [[ "$CODEX_OFFLINE" != "1" && ( "$CODEX_AUDIT_BOOTSTRAP" == "1" || "$CODEX_VENDOR_SAMPLE_ENABLE" == "1" ) ]]; then
...
def run_audit():
    ...
    audit={
      ...
      "policy":{"allow_triton_cpu":allow_triton,"max_packages":max_pkgs,"max_size_kb":max_size_kb,"vendor_log_agg":vendor_log_agg,"bootstrap":bootstrap,
                "sample_enable": getenv_bool("CODEX_VENDOR_SAMPLE_ENABLE", False),
    ...
    (REPO_ROOT/".codex"/"cache"/"vendor_audit.setup.json").write_text(json.dumps(audit, indent=2))
    ...
    # System snapshot
    print(f"-- System Capabilities Snapshot --")
    ...
    if audit["verdict"]["ok"]: print("- Verdict: OK (no vendor policy violations)")
    else:
        print("- Verdict: VIOLATIONS detected:")
        for v in audit["verdict"]["violations"]: print(f"  * {v}")
...
if __name__ == "__main__":
    try:
        ...
        try:
            (REPO_ROOT/".codex"/"cache"/"vendor_audit.setup.error.json").write_text(json.dumps(err, indent=2))
        except Exception:
            pass
        print(f"[SETUP-AUDIT][ERROR] {type(e).__name__}: {e}", file=sys.stderr)
        sys.exit(2)
PY

# Execute Python file (tee preserves human-readable log; with pipefail we still get non-zero on Python error)
python "$PY_FILE" | tee /tmp/vendor_audit_setup.out

# Exit gate
VIOLATIONS=0
if grep -q "VIOLATIONS detected" /tmp/vendor_audit_setup.out; then VIOLATIONS=1; fi
if [[ "${CODEX_FAIL_ON_VIOLATION}" == "1" && $VIOLATIONS -ne 0 ]]; then die "Vendor policy violations in setup audit"; fi
