...
def generate_inventory():
    ...
    if not workflows_dir.exists():
        print(f"❌ Workflows directory not found: {workflows_dir}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
        return
    ...
    print(f"📊 Cataloging {len(workflow_files)} workflows...")  # nosec  # codeql[py/clear-text-logging-sensitive-data]

    for workflow_file in workflow_files:
        print(f"  Processing: {workflow_file.name}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
    ...
    print(f"\n✅ Inventory saved to: {inventory_path}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
    print(f"   Total workflows: {inventory['metadata']['total_workflows']}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
    print(f"   Active: {inventory['metadata']['active_count']}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
    print(f"   Consolidation candidates: {sum(1 for w in inventory['workflows'] if w.get('consolidation_candidate'))}")  # nosec  # codeql[py/clear-text-logging-sensitive-data]
...
def generate_summary_report(inventory: dict):
    ...
    with open(report_path, "w") as f:
        ...
        if candidates:
            ...
            for workflow in candidates:  # nosec  # codeql[py/clear-text-storage-sensitive-data]
