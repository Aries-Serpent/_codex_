        ...
        for name in secret_names:
            ...
            if value:
                backup_data['secrets'][name] = {
                    'hash': hashlib.sha256(value.encode()).hexdigest(),  # nosec  # codeql[py/clear-text-logging-sensitive-data]  # pragma: allowlist secret
                    'length': len(value)
                }

        with open(backup_file, 'w') as f:  # nosec  # codeql[py/clear-text-logging-sensitive-data] Backup only stores hashes and lengths, not secrets
