...
class GitHubSecretsManager:
    ...
    def backup_secrets(self) -> dict[str, str]:
        ...
        print("Backing up GitHub Secrets...")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        for name in secret_names:
            ...
            if value:
                backup_data['secrets'][name] = {
                    'hash': hashlib.sha256(value.encode()).hexdigest(),  # codeql[py/clear-text-logging-sensitive-data]  # pragma: allowlist secret
                    'length': len(value)
                }

        with open(backup_file, 'w') as f:  # codeql[py/clear-text-logging-sensitive-data] Backup only stores hashes and lengths, not secrets
            json.dump(backup_data, f, indent=2)

        print(f"✓ Backed up {len(backup_data['secrets'])} secrets to: {backup_file}")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    def rotate_secrets(self, secret_names: list[str]) -> dict[str, str]:
        ...
        print(f"Rotating {len(secret_names)} secrets...")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        for name in secret_names:
            ...
            try:
                ...
                if self.repo:
                    ...
                    print("✓ Rotated secret")  # codeql[py/clear-text-logging-sensitive-data]
                    results['rotated'].append({'secret_ref': secret_ref, 'status': 'success'})
                else:
                    print("⚠ Skipped secret rotation (no repo connection)")  # codeql[py/clear-text-logging-sensitive-data]
                    results['failed'].append({'secret_ref': secret_ref, 'reason': 'no_repo'})
            except Exception as e:
                logger.warning("Secret rotation failed for %s: %s", secret_ref, _safe_error(e))  # codeql[py/clear-text-logging-sensitive-data]  # pragma: allowlist secret
                print(f"✗ Failed to rotate secret ({secret_ref})")  # codeql[py/clear-text-logging-sensitive-data]  # pragma: allowlist secret
    ...
    def validate_secrets(self) -> bool:
        ...
        print("Validating GitHub Secrets...")  # codeql[py/clear-text-logging-sensitive-data]
        ...
        for name in required_secrets:
            ...
            secret_ref = _secret_ref(name)
            if not value:
                print("✗ Missing required secret")  # codeql[py/clear-text-logging-sensitive-data]
                validations.append({'secret_ref': secret_ref, 'passed': False})
                all_valid = False
            elif len(value) < 32:
                print("✗ A required secret is too short")  # codeql[py/clear-text-logging-sensitive-data]
                validations.append({'secret_ref': secret_ref, 'passed': False})
                all_valid = False
            else:
                print("✓ Secret passed validation")  # codeql[py/clear-text-logging-sensitive-data]
    ...
    def sync_downstream(self) -> None:
        ...
        print("Syncing secrets to downstream systems...")  # codeql[py/clear-text-logging-sensitive-data]
