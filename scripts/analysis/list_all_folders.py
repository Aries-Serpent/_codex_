...
def save_compressed(data: str, output_path: Path, format: str) -> None:
    ...
    if format == 'gzip':
        ...
        if not HAS_ZSTD:
            print("⚠️  Zstandard not available. Install with: pip install zstandard")
            return
...
def main() -> None:
    ...
    print("\nGenerated files:")
    if args.format in ['plain', 'all']:
        print("  - ALL_FOLDERS.txt (plain text list)")
    if args.format in ['markdown', 'all']:
        print("  - ALL_FOLDERS_LINKS.md (alphabetical with links)")
    if args.format in ['categorized', 'all']:
        print("  - ALL_FOLDERS_CATEGORIZED.md (grouped by top-level)")
    if args.format in ['tree', 'all']:
        print("  - ALL_FOLDERS_TREE.md (tree structure with links)")
    if args.format in ['files', 'all'] and files_by_folder:
        print("  - ALL_FILES_BY_FOLDER.md (files in each folder)")
    if args.format in ['json', 'jsonl', 'all']:
        print("  - repository_structure.json (JSON format)")
        if args.format in ['jsonl', 'all']:
            print("  - repository_structure.jsonl (JSON Lines format)")
        if args.compress:
            compress_formats = ['gzip', 'brotli', 'zstd'] if args.compress == 'all' else [args.compress]
            for fmt in compress_formats:
                suffix = {'gzip': '.gz', 'brotli': '.br', 'zstd': '.zst'}[fmt]
