...
def check_table_spacing(file_path: Path) -> List[Dict[str, any]]:
    ...
    for i in range(len(lines) - 1):
        ...
        if next_line.startswith('|'):
            ...
            # Also exclude lines starting with # (markdown headers should be allowed)
...
def fix_table_spacing(file_path: Path, issues: List[Dict[str, any]], dry_run: bool = False) -> bool:
    """
...
def scan_directory(docs_dir: Path, fix: bool = False, dry_run: bool = False) -> Tuple[int, int, int]:
    """
