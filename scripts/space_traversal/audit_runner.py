...
def import_yaml_from_sitepackages():
    ...
    try:
        ...
        filtered = [
            p
            for p in sys.path
            if p and os.path.abspath(p) != repo_root and os.path.abspath(p) != ""
        ]
        sys.path = filtered
        import yaml  # noqa
...
def _get_matrix_template(cfg: dict) -> str:
    ...
    if mt2:
        return mt2
    raise KeyError(
        "matrix_template not found in config. Provide either 'output.matrix_template' or top-level 'matrix_template'"
    )
        ...
        for canonical, aliases in overrides.items():
            ...
            for alias in aliases:
                ...
                base["evidence_files"] = sorted(
                    set(base.get("evidence_files", []) + a.get("evidence_files", []))
                )
                base["found_patterns"] = sorted(
                    set(base.get("found_patterns", []) + a.get("found_patterns", []))
                )
                base["required_patterns"] = sorted(
                    set(base.get("required_patterns", []) + a.get("required_patterns", []))
                )
                base["docs_keywords"] = sorted(
                    set(base.get("docs_keywords", []) + a.get("docs_keywords", []))
                )
            ...
            if (
                not base_from_existing
                and not alias_contributed
                and not missing_by_canonical.get(canonical)
            ):
...
def stage_s4_scoring(cfg, raw_caps):
    ...
    for cap in raw_caps:
        ...
        meta_safeguards = (
            cap.get("meta", {}).get("safeguards", []) if isinstance(cap.get("meta"), dict) else []
        )
        safeguards = safeguard_score(
            cap.get("evidence_files", []), file_cache, detector_safeguards, meta_safeguards
        )
        ...
        try:
            ...
            print(
                f"Warning: Could not normalize components for {cap.get('id')}: {e}", file=sys.stderr
            )
        scored.append(
            {
                "id": cap.get("id"),
    ...
    out.write_text(
        json.dumps(
            {"generated": time.time(), "capabilities": scored, "version": VERSION},
            indent=2,
            sort_keys=True,
            ensure_ascii=False,
        ),
...
def stage_s5_gaps(cfg, scored_caps):
    ...
    component_out.write_text(
        json.dumps(component_payload, indent=2, sort_keys=True), encoding="utf-8"
    )
...
def stage_s7_manifest(cfg):
    ...
    if total_w > 0 and abs(total_w - 1.0) > 1e-9:
        normalized_weights = (
            cs.normalize_weights(weights) if cs else {k: v / total_w for k, v in weights.items()}
        )
    ...
    if options.get(OPT_FAIL_ON_LOW_MATURITY, False):
        ...
        if low_maturity:
            for cap in low_maturity:
                violations.append(
                    f"LOW_MATURITY: {cap['id']} score={cap['score']:.4f} < {low_threshold}"
                )
            ...
            if canonical not in capability_ids:
                ...
                if not aliases_found:
                    ...
                    if cap and cap.get("meta", {}).get("missing_detectors"):
                        violations.append(
                            f"MISSING_DETECTOR: {canonical} has missing alias detectors: {cap['meta']['missing_detectors']}"
                        )
...
def cmd_store_trend(args, cfg):
    ...
    if not scored_path.exists():
        print(
            f"No capabilities_scored.json found at {scored_path}. Run audit first.", file=sys.stderr
        )
        sys.exit(EXIT_MISSING_ARTIFACTS)
...
def main():
    ...
    compare_p = sub.add_parser(
        "compare-runs", help="Compare two audit runs with detailed analysis (v1.5.1)"
    )
    ...
    cli_builder_p = sub.add_parser(
        "cli-builder", help="Generate interactive CLI builder HTML (v1.5.3)"
    )
    cli_builder_p.add_argument("--output", help="Output path for CLI builder HTML")

    # v1.5.4 API collection command
    api_collection_p = sub.add_parser(
        "api-collection", help="Generate API collection HTML with adjusters (v1.5.4)"
    )
    api_collection_p.add_argument("--output", help="Output path for API collection HTML")

    # v1.5.4 Swagger docs command
    swagger_p = sub.add_parser(
        "api-docs", help="Generate Swagger/OpenAPI-style API documentation (v1.5.4)"
    )
