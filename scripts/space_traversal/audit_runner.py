...
Enhancements (v1.2.0):
- Propagate required_patterns into scored payloads for downstream rendering.
- Compute missing_patterns and write expanded gap metadata (gaps.json).
- Emit component_gaps.json for zero-value components and deficit hints.
- Add `validate` command (low-maturity + missing-detector gates).
- Include missing_detectors and thresholds snapshot in manifest.
"""
...
def main():
    ...
    run_p.add_argument("--artifacts-dir", help="Override artifacts directory path")
    stage_p = sub.add_parser("stage", help="Run a single stage")
    stage_p.add_argument("stage_id", help="Stage code (S1..S7)")
    stage_p.add_argument("--artifacts-dir", help="Override artifacts directory path")
    ...
    # Override artifacts_dir if provided via command line
