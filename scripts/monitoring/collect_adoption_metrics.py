...
class AdoptionMetricsCollector:
    ...
    def collect_validation_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_reports:
                ...
                # Get unique datasets
                datasets = set(r.get("dataset_name") for r in recent_reports if r.get("dataset_name"))
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if metrics["mlflow"]["enabled"] and metrics["mlflow"]["runs_total"] < 10:
            recommendations.append("RECOMMENDATION: MLflow enabled but low usage. Consider increasing experiment logging.")
        
        # Feature store recommendations
        if metrics["feature_store"]["enabled"]:
            if metrics["feature_store"]["stale_groups"] > 0:
                recommendations.append(f"ACTION_REQUIRED: {metrics['feature_store']['stale_groups']} feature groups are stale (>48h). Trigger feature refresh.")
            
            if metrics["feature_store"]["sla_compliance"] and metrics["feature_store"]["sla_compliance"] < 0.95:
                recommendations.append("WARNING: Feature freshness SLA below 95%. Review feature update pipeline.")
        
        # Validation recommendations
        if metrics["validation"]["enabled"]:
            if metrics["validation"]["pass_rate"] and metrics["validation"]["pass_rate"] < 0.95:
                recommendations.append(f"ALERT: Validation pass rate at {metrics['validation']['pass_rate']:.1%}. Investigate data quality issues.")
        
        # Overall adoption
        if report["overall_adoption_score"] < 0.5:
            recommendations.append("LOW_ADOPTION: Overall adoption score below 50%. Consider enabling more features.")
