...
def main() -> int:
    ...
    print("\n" + "="*70)
    if args.dry_run:
        print(f"✅ Dry run completed: {total_fixes} potential fixes identified")  # nosec  # codeql[py/clear-text-logging-sensitive-data]  # B110
        print("\nTo apply fixes, run without --dry-run:")
        print("  python scripts/fix_security_issues.py")
    else:
        print(f"✅ Security fixes completed: {total_fixes} automatic fixes applied")  # nosec  # codeql[py/clear-text-logging-sensitive-data]  # B110
        print("\nNext steps:")
