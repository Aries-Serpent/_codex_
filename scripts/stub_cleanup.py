...
class StubDetector(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        self.current_class_is_abstract = bool(
            self._class_bases & self.PROTOCOL_BASES
        )
    ...
    def _has_abstract_decorator(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
        ...
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Name) and decorator.id in ('abstractmethod', 'abstractproperty'):
                return True
            if isinstance(decorator, ast.Attribute) and decorator.attr in ('abstractmethod', 'abstractproperty'):
                return True
        return False
    ...
    def _has_property_decorator(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
        ...
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Name) and decorator.id == 'property':
                return True
            if isinstance(decorator, ast.Attribute) and decorator.attr in ('property', 'setter', 'getter', 'deleter'):
                return True
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AST-based stub detection and cleanup tool"
    )
    parser.add_argument(
        'path',
        type=Path,
    ...
    parser.add_argument(
        '--include-abstract',
        action='store_true',
        help="Include abstract methods in results"
    )
    parser.add_argument(
        '--min-confidence',
        type=float,
