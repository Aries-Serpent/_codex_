...
class MarkdownAnalyzer:
    ...
    def analyze(self) -> List[MarkdownFile]:
        ...
        self.files.sort(key=lambda f: (
            0 if f.recommendation == "archive" else 
            1 if f.recommendation == "review" else 2,
            -f.age_days
        ))
    ...
    def _categorize_file(self, filename: str, content: str) -> str:
        ...
        if any(re.search(p, filename_lower) or re.search(p, content[:1000]) 
    ...
    if review_count > 0:
        ...
        for f in files:
            if f.recommendation == "review":
                lines.append(
                    f"| `{f.path.name}` | {f.age_days} | {f.category} | {f.references} |"
                )
        lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Execute archival (move files to archive)"
    )
    parser.add_argument(
        "--archive-dir",
        type=Path,
