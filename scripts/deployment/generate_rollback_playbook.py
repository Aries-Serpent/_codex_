...
class RollbackPlaybookGenerator:
    ...
    def generate_playbook(self) -> str:
        ...
        for deployment in self.resources['deployments']:
            ...
            playbook.append(f"# Get current revision\n")
            playbook.append(f"kubectl rollout history deployment/{name} -n {ns}\n\n")
            playbook.append(f"# Rollback to previous revision\n")
            playbook.append(f"kubectl rollout undo deployment/{name} -n {ns}\n\n")
            playbook.append(f"# Wait for rollback to complete\n")
        ...
        for deployment in self.resources['deployments']:
            ...
            playbook.append("```bash\n")
            playbook.append(f"# Step 1: View rollout history\n")
            playbook.append(f"kubectl rollout history deployment/{name} -n {ns}\n\n")
            playbook.append(f"# Step 2: Get details for specific revision (optional)\n")
            playbook.append(f"kubectl rollout history deployment/{name} -n {ns} --revision=<N>\n\n")
            playbook.append(f"# Step 3: Perform rollback to previous revision\n")
            playbook.append(f"kubectl rollout undo deployment/{name} -n {ns}\n\n")
            playbook.append(f"# OR: Rollback to specific revision\n")
            playbook.append(f"kubectl rollout undo deployment/{name} -n {ns} --to-revision=<N>\n\n")
            playbook.append(f"# Step 4: Monitor rollback progress\n")
            playbook.append(f"kubectl rollout status deployment/{name} -n {ns} --timeout=10m\n\n")
            playbook.append(f"# Step 5: Verify pods are running\n")
        ...
        for deployment in self.resources['deployments']:
            ...
            playbook.append("```bash\n")
            playbook.append(f"# Option 1: Kill all pods (Kubernetes will restart with previous image)\n")
            playbook.append(f"kubectl delete pods --all -n {ns}\n\n")
            playbook.append(f"# Option 2: Immediate rollback\n")
            playbook.append(f"kubectl set image deployment/{name} codex-ml=codex-ml:stable -n {ns} --record\n\n")
            playbook.append(f"# Option 3: Scale down and up\n")
            playbook.append(f"kubectl scale deployment {name} --replicas=0 -n {ns}\n")
            playbook.append(f"sleep 10\n")
        ...
        for deployment in self.resources['deployments']:
            ...
            playbook.append(f"- ✅ All replicas are Ready (1/1)\n")
