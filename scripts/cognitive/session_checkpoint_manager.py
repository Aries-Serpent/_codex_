...
class SessionCheckpointManager:
    ...
    def create_checkpoint(
        ...
        if compress and self.compression_algorithm != 'none':
            try:
                if self.compression_algorithm == 'zstd':
                    if zstd is None:
                        raise CompressionError("zstd not installed, install with: pip install zstandard")
                    cctx = zstd.ZstdCompressor(level=self.compression_level)
                    compressed_bytes = cctx.compress(json_bytes)
                    file_ext = ".json.zst"
                elif self.compression_algorithm == 'gzip':
                    compressed_bytes = gzip.compress(json_bytes, compresslevel=self.compression_level)
                    file_ext = ".json.gz"
                else:
        ...
        checkpoint_file = session_dir / f"checkpoint_{timestamp.strftime('%Y%m%d_%H%M%S')}{file_ext}"
    ...
    def list_checkpoints(
        ...
        for cp_file in checkpoint_files:
            try:
                ...
                stem = cp_file.stem.replace('.json', '')
    ...
    def validate_checkpoint(
        ...
        if not data:
            ...
            if not quick_check:
                checks_total += 1
                required_fields = [
                    "schema_version", "checkpoint_id", "session_id",
                    "timestamp", "agent_state", "memory_snapshot",
                    "execution_progress"
                ]
