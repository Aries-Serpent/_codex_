    ...
    try:
        ...
        try:
            conda_version = subprocess.check_output(
                ["conda", "--version"], text=True, stderr=subprocess.STDOUT, timeout=5
            ).strip()
        ...
        try:
            packages_output = subprocess.check_output(
                ["conda", "list", "--export"], text=True, stderr=subprocess.STDOUT, timeout=10
            )
            packages = [
                line.strip()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--seed", type=int, help="Random seed value for reproducibility tracking")
    parser.add_argument(
        "--verbose", action="store_true", help="Print snapshot to stdout in addition to file"
    )
