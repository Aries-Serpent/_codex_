...
def test_ingest_creates_default(tmp_path: Path) -> None:
    ...
    assert len(rows) == 10, "Rows must not be empty"
...
def test_clean_filters_and_coerces(tmp_path: Path) -> None:
    ...
    assert len(cleaned) == 1, "Cleaned must not be empty"
...
def test_features_writes_csv(tmp_path: Path) -> None:
    ...
    assert feats, "feats is not valid"
    assert feats[0]["value_is_even"] is True, "Value must be initialized"
