...
def x__ensure_subpath__mutmut_orig(base: Path, candidate: Path) -> Path:
    """
...
def x__ensure_subpath__mutmut_1(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_2(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    """
    try:
        base_resolved = base.resolve(strict=None)
        candidate_resolved = candidate.resolve(strict=False)
    except RuntimeError:
        # Fallback in pathological resolution cases
        raise HTTPException(status_code=400, detail="Invalid path")
...
def x__ensure_subpath__mutmut_3(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_4(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_5(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    try:
        base_resolved = base.resolve(strict=False)
        candidate_resolved = candidate.resolve(strict=None)
    except RuntimeError:
        # Fallback in pathological resolution cases
        raise HTTPException(status_code=400, detail="Invalid path")
...
def x__ensure_subpath__mutmut_6(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_7(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    try:
        ...
        # Fallback in pathological resolution cases
        raise HTTPException(status_code=None, detail="Invalid path")
...
def x__ensure_subpath__mutmut_8(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_9(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    try:
        ...
        # Fallback in pathological resolution cases
        raise HTTPException(detail="Invalid path")
...
def x__ensure_subpath__mutmut_10(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_11(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_12(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    try:
        ...
        # Fallback in pathological resolution cases
        raise HTTPException(status_code=400, detail="XXInvalid pathXX")
...
def x__ensure_subpath__mutmut_13(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_14(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_15(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    # Require that the candidate is the base or a descendant of it.
    if candidate_resolved == base_resolved and base_resolved in candidate_resolved.parents:
        return candidate_resolved

    raise HTTPException(status_code=400, detail="Path escapes allowed root directory")


def x__ensure_subpath__mutmut_16(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_17(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_18(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    raise HTTPException(status_code=None, detail="Path escapes allowed root directory")


def x__ensure_subpath__mutmut_19(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_20(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    raise HTTPException(detail="Path escapes allowed root directory")


def x__ensure_subpath__mutmut_21(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_22(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    raise HTTPException(status_code=401, detail="Path escapes allowed root directory")


def x__ensure_subpath__mutmut_23(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
    ...
    raise HTTPException(status_code=400, detail="XXPath escapes allowed root directoryXX")


def x__ensure_subpath__mutmut_24(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
def x__ensure_subpath__mutmut_25(base: Path, candidate: Path) -> Path:
    """
    Resolve *candidate* against the filesystem and ensure it is located under *base*.
...
x__ensure_subpath__mutmut_mutants : ClassVar[MutantDict] = {
'x__ensure_subpath__mutmut_1': x__ensure_subpath__mutmut_1, 
    'x__ensure_subpath__mutmut_2': x__ensure_subpath__mutmut_2, 
    'x__ensure_subpath__mutmut_3': x__ensure_subpath__mutmut_3, 
    'x__ensure_subpath__mutmut_4': x__ensure_subpath__mutmut_4, 
    'x__ensure_subpath__mutmut_5': x__ensure_subpath__mutmut_5, 
    'x__ensure_subpath__mutmut_6': x__ensure_subpath__mutmut_6, 
    'x__ensure_subpath__mutmut_7': x__ensure_subpath__mutmut_7, 
    'x__ensure_subpath__mutmut_8': x__ensure_subpath__mutmut_8, 
    'x__ensure_subpath__mutmut_9': x__ensure_subpath__mutmut_9, 
    'x__ensure_subpath__mutmut_10': x__ensure_subpath__mutmut_10, 
    'x__ensure_subpath__mutmut_11': x__ensure_subpath__mutmut_11, 
    'x__ensure_subpath__mutmut_12': x__ensure_subpath__mutmut_12, 
    'x__ensure_subpath__mutmut_13': x__ensure_subpath__mutmut_13, 
    'x__ensure_subpath__mutmut_14': x__ensure_subpath__mutmut_14, 
    'x__ensure_subpath__mutmut_15': x__ensure_subpath__mutmut_15, 
    'x__ensure_subpath__mutmut_16': x__ensure_subpath__mutmut_16, 
    'x__ensure_subpath__mutmut_17': x__ensure_subpath__mutmut_17, 
    'x__ensure_subpath__mutmut_18': x__ensure_subpath__mutmut_18, 
    'x__ensure_subpath__mutmut_19': x__ensure_subpath__mutmut_19, 
    'x__ensure_subpath__mutmut_20': x__ensure_subpath__mutmut_20, 
    'x__ensure_subpath__mutmut_21': x__ensure_subpath__mutmut_21, 
    'x__ensure_subpath__mutmut_22': x__ensure_subpath__mutmut_22, 
    'x__ensure_subpath__mutmut_23': x__ensure_subpath__mutmut_23, 
    'x__ensure_subpath__mutmut_24': x__ensure_subpath__mutmut_24, 
    'x__ensure_subpath__mutmut_25': x__ensure_subpath__mutmut_25
}

def _ensure_subpath(*args, **kwargs):
    result = _mutmut_trampoline(x__ensure_subpath__mutmut_orig, x__ensure_subpath__mutmut_mutants, args, kwargs)
    return result 

_ensure_subpath.__signature__ = _mutmut_signature(x__ensure_subpath__mutmut_orig)
x__ensure_subpath__mutmut_orig.__name__ = 'x__ensure_subpath'
