...
def fetch_rows(
    ...
    try:
        ...
        sql = f"SELECT {select_list} FROM {table}"  # nosec B608
        params: list[object] = []
        where_clause = ""
        if session_id:
            if not sid_col:
                raise RuntimeError("Session filtering requested but no session id column found")
            where_clause = f" WHERE {sid_col}=?"  # nosec B608
            params.append(session_id)

        if last_n is not None:
            inner_sql = (  # nosec B608
                f"SELECT {select_list} FROM {table}{where_clause} "  # nosec B608
