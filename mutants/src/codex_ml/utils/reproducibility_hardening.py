...
def enable_deterministic_training(seed: int = 42, *, strict: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logger.debug("✓ PyTorch CUDA seeded")
            ...
            logger.debug("✓ CUBLAS workspace configured")
        else:
...
def save_env_snapshot(output_path: Path | str, include_pip_freeze: bool = True) -> dict[str, Any]:
    ...
    """
    logger.info(f"Capturing environment snapshot to {output_path}")
    ...
    logger.info(f"✓ Environment snapshot saved to {output_path} and {json_path}")
...
def create_reproducibility_manifest(
    ...
    logger.info(f"✓ Reproducibility manifest created: {manifest_path}")
