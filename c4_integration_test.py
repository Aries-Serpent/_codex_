#!/usr/bin/env python3
"""
C4: Training Pipeline Integration Testing
...
def c4_3_checkpoint_resume() -> dict[str, Any]:
...
def c4_5_performance_comparison() -> dict[str, Any]:
    ...
    try:
        ...
        # Test 2: Legacy API Performance
        logger.info("Testing Legacy API performance...")
        try:
            from codex_ml.training.legacy_api import TrainingRunConfig, run_functional_training

            output_dir_legacy = Path("runs/c4_perf_legacy")
            output_dir_legacy.mkdir(parents=True, exist_ok=True)

            cfg_legacy = TrainingRunConfig(
                model="dummy",
                max_epochs=1,
                batch_size=2,
                learning_rate=0.001,
                seed=42,
                output_dir=str(output_dir_legacy),
                deterministic=True,
                mlflow_enable=False,
            )
            ...
            try:
                result_legacy = run_functional_training(cfg_legacy)
                results["legacy_time_seconds"] = time.perf_counter() - start_legacy
                logger.info(f"✓ Legacy API completed in {results['legacy_time_seconds']:.4f}s")
            except Exception as e:
                logger.warning(f"Legacy API test failed: {e}")
                results["errors"].append(f"Legacy API: {e}")
                results["legacy_time_seconds"] = None

        except ImportError as e:
            logger.warning(f"Legacy API not available for comparison: {e}")
            results["notes"].append("Legacy API not available for performance comparison")
            results["legacy_time_seconds"] = None

        # Calculate performance delta
        if (
            results["unified_time_seconds"] is not None
            and results["legacy_time_seconds"] is not None
        ):
            # Positive = unified is faster
            delta_percent = (
                (results["legacy_time_seconds"] - results["unified_time_seconds"])
                / results["legacy_time_seconds"]
                * 100
            )
            results["performance_delta_percent"] = delta_percent
            results["unified_faster"] = delta_percent > 0
            logger.info(
                f"Performance delta: {delta_percent:.2f}% "
                f"({'unified faster' if results['unified_faster'] else 'legacy faster'})"
            )
        else:
            results["notes"].append("Could not calculate performance delta due to missing timing data")

        results["status"] = "PASS" if not results["errors"] else "WARN"
...
def main() -> None:
    ...
    all_results["C4.2"] = c4_2_end_to_end_training()
    all_results["C4.3"] = c4_3_checkpoint_resume()
