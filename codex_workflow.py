...
TARGETS = [
    ...
    {"path": Path("tests/test_logging_viewer_cli.py"), "import_name": "json", "label": "t2"},
...
def assert_clean_state():
    ...
    if code != 0:
        ...
        sys.exit(2)
    if out.strip():
        record_error("1.1", "Working tree not clean", out, "Commit/stash changes before running workflow")
        sys.exit(3)
...
def ensure_branch(branch: str):
    ...
    if out != branch:
        ...
        if code2 != 0:
            record_error("1.2", f"Checkout branch '{branch}'", err2 or _out2, f"git checkout {branch}")
...
def scan_inventory():
    exts = {".py", ".sh", ".sql", ".js", ".ts", ".jsx", ".tsx", ".html", ".md", ".yml", ".yaml"}
    items = []
    for path in REPO_ROOT.rglob("*"):
        if path.is_file() and path.suffix.lower() in exts:
            try:
                items.append({"path": str(path.relative_to(REPO_ROOT)), "size": path.stat().st_size, "ext": path.suffix.lower()})
            except Exception as e:
...
def detect_workflows():
    ...
    return {"workflows_present": bool(present), "note": "DO NOT ACTIVATE ANY GitHub Actions files."}
...
def find_insert_index(src: str) -> int:
    ...
    if i < len(lines) and re.match(r"#\s*-\*-\s*coding\s*:\s*.*-\*-\s*$", lines[i]):
        i += 1
    # module docstring
    def is_triple_start(s): return s.lstrip().startswith('"""') or s.lstrip().startswith("'''")
...
def phase_1():
    ...
    append_file(CHANGE_LOG, f"- Constraint: DO_NOT_ACTIVATE_GITHUB_ACTIONS={DO_NOT_ACTIVATE_GITHUB_ACTIONS}\n")
...
def phase_2_mapping():
    ...
    for t in TARGETS:
        p = REPO_ROOT / t["path"]
        rationale = "Exact path target" if p.exists() else "Primary target missing; will search fallback by filename"
        mapping_rows.append({
            "task": t["label"],
...
def best_effort_edit(target_path: Path, import_name: str, label: str):
    ...
    try:
        ...
        return
    if after == before:
        record_change(p, "no-op (import already present)", f"'{import_name}' detected via AST", before[:0], after[:0])
    else:
        try:
            write_text(p, after)
            record_change(p, "insert_import", f"Add '{import_name}' while preserving header/future imports", before, after)
        except Exception as e:
...
def phase_3_best_effort():
    ...
    if readme.exists():
        try:
            _ = read_text(readme)
            append_file(CHANGE_LOG, "- README reviewed: no path updates required for test files.\n")
        except Exception as e:
