"""Context manager for logging chat conversations.

Initializes a session via ``codex.logging.conversation_logger`` and propagates
``CODEX_SESSION_ID`` so related components can access the current session.
"""
...
class ChatSession:
    ...
    def __enter__(self) -> ChatSession:  # type: ignore[override]
